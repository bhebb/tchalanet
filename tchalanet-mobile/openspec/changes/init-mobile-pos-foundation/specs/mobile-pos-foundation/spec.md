...
### Requirement: Mobile runtime settings support V1 feature toggles

The Mobile application SHALL load runtime settings and use them as the V1 feature-toggle/config mechanism until future Unleash integration.

#### Scenario: Missing setting defaults safely

- **GIVEN** a backend setting is absent
- **WHEN** the mobile app checks the setting
- **THEN** it SHALL use a safe default.
