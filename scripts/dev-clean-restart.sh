...
pkill -f "vite.*tchalanet-web" || echo "  No Vite processes found"
echo
...
rm -rf "$ROOT/dist/apps/tchalanet-web"
echo "✔ Dist cleaned"
echo

# 4. Afficher la config allowedHosts
echo "→ Step 4: Verifying vite.config.mts..."
if grep -q "allowedHosts" "$ROOT/apps/tchalanet-web/vite.config.mts"; then
  echo "✔ allowedHosts is configured:"
  grep -A 5 "allowedHosts" "$ROOT/apps/tchalanet-web/vite.config.mts" | head -6
else
