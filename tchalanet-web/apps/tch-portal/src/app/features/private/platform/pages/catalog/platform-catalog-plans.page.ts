...
import { PlatformCatalogApi, CatalogPlanView, CreatePlanRequest } from '../../platform-catalog-api.service';
...
export class PlatformCatalogPlansPage implements OnInit {
  ...
  openCreate(): void {
    ...
    ref.afterClosed().subscribe((created: CatalogPlanView | null) => {
  ...
  deactivate(plan: CatalogPlanView): void {
    ...
    this.api.deactivatePlan(plan.id.value).subscribe({
  ...
  delete(plan: CatalogPlanView): void {
    ...
    this.api.deletePlan(plan.id.value).subscribe({
