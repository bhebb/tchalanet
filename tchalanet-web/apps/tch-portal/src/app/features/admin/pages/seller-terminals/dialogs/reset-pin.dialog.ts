...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>Réinitialiser le PIN seller-terminal — {{ data.displayName }}</h2>
    <mat-dialog-content>
      <form [formGroup]="form" class="reset-pin-dialog__form">
        <mat-form-field appearance="outline" class="reset-pin-dialog__field">
          ...
          <input matInput formControlName="pin" type="password" inputmode="numeric" maxlength="6" placeholder="6 chiffres" />
          @if (form.controls.pin.invalid && form.controls.pin.touched) {
            <mat-error>6 chiffres uniquement.</mat-error>
          }
        </mat-form-field>
        @if (error()) {
