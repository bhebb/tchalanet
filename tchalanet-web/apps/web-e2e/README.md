...
app. Each app is a Playwright _project_ in `playwright.config.cts` (own base URL,
...
The full-validation workflow always runs this deterministic REST-stub variant.
When a disposable runtime is available, it tunnels only its Firebase emulator
so browser sign-in uses the same bootstrap identities; server E2E owns the real
API/runtime assertions. The API-backed variant below remains a focused local
integration tool, not the full-validation suite.
...
### GitHub workflow handoff — disposable runtime

In `full-validation.yml`, the runtime deploy is the source of truth for the
Firebase emulator coordinates. Web E2E keeps REST stubbed, so it does not
consume the disposable API base URL. The deploy job publishes:
...
The runtime files are not rewritten in CI. They stay on their checked-in test
profile and `ApiStub` supplies all REST payloads. This keeps browser results
reproducible even when the disposable database starts empty.
