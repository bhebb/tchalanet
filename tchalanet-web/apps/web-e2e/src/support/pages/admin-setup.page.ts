...
export class AdminSetupPage {
  ...
  async expectReadinessCards(): Promise<void> {
    ...
    await expect(this.page.getByTestId('admin-setup-card-pos_settings')).toBeVisible();
  }
  ...
  async expectReadySetupWithOperationalPrintingIssue(): Promise<void> {
    ...
    await expect(this.page.getByTestId('admin-setup-card-pos_settings').locator('.setup__card--ready')).toBeVisible();
    const posPrintingCard = this.page.getByTestId('admin-setup-card-pos_settings');
