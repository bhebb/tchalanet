 ...
 * UI-observable only. Runs backend-free via the `apiStub` fixture (REST is
 * stubbed; the public shell also has a fallback bundle). See
 * specs/web-e2e-auth-phase1.
 */
test.describe('Phase 1 — public', () => {
  test('public shell is reachable anonymously', async ({ page, loginPage, apiStub }) => {
    void apiStub; // fixture presence installs the REST stubs
    const response = await page.goto('/');
    expect(response?.ok()).toBe(true);
    await expect(loginPage.root).toHaveCount(0);
  });

  test('login page is reachable from the public app', async ({ loginPage, apiStub }) => {
    void apiStub;
