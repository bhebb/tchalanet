...
- [x] 0.4 Confirm backend ownership: handoff orchestration in the existing platform identity/auth
      boundary; support access in `platform.accesscontrol`.
- [x] 0.5 Confirm target portal URL runtime config keys and allowed entry routes.
      Web already exposes `portalBaseUrls`; backend needs trusted portal URL properties for
      `targetUrl`. Initial allow-list follows existing private routes:
      `/app/admin`, `/app/platform`, `/app/account/activation`, `/app/profile`,
      `/app/seller-terminal/activation`.

## 1. Backend — handoff slice (`platform.auth` contract)

- [x] 1.1 Handoff record + repository: `handoffId`, `codeHash` (SHA-256), `subjectUserId`,
      `targetPortal`, `entryRoute`, `supportAccessSessionId`, `createdAt`, `expiresAt` (TTL 60 s),
      `consumedAt`; DB-backed.
- [x] 1.2 `POST /platform/auth/portal-handoffs`: authenticate creator, verify target-portal
...
- [x] 1.3 `POST /platform/auth/portal-handoffs/{handoffId}/consume`: anonymous, code in body,
...
- [x] 1.4 Rate limiting on consume, per IP and per `handoffId`.
- [x] 1.5 Audit events:
...
- [x] 2.1 Server-backed support access session record: super admin uid, tenant, mode, reason,
      `grantedAt`, `expiresAt` (30–60 min), cleared/stopped state, audit metadata.
- [x] 2.2 Start endpoint creates the session, wired from existing tenant admin access flow near
      `TenantAdminController`.
- [x] 2.3 `GET /platform/tenants/admin-access/current` for post-handoff restoration; keep existing
      `DELETE .../current` as stop.
- [x] 2.4 `EffectiveTenantResolver` resolves tenant from the active server-side session for super
      admins; expired/cleared sessions never authorize support mode.
- [x] 2.5 Audit events: `SUPPORT_ACCESS_{STARTED,RESTORED,STOPPED,EXPIRED}`.

## 3. Web — public-portal

- [x] 3.1 `AuthRedirectService`: compare target portal base URL from runtime config with current
...
- [x] 3.2 `login.page.ts`: wire post-sign-in flow through the redirect service; never place ID
      tokens in URLs.

## 4. Web — admin-portal and platform-portal

- [x] 4.1 `/login/handoff` route in both apps, registered ahead of auth guards.
- [x] 4.2 Handoff page: read fragment, strip from history (`replaceUrl: true`), consume, sign in
      with custom token, load `/runtime/private`, navigate to entry route.
- [x] 4.3 Failure path: local login page with non-sensitive error code; never bounce back with the
      code in the URL.
- [x] 4.4 Admin-portal: hydrate `SupportAccessStore` from `GET .../admin-access/current` on app
...
- [x] 4.5 Platform-portal `start-tenant-admin-access-dialog`: create server session, then reuse the
      same handoff mechanism toward admin-portal.
- [x] 4.6 Verify route/guard ordering: existing landing guard (valid-local-session check) and
      shared login page unchanged; `/login/handoff` reachable without a session.
- [x] 4.7 Support-mode UI (sidebar, banner, tenant indicator, stop button, back-to-platform link)
...
- [x] 6.1 Update `tchalanet-web/docs/conventions/auth.md` with the handoff flow and same-origin
      fast path.
