...
### Requirement: Tout scheduler draw SHALL posséder @SchedulerLock

Tout composant `@Scheduled` dans les packages `core.draw.*` et `core.drawresult.*` SHALL être annoté `@SchedulerLock` avec des valeurs `lockAtMostFor` et `lockAtLeastFor`
...
### Requirement: Tout scheduler draw SHALL vérifier BatchGate

Tout composant `@Scheduled` dans les packages `core.draw.*` et `core.drawresult.*` SHALL appeler `batchGate.enabled(<key>, null)` en première instruction et retourner
...
### Requirement: Les schedulers draws SHALL éviter les crons hardcodés

Toute `@Scheduled(cron = ...)` dans les packages `core.draw.*` et `core.drawresult.*` SHALL utiliser une expression `${tch.draw.<scope>.<method>_cron:<défaut>}` résolue
...
### Requirement: DrawResultIngestedEvent SHALL être replay-safe

L'événement `DrawResultIngestedEvent` traité par `DrawDomainEventListener.onDrawResultIngested` SHALL être idempotent : rejouer l'événement avec le même `eventId` ne doit pas déclencher
