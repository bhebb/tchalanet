...
- [ ] A.1 Supprimer `features/pagemodel_backup/` entier
...
- [ ] A.2 Supprimer `PageModelRepository` (@RepositoryRestResource)
...
- [ ] A.3 Supprimer `features/pagemodel/admin/` entier
...
- [ ] A.4 Supprimer `features/pagemodel/PageModelController.java`
...
- [ ] A.5 Supprimer `features/pagemodel/PageModelOrchestrator.java`
...
- [ ] A.6 Supprimer `features/pagemodel/PageModelService.java`
...
- [ ] A.7 Supprimer `features/pagemodelruntime/PageModelController.java`
...
- [ ] `./mvnw clean compile` passe sans erreur
- [ ] Aucun conflit `@RequestMapping` détecté (vérifier avec `grep -r "pagemodel" --include="*.java" | grep "@RequestMapping"`)
- [ ] Aucun `@RepositoryRestResource` dans `features/pagemodel/`
...
- [ ] B.1 Supprimer `features/pagemodel/PageModel.java` (record doublon)
...
- [ ] B.2 Supprimer `features/pagemodel/PageModelType.java` (enum doublon)
...
- [ ] B.3 Supprimer `features/pagemodel/PageStatus.java` (enum doublon)
...
- [ ] B.4 Supprimer `features/pagemodel/PageModelEntity.java` (entité doublon)
...
- [ ] B.5 Supprimer `features/pagemodel/PageModelResponse.java`
...
- [ ] B.6 Supprimer `PageModelDynamicResolver.toShared()` (bridge inutile)
...
- [ ] B.7 Supprimer `core/application/port/PageModelReadPort.java` (doublon de port/out/)
...
- [ ] B.8 Supprimer `core/application/port/PageModelWritePort.java` (doublon de port/out/)
...
- [ ] `./mvnw clean compile` passe sans erreur
- [ ] `grep -r "features.pagemodel.PageModel[^D]" --include="*.java"` retourne 0 résultat
- [ ] `grep -r "features.pagemodel.PageModelType" --include="*.java"` retourne 0 résultat
- [ ] `grep -r "application.port.PageModelReadPort" --include="*.java"` retourne 0 résultat (sauf port/out)
...
- [ ] C.1 Créer slice `features/pagemodel/public/`
      Fichier(s) :
...
- [ ] C.2 Créer slice `features/pagemodel/dashboard/`
      Fichier(s) :
...
- [ ] C.3 Créer slice `features/pagemodel/onboarding/`
      Fichier(s) :
...
- [ ] C.4 Organiser `features/pagemodel/shared/`
      Fichier(s) :
...
- [ ] C.5 Organiser `features/pagemodel/dynamic/`
      Fichier(s) :
...
- [ ] C.6 Supprimer `features/pagemodelruntime/` entier
...
- [ ] `./mvnw clean compile` passe
- [ ] `GET /api/v1/public/pagemodel/public.home` → HTTP 200 (test manuel ou curl)
- [ ] `GET /api/v1/tenant/pagemodel/dashboard` avec token CASHIER → HTTP 200
- [ ] Aucun import `core.pagemodel.infra.init.PageModelBootstrapService` (supprimé)
- [ ] Aucun import `features.pagemodelruntime.*` (supprimé)
...
- [ ] D.1 Fix `PageModelPersistenceAdapter.list()` — cross-tenant
...
- [ ] D.2 Fix import `PageModelType` dans `PageModelTemplateSeedRunner`
...
- [ ] D.3 Vérifier + créer policy RLS sur table `page_model`
...
- [ ] D.5 Fix `PublishPageModelHandler` — archiver le PUBLISHED précédent
...
- [ ] `grep -r "features.pagemodel.PageModelType" --include="*.java"` → 0 résultats
- [ ] Policy RLS confirmée active sur `page_model` (V40, V41 ou V52)
- [ ] `PageModelId.java` existe dans `core/pagemodel/domain/model/`
...
- [ ] E.1 Créer `DrawsProvider` (source: `results_by_game`)
...
- [ ] E.2 Créer `PlansProvider` (source: `plans`)
...
- [ ] E.3 Créer `HeroProvider` (source: `hero`)
...
- [ ] E.4 Fix `CashierOverviewProvider` — remplacer le stub par des données réelles
...
- [ ] `GET /api/v1/public/pagemodel/public.home` → `dynamic.widgets` non vide (si draws/plans existent en base)
- [ ] `dynamic.errors` vide ou limité à des widgets non encore implémentés côté Angular
- [ ] `GET /api/v1/public/pagemodel/private.dashboard.cashier` → `dynamic.widgets.overview` non null

---

## Completion Checklist

- [ ] Groupe A complet — `./mvnw clean compile` passe
- [ ] Groupe B complet — 0 doublon de types entre features/ et core/
- [ ] Groupe C complet — Slices créées, pagemodelruntime/ supprimé
- [ ] Groupe D complet — RLS OK, TypedId OK, handlers corrects
- [ ] Groupe E complet — 4 providers branchés
- [ ] `./mvnw clean verify` passe (tests existants non cassés)
- [ ] Aucun conflit `@RequestMapping` dans le module pagemodel
- [ ] Aucune violation hexagonale `core/ → features/`
- [ ] Documentation mise à jour : `FEATURE_PAGEMODEL.md` + `PAGEMODEL-ARCHITECTURE-CIBLE.md`
- [ ] Prêt pour `/openspec-archive 90-finish-pagemodel-migration-20260424`
