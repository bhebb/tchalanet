...
- [ ] 3.1 Inventorier les migrations historiques V1–V54 à remplacer par la baseline cible `V001`, `V100`, `V101`, `V102`, `V103`, `V104`, `V105`, `V106`, `V107`
- [ ] 3.1 bis Inventorier **tous** les `ALTER TABLE`, `ALTER INDEX`, `ALTER SEQUENCE`, `ALTER POLICY`, changements de `DEFAULT`, contraintes `CHECK`, triggers et permissions dans les migrations existantes
- [ ] 3.1 ter Pour chaque objet SQL, produire son **état final consolidé** : la dernière mutation historique gagne et doit être intégrée directement dans la baseline
...
- [ ] 3.3 Tables entièrement absentes de Flyway → les intégrer dans `V100__create_core_tables.sql` si métier, dans `V101__create_audit_tables.sql` si Envers, ou dans `V102__create_technical_tables.sql` si table technique
- [ ] 3.3 bis Lister explicitement les tables core à couvrir dans `V100__create_core_tables.sql`
- [ ] 3.3 ter Lister explicitement les tables techniques à couvrir dans `V102__create_technical_tables.sql` : `processed_event`, `idempotency_record`, `stats_draw`, `stats_daily`, `stats_event_log`, `shedlock`
- [ ] 3.3 quater Isoler les tables Spring Batch dans `V107__spring_batch_schema.sql` et ne pas les mélanger aux autres tables techniques

- [ ] 3.4 Lister les tables `*_AUD` à créer (uniquement pour les entités classées ✅ en tâche 1.2) et les tables `*_AUD` stales à corriger, puis les intégrer dans `V101__create_audit_tables.sql`

---

## 4. Découpage cible : baseline centralisée recommandée

- [ ] 4.1 Créer ou ajuster les fichiers suivants :
...
- [ ] 4.2 Dans `V001__extensions_and_rls_helpers.sql` : standardiser `current_tenant()`, `deleted_visibility()`, reset contexte
- [ ] 4.3 Dans `V100__create_core_tables.sql` : créer toutes les tables métier avec PK/UK/CHECK/FK simples et colonnes d'audit standard
- [ ] 4.3 bis Dans `V100__create_core_tables.sql` : ne garder que les tables métier, avec leur état final consolidé
- [ ] 4.4 Dans `V101__create_audit_tables.sql` : créer `revinfo` et toutes les tables `*_AUD` retenues au format Envers strict
- [ ] 4.5 Dans `V102__create_technical_tables.sql` : créer `processed_event`, `idempotency_record`, `stats_draw`, `stats_daily`, `stats_event_log`, `shedlock`, avec leur état final consolidé
- [ ] 4.6 Dans `V103__create_indexes.sql` : créer tous les index métier et RLS, en priorité ceux sur `(tenant_id, ...)`
- [ ] 4.7 Dans `V104__create_triggers.sql` : créer `set_updated_at()`, `increment_draw_exposure(...)` si conservée comme fonction technique, et les triggers techniques
- [ ] 4.8 Dans `V105__configure_rls.sql` : activer RLS et créer les policies sur toutes les tables `BaseTenantEntity`
- [ ] 4.9 Dans `V106__configure_permissions.sql` : définir owner / grants si nécessaire
- [ ] 4.10 Dans `V107__spring_batch_schema.sql` : conserver les tables `BATCH_*` dans un fichier séparé des autres migrations
- [ ] 4.11 Ajouter au design SQL une convention de naming explicite pour tables, colonnes, index, contraintes, triggers, fonctions et policies
- [ ] 4.12 Pour chaque colonne métier modifiée par consolidation, tracer les impacts Java : entité JPA, mapper, adapter persistence, use cases, read models, couche web/feature si exposée

---

## 5. Nettoyage des fichiers de migration existants

- [ ] 5.1 Reprendre les définitions utiles des migrations historiques sans conserver les divergences (`address`, `draw_exposure`, tables audit stales, etc.)
- [ ] 5.2 Supprimer de la baseline toute référence à `current_setting('app.tenant_id', ...)` au profit de `public.current_tenant()` et `public.deleted_visibility()`
- [ ] 5.3 Vérifier que la baseline ne contient aucun `DROP TABLE`, aucun `ALTER TABLE ADD/DROP COLUMN`, aucun bloc PL/pgSQL conditionnel de rattrapage
- [ ] 5.4 Vérifier que chaque `ALTER` historique a été absorbé dans une définition finale ou dans le bon fichier cible (`V102`, `V103`, `V104`, `V105`, `V106`, `V107`) sans perte de comportement
- [ ] 5.5 Vérifier que chaque colonne métier consolidée est alignée côté code Java et couche applicative quand elle porte un sens métier
...
- [ ] 6.7 Vérifier qu'aucun `DROP TABLE` ne subsiste dans la baseline cible
...
- [ ] 7.5 Ajouter un contrôle ciblé ou une checklist de revue pour vérifier qu'une colonne métier ajoutée/supprimée dans SQL est aussi reflétée dans l'entité et la couche applicative concernée
...
- [ ] 8.3 ✅ Aucun `DROP TABLE` dans la baseline cible
...
- [ ] 8.10 ✅ `theme_preset` et `user_notification` créés dans `V100__create_core_tables.sql`
...
- [ ] 8.14 ✅ Les tables audit sont regroupées dans `V101__create_audit_tables.sql`, séparées des tables métier et des tables techniques
- [ ] 8.15 ✅ Toutes les altérations historiques pertinentes sont absorbées dans l'état final de la baseline
- [ ] 8.16 ✅ Les tables techniques (`processed_event`, `idempotency_record`, `stats_*`, `shedlock`) sont explicitement couvertes dans `V102__create_technical_tables.sql`
- [ ] 8.17 ✅ Les tables Spring Batch sont isolées dans `V107__spring_batch_schema.sql`
- [ ] 8.18 ✅ Toute colonne métier consolidée est alignée côté SQL, entité JPA et couche applicative concernée
