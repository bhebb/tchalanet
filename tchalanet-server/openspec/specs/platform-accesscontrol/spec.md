# platform-accesscontrol Specification

## Purpose

`platform.accesscontrol` owns authorization policy: roles, permissions, assignments and permission
decisions. It answers whether an actor may attempt an action in the current scope.

It does not validate domain resource state. Business invariants remain in the owning `core` domain.
