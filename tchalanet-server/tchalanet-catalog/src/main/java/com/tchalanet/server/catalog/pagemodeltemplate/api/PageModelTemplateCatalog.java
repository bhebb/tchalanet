...
public interface PageModelTemplateCatalog {
  ...
  /**
   * List templates visible under current RLS context. (GLOBAL + TENANT for current tenant, based on
   * DB policy)
   */
  List<PageModelTemplateView> listVisible();

  /** Optional: paged search if you need admin screens. */
  TchPage<PageModelTemplateView> search(
   ...
   *
   * <p>Ordering should be stable by code/logicalId.
   */
