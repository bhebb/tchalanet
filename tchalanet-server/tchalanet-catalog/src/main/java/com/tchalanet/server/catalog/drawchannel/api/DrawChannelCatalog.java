 ...
 *
 * <p>This interface defines the public catalog operations (read/query) related to draw channels
 ...
 *
 * <p>Important contracts: - This catalog is tenant-scoped: callers must provide a non-null {@link
 * TenantId} for tenant oriented queries. Platform-wide (cross-tenant) queries belong to a distinct
 * API (e.g. platform search) and should not be performed through this interface. - The catalog is
 * read-only: it returns Views/DTOs and never JPA entities.
 */
public interface DrawChannelCatalog {
   ...
   *
   * <p>Contract: - tenantId and id must be non-null. - returns Optional.empty() when the channel
   * does not exist or is soft-deleted or does not belong to the tenant.
   *
   ...
   *
   * <p>Notes: - Code lookup is tenant-scoped and should match exactly. - Returns Optional.empty()
   * if no channel matches or if the channel is deleted.
   *
   * @param tenantId tenant scope (must not be null)
   ...
   * @return List of {@link DrawChannelGameView} describing per-game settings for that channel (may
   *     be empty).
   */
   ...
   *
   * <p>Replaces the previous ambiguous Map-based API. Returns a stable list where each element
   ...
   *
   * <p>The method is explicit about which filters are applied. Implementations should apply the
   ...
   *
   * @param tenantId tenant scope (must not be null)
   * @param activeOnly when true, only include channels with channel.active == true; when null, do
   *     not filter by channel active flag.
   * @param enabledOnly when true, only include calendar rows that have at least one enabled game
   *     association (or reflect enabled semantics per implementation); when null, do not filter by
   *     enabled association.
   * @return list of {@link DrawChannelCalendarRow} (may be empty)
   */
  List<DrawChannelCalendarRow> listCalendarRows(
   ...
   *
   * <p>Use {@link DrawChannelSearchCriteria} to express tenant-scoped or platform-scoped search;
   * when using this catalog (tenant-scoped) pass a non-null tenantId in the criteria. For
   * platform/cross-tenant search use a platform API (not this tenant-scoped catalog) or a dedicated
   * searchPlatform method.
   *
