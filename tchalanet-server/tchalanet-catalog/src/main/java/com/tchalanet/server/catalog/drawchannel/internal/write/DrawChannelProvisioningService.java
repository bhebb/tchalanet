...
public class DrawChannelProvisioningService implements DrawChannelProvisioningApi {
  ...
  private static final Set<String> DEFAULT_HAITI_GAME_CODES =
      Set.of("HT_BOLET", "HT_MARYAJ", "HT_MARYAJ_GRATIS", "HT_LOTO3", "HT_LOTO4", "HT_LOTO5");
  ...
  public void ensureDefaultHaitiLotteryChannels(
    ...
    for (DefaultDrawChannel row : defaultHaitiDrawChannels()) {
      ...
      var gameRequests =
          enabledTenantGames.stream()
              .map(
                  game ->
                      new CreateDrawChannelGameRequest(
                          game.tenantGameId(), true, JsonUtils.emptyObject()))
              .toList();
      drawChannelGameAdminService.bulkUpsert(
          tenantId, DrawChannelId.of(channel.getId()), gameRequests);
    }
  }
  ...
  private DrawChannelEntity ensureChannel(TenantId tenantId, DefaultDrawChannel row) {
    ...
    var existing =
        drawChannelRepository.findFirstByTenantIdAndCodeIgnoreCaseAndDeletedAtIsNull(
            tenantId.value(), row.code());
