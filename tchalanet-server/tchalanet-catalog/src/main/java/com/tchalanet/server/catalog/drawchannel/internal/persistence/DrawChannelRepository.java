...
public interface DrawChannelRepository
    extends JpaRepository<DrawChannelEntity, UUID>, JpaSpecificationExecutor<DrawChannelEntity> {
  ...
  List<DrawChannelEntity> findByTenantIdAndActiveTrueAndDeletedAtIsNullOrderBySortOrderAsc(
      UUID tenantId);

  Optional<DrawChannelEntity> findByIdAndDeletedAtIsNull(UUID id);

  // Find channels for tenant by result_slot_id
  List<DrawChannelEntity> findByTenantIdAndResultSlotIdAndDeletedAtIsNull(
      UUID tenantId, UUID resultSlotId);
  ...
  @Query(
      ...
      """,
      nativeQuery = true)
  List<DrawChannelEntity> findByTenantIdAndResultSlotProviderAndKey(
      UUID tenantId, String provider, String slotKey);

  // Repository helper: find by tenant + code (case-insensitive)
  Optional<DrawChannelEntity> findFirstByTenantIdAndCodeIgnoreCaseAndDeletedAtIsNull(
