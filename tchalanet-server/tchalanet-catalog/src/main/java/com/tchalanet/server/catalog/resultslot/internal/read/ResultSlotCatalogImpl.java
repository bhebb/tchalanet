...
/**
 * Implementation of ResultSlotCatalog (read-only, cacheable). Maps to spec requirement G1 (read
 * operations).
 */
@Service
...
public class ResultSlotCatalogImpl implements ResultSlotCatalog {
  ...
  @Override
  @Cacheable(
      cacheNames = ResultSlotCacheNames.BY_KEY,
      key = "#slotKey == null ? '' : #slotKey.toLowerCase()")
  ...
  public ResultSlotView requireByKey(String slotKey) {
    ...
    return repo.findFirstBySlotKeyIgnoreCaseAndDeletedAtIsNull(slotKey)
        .map(mapper::toView)
        .orElseThrow(
            () -> new EntityNotFoundException("result_slot_not_found with slotKey=" + slotKey));
  }
