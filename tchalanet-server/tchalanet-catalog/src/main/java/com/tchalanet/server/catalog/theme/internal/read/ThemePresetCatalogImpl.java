...
public class ThemePresetCatalogImpl implements ThemeCatalog {
  ...
  public List<ThemePresetView> listActive() {
    var entities =
        repo.findAll().stream().filter(e -> e.isActive() && e.getDeletedAt() == null).toList();
    return entities.stream().map(mapper::toView).toList();
  }

  @Override
  @Cacheable(
      cacheNames = ThemeCacheNames.PRESET_BY_CODE,
      key = "#code == null ? '' : #code.toLowerCase()")
  ...
  public ThemePresetStatsView stats() {
    ...
    long active =
        repo.findAll().stream().filter(e -> e.isActive() && e.getDeletedAt() == null).count();
