...
/**
 * Implementation of PlanCatalog (read-only, cacheable). Maps to spec requirement P1 (read
 * operations) + P5 (cache). Filters deleted_at IS NULL on all reads.
 */
@Service
...
public class PlanCatalogImpl implements PlanCatalog {
  ...
  public List<PlanView> listActive() {
    return repository.findAllByActiveTrueAndDeletedAtIsNull().stream()
        .map(mapper::toView)
  ...
  public Optional<PlanView> findByCode(String code) {
    return repository.findFirstByCodeIgnoreCaseAndDeletedAtIsNull(code).map(mapper::toView);
  }
