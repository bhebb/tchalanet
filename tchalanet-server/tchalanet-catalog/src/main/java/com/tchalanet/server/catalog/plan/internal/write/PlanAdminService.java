...
/**
 * Admin service for Plan CRUD (internal write operations). Maps to spec requirement P2 (admin
 * writes). All writes evict relevant caches.
 */
@Service
...
public class PlanAdminService {
  ...
  private PlanJpaEntity getRequired(PlanId id) {
    return repository.findById(id.value()).orElseThrow(() -> ProblemRest.notFound("plan", id));
  }
  ...
  public record PlanFeaturesUpdateRequest(String featuresJson) {}

  public record PlanLimitsUpdateRequest(String limitsJson) {}
}
