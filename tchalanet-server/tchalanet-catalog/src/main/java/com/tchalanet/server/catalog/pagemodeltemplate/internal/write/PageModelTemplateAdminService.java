...
public class PageModelTemplateAdminService {
  ...
  public PageModelTemplateView createFromView(PageModelTemplateView view) {
    ...
    repository
        .findFirstByLogicalIdAndDeletedAtIsNull(view.logicalId())
        .ifPresent(
            x -> {
  ...
  @CacheEvict(
      ...
      allEntries = true)
  public PageModelTemplateView updateFromView(
      PageModelTemplateId id, PageModelTemplateView view, UserId actorId) {
    var existing =
        repository
            .findById(id.value())
    ...
    var schemaVersion =
        view.schemaVersion() != null ? view.schemaVersion() : saved.getSchemaVersion();
    ...
    AfterCommit.run(
        () ->
            eventPublisher.publishEvent(
                new PageModelTemplateUpdatedEvent(
                    id,
                    saved.getLogicalId(),
                    view.model(),
  ...
  public PageModelTemplateView upsertGlobalFromSeed(PageModelTemplateView seed) {
    ...
    if (seed.scope() == null || seed.scope().isBlank()) {
      throw new IllegalArgumentException(
          "Seed template requires scope for logicalId=" + seed.logicalId());
    }
    if (seed.slug() == null || seed.slug().isBlank()) {
      throw new IllegalArgumentException(
          "Seed template requires slug for logicalId=" + seed.logicalId());
    }
  ...
  @CacheEvict(
      ...
      allEntries = true)
  public PageModelTemplateView duplicate(
      PageModelTemplateId id, String newLogicalId, String newCode) {
    var source =
        repository
            .findById(id.value())
            .orElseThrow(() -> ProblemRest.notFound("page_model_template", id));
    ...
    String targetLogicalId =
        (newLogicalId != null && !newLogicalId.isBlank())
            ? newLogicalId
            : source.getLogicalId() + "-copy";
    String targetCode =
        (newCode != null && !newCode.isBlank()) ? newCode : source.getCode() + "-copy";

    repository
        .findFirstByLogicalIdAndDeletedAtIsNull(targetLogicalId)
        .ifPresent(
            x -> {
  ...
  private static @NonNull PageModelTemplateEntity createNewModelTemplateEntity(
      String targetCode, String targetLogicalId, PageModelTemplateEntity source) {
