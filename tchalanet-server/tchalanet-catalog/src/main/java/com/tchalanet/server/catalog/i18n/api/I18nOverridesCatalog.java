...
public interface I18nOverridesCatalog {
  ...
  /**
   * Resolve effective overrides for a locale. Returns a map where tenant overrides overwrite global
   * ones.
   *
   * <p>key = i18n_key value = i18n_value
   */
  ...
  /**
   * Load translations grouped by surface for runtime bootstrap. GLOBAL rows are merged under TENANT
   * rows (tenant wins per surface per key). Caller must validate surfaces against policy before
   * calling.
   */
