...
public class PageModelTemplateService {
    ...
    public PageModelTemplateService(PageModelTemplateRepository repository, PageModelService pageModelService) {
    ...
    public PageModelTemplateEntity update(UUID id, PageModelTemplateEntity dto, UUID actorId) {
        PageModelTemplateEntity existing = repository.findById(id)
    ...
    /**
     * Met à jour un template puis, si propagate=true, applique le model du template
     * à toutes les PageModel instances liées (safe mode : met les pages en DRAFT pour relecture).
     */
    public PageModelTemplateEntity updateAndPropagate(UUID id, PageModelTemplateEntity dto, UUID actorId, boolean propagate) {
        ...
        if (propagate) {
            ...
            pageModelService.applyTemplateToInstances(updated.getId(), updated.getModelJson(), updated.getSchemaVersion(), actorId, true);
        }
        return updated;
    }
    ...
    public PageModelTemplateEntity setDefault(UUID id, UUID actorId) {
        PageModelTemplateEntity toDefault = repository.findById(id)
                .orElseThrow(() -> new IllegalArgumentException("Template not found: " + id));
        String logicalId = toDefault.getLogicalId();

        List<PageModelTemplateEntity> siblings = repository.findAllByLogicalIdAndDeletedAtIsNull(logicalId);
        Instant now = Instant.now();
