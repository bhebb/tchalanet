...
public class LangResolver {
    ...
    public String resolve(LangResolverContext ctx) {
        ...
        // 1) URL
        Optional<String> candidate = ctx.langFromUrl()
            .filter(l -> isSupported(l, available));
        if (candidate.isPresent()) {
            return candidate.get();
        }

        // 2) user preference
        candidate = ctx.userPreferredLang()
            .filter(l -> isSupported(l, available));
        if (candidate.isPresent()) {
            return candidate.get();
        }
        ...
        // 3) tenant default
        candidate = ctx.tenantDefaultLang()
            .filter(l -> isSupported(l, available));
        if (candidate.isPresent()) {
            return candidate.get();
        }
