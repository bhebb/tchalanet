...
public class PageModelService {
    ...
    private static final UUID DEFAULT_TENANT_UUID = UUID.fromString("00000000-0000-0000-0000-000000000000");

    public PageModel loadByLogicalId(UUID tenantId, String logicalId) {
        PageModelEntity entity = repository
            .findByTenantIdAndLogicalIdAndDeletedAtIsNull(tenantId, logicalId)
            .orElseThrow(() -> new IllegalArgumentException("PageModel not found for logicalId=" + logicalId));
    ...
    /**
     * Charge le PageModel "effectif" utilisé au runtime, en ne considérant que les PUBLISHED.
     * Ordre de fallback :
     *  1) PUBLISHED pour le tenant courant
     *  2) PUBLISHED pour le tenant "default"
     *  3) template JSON embarqué dans les resources
     */
    public PageModel loadEffectiveModel(UUID tenantId, String logicalId) {
        // 1) PUBLISHED pour le tenant courant
        return repository
            .findByTenantIdAndLogicalIdAndStatusAndDeletedAtIsNull(tenantId, logicalId, PageStatus.PUBLISHED)
            .map(this::toDomain)
            // 2) PUBLISHED pour le tenant "default"
            .or(() -> repository
                .findByTenantIdAndLogicalIdAndStatusAndDeletedAtIsNull(DEFAULT_TENANT_UUID, logicalId, PageStatus.PUBLISHED)
                .map(this::toDomain))
    ...
    private PageModel loadFromTemplate(String logicalId) {
        ...
        if (!resource.exists()) {
            throw new IllegalStateException("No effective PageModel found and no template for logicalId=" + logicalId);
        }
        try (InputStream is = resource.getInputStream()) {
            return objectMapper.readValue(is, PageModel.class);
        } catch (IOException e) {
            throw new IllegalStateException("Unable to load PageModel template from resources: " + path, e);
        }
    }
    ...
    public PageModelEntity createFromTemplate(UUID tenantId, PageModelEntity template, boolean publish, UUID actorId) {
    ...
    public PageModelEntity createFromTemplate(UUID tenantId, PageModelTemplateEntity template, boolean publish, UUID actorId) {
        PageModelEntity e = new PageModelEntity();
        e.setTenantId(tenantId);
        e.setLogicalId(template.getLogicalId());
        e.setScope(null);
    ...
    public PageModelEntity duplicateToTenant(UUID sourceId, UUID targetTenantId, UUID actorId) {
        PageModelEntity src = repository.findById(sourceId)
            .orElseThrow(() -> new IllegalArgumentException("Source PageModel not found: " + sourceId));
    ...
    public void applyTemplateToInstances(UUID templateId, String modelJson, int schemaVersion, UUID actorId, boolean setDraft) {
