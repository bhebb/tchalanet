...
import com.tchalanet.server.features.privatedashboard.block.SuperadminOverviewBlock;
import com.tchalanet.server.features.privatedashboard.block.TenantAdminOverviewBlock;
import com.tchalanet.server.features.privatedashboard.block.TicketsBlock;
import com.tchalanet.server.features.privatedashboard.block.ValidationsBlock;
...
public class TenantAdminDashboardService {
    ...
    try {
      ...
      items.add(
          new KpiBlock.KpiItem(
              "sales",
              "kpi.sales",
              kpis.totalSales() != null ? kpis.totalSales().toString() : "0",
              ""));
      items.add(
          new KpiBlock.KpiItem(
              "payout",
              "kpi.payout",
              kpis.totalPayout() != null ? kpis.totalPayout().toString() : "0",
              ""));
      items.add(
          new KpiBlock.KpiItem(
              "ggr",
              "kpi.ggr",
              kpis.netRevenue() != null ? kpis.netRevenue().toString() : "0",
              ""));
  ...
  private KpiBlock buildSalesKpis(TenantDashboardStatsResponse statsResponse) {
    ...
    var items =
        List.of(
            new KpiBlock.KpiItem(
                "sales_total_period", "kpi.sales.period_total", total.toString(), ""),
  ...
  private ActivityFeedBlock buildActivity(TenantId tenantId, UserId userId, String currentLang) {
    try {
      var activities =
          listTenantRecentActivityQueryHandler.handle(new AuditEventQuery(tenantId, 20));
      ...
      var items =
          activities.stream()
              .map(
                  dto ->
                      new ActivityFeedBlock.ActivityItem(
  ...
  private TenantAdminOverviewBlock buildOverview(
      TenantId tenantId,
      UserId userId,
      @SuppressWarnings("unused") String currentLang,
      TenantDashboardStatsResponse tenantStats) {
    try {
      // use tenant default period from tenantStats
      var from =
          tenantStats != null && tenantStats.stats() != null
              ? tenantStats.stats().fromDate()
              : null;
      var to =
          tenantStats != null && tenantStats.stats() != null ? tenantStats.stats().toDate() : null;
  ...
  private List<
          com.tchalanet.server.features.stats.cashier_dashboard.dto.CashierDashboardStatsResponse>
      fetchCashierPreview(TenantId tenantId, UserId userId, LocalDate from, LocalDate to) {
    List<com.tchalanet.server.features.stats.cashier_dashboard.dto.CashierDashboardStatsResponse>
