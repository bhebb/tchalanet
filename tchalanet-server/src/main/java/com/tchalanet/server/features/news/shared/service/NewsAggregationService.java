...
public class NewsAggregationService {
  ...
  public List<LotteryNewsModels.LotteryNewsArticle> aggregateOrdered(Instant now) {
    ...
    // helper
    var notHidden =
        (java.util.function.Predicate<LotteryNewsModels.LotteryNewsArticle>)
            a -> !hiddenIds.contains(a.id());

    // 1) internes PUBLISHED, filtrées, triées desc
    var internal =
        internalNewsService.findPublished(now).stream()
            .filter(notHidden)
            .sorted(
    ...
    var external =
        externalNewsService.fetchArticles().stream()
            .filter(notHidden)
            .sorted(
                Comparator.comparing(LotteryNewsModels.LotteryNewsArticle::publishedAt).reversed())
            .toList();
    ...
    var result =
        new ArrayList<LotteryNewsModels.LotteryNewsArticle>(internal.size() + external.size());
