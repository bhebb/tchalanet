...
public class PageModelAdminController {
    ...
    /**
     * Liste les PageModel avec filtres optionnels :
     * - tenantId : null => global / tous
     * - scope    : "public" / "private" (optionnel)
     * - logicalId: ex. "public.home", "private.tenant_dashboard"
     */
    @GetMapping
    ...
    public ResponseEntity<PageModelAdminDetailDto> update(
        ...
        var withId = new PageModelAdminUpsertRequest(
            id,
            request.logicalId(),
            request.scope(),
            request.slug(),
    ...
    /**
     * Duplique un PageModel :
     * - newLogicalId / newSlug optionnels (sinon générés par le service).
     */
    ...
    /**
     * Preview "brute" du PageModel tel qu'il serait renvoyé au BFF,
     * pratique pour le backoffice visuel.
     */
    ...
    /**
     * Publie une version (draft -> published).
     * Règle métier: une seule version "published" par logicalId/tenant/scope.
     */
