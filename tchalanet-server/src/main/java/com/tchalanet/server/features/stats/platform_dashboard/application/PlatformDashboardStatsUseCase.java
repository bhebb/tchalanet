...
public class PlatformDashboardStatsUseCase {
    ...
    public PlatformDashboardStatsResponse handle(PlatformDashboardStatsQuery query) {
        ...
        List<StatsDailyEntity> platformRows = statsDailyRepo.findByDimensionTypeAndDimensionIdAndRefDateBetween("platform", null, from, to);
        ...
        for (StatsDailyEntity row : platformRows) {
            ...
            dailyPoints.add(new PlatformDailySalesPointDto(row.getRefDate(), row.getStakeSumCents(), row.getWinningsSumCents(), row.getNetRevenueCents()));
        }

        // 2) tenant aggregation across the period
        // fetch tenant rows in range
        var tenantRows = statsDailyRepo.findByDimensionTypeAndDimensionIdAndRefDateBetween("tenant", null, from, to);
        ...
        Map<UUID, String> tenantNames = tenantRepo.findAllById(tenantAgg.keySet()).stream()
                .collect(Collectors.toMap(com.tchalanet.server.core.tenant.infra.persistence.TenantJpaEntity::getId, com.tchalanet.server.core.tenant.infra.persistence.TenantJpaEntity::getName));
        // Note: above fully-qualified references are required in this context

        var tenants = tenantAgg.entrySet().stream()
                .map(e -> new PlatformTenantStatsDto(e.getKey(), tenantNames.getOrDefault(e.getKey(), "Tenant "+e.getKey()), e.getValue().ticketsCount, e.getValue().stakeSumCents, e.getValue().netRevenueCents))
        ...
        var cashierRows = statsDailyRepo.findByDimensionTypeAndDimensionIdAndRefDateBetween("cashier", null, from, to);
        ...
        var topCashiers = cashierAgg.entrySet().stream()
                .map(e -> new PlatformCashierStatsDto(e.getKey(), "Cashier " + e.getKey(), e.getValue().ticketsCount, e.getValue().stakeSumCents, e.getValue().netRevenueCents))
                .sorted(Comparator.comparingLong(PlatformCashierStatsDto::netRevenueCents).reversed())
                .limit(10)
                .toList();

        // 4) game breakdown: use stats_draw or tenant-level breakdown; for now aggregate by game_code using stats_draw
        var drawRows = statsDrawRepo.findByTenantIdAndScheduledAtBetween(null, from.atStartOfDay().toInstant(java.time.ZoneOffset.UTC), to.plusDays(1).atStartOfDay().toInstant(java.time.ZoneOffset.UTC));
        ...
        List<PlatformGameBreakdownItemDto> gameBreakdown = gameAgg.entrySet().stream()
                .map(e -> new PlatformGameBreakdownItemDto(e.getKey(), e.getValue().ticketsCount, e.getValue().stakeSumCents, e.getValue().netRevenueCents))
                .sorted(Comparator.comparingLong(PlatformGameBreakdownItemDto::netRevenueCents).reversed())
                .toList();
        ...
        PlatformSummaryCardDto summary = new PlatformSummaryCardDto(totalTenants, totalOutlets, totalCashiers, totalTickets, totalStake, totalWinnings, totalNet);

        return new PlatformDashboardStatsResponse(from, to, summary, tenants, dailyPoints, gameBreakdown, topCashiers);
    }
