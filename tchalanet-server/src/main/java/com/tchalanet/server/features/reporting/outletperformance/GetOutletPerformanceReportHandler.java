...
public class GetOutletPerformanceReportHandler {
    ...
    public OutletPerformanceReportResponse handle(GetOutletPerformanceReportQuery query) {
        ...
        var snapshot = outletPerformanceRepository.findOutletPerformance(query.tenantId(), query.fromDate(), query.toDate(), query.gameCode());

        var lines = snapshot.stream()
            .map(line -> new OutletPerformanceLineDto(
                line.outletId(),
                line.outletCode(),
                line.outletName(),
                line.gameCode(),
