...
public class SharedResultsByGameAggregator {
    ...
    public ResultsByGameBlock buildResultsBlock(UUID tenantId) {
        ...
        var channelCodes = channels.stream()
            .map(DrawChannelSummary::channelCode) // adapte au vrai nom
            .toList();

        // 2) Next draws pour tous ces channels
        var nextDraws = getNextDrawsHandler.handle(
            new GetNextDrawsQuery(tenantId, ZonedDateTime.now(clock), 10)
        );
        ...
        for (var channel : channels) {
            var results = listLastDaysDrawResultsHandler.handle(
