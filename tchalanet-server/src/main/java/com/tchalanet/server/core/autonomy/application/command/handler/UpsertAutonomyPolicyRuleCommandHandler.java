...
public class UpsertAutonomyPolicyRuleCommandHandler
    implements CommandHandler<UpsertAutonomyPolicyRuleCommand, AutonomyPolicyRule> {
  ...
  public AutonomyPolicyRule handle(UpsertAutonomyPolicyRuleCommand command) {
    Optional<AutonomyPolicyRule> existing =
        repository.findByTarget(command.tenantId(), command.targetType(), command.targetId());

    if (existing.isPresent()) {
      var old = existing.get();
      var updated =
          new AutonomyPolicyRule(
              old.id(),
              command.tenantId(),
              command.targetType(),
              command.targetId(),
