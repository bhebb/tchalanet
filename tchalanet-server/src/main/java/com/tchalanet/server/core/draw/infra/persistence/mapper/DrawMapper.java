...
/**
 * Mapper between persistence entities and domain Draw objects. Uses the dedicated DrawChannelMapper
 * and DrawResultMapper to map nested objects.
 */
@Mapper(
    componentModel = "spring",
    unmappedTargetPolicy = ReportingPolicy.IGNORE,
    uses = {DrawChannelMapper.class, DrawResultMapper.class})
public abstract class DrawMapper {

  @Autowired protected DrawChannelMapper drawChannelMapper;

  @Autowired protected DrawResultReaderPort drawResultReaderPort;
  ...
  public Draw toDomain(DrawJpaEntity entity) {
    ...
    var source =
        entity.getDrawSource() == null
            ? DrawSource.SYSTEM
            : DrawSource.valueOf(entity.getDrawSource());

    // Try to load result via the reader port (explicitly call repository/port to avoid missing
    // join)
    ...
    if (drawResultReaderPort != null) {
      var maybe =
          drawResultReaderPort.findByDrawId(
              TenantId.of(entity.getTenantId()), new DrawId(entity.getId()));
