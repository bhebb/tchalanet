  ...
  public Optional<SessionWithTotalsDto> handle(GetSessionWithTotalsQuery query) {
    ...
    var dto =
        new SessionWithTotalsDto(
            ...
            session.openedAt(),
            session.closedAt(),
            session.openingFloatCents() != null
                ? BigDecimal.valueOf(session.openingFloatCents()).movePointLeft(2)
                : null,
            session.closingAmountCents() != null
                ? BigDecimal.valueOf(session.closingAmountCents()).movePointLeft(2)
                : null,
            totalsDto);
