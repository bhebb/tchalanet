...
public class TicketWebMapper {
    ...
    public TicketSummaryResponse toSummaryResponse(Ticket ticket) {
        return new TicketSummaryResponse(
            ...
            ticket.getTotalAmount(),
            ticket.getCreatedAt(),
            "Terminal " + (ticket.getTerminalId() != null ? ticket.getTerminalId().uuid().toString().substring(0, 4) : "-"),
            "Draw " + (ticket.getDrawId() != null ? ticket.getDrawId().uuid().toString().substring(0, 4) : "-"));
    }
    ...
    public ListTicketsQuery toListTicketsQuery(TerminalId terminalId, DrawId drawId, String status, Instant from, Instant to, int page, int size) {
        ...
        return new ListTicketsQuery(
            new ListTicketsQuery.TicketFilter(
                null, // tenantId will be injected by aspect or controller if needed, or pass it as param
                terminalId,
                drawId,
                ticketStatus,
                from,
    ...
    public MarkPaymentPendingCommand toMarkPaymentPendingCommand(TicketId ticketId, MarkPaymentPendingRequest request) {
        return new MarkPaymentPendingCommand(
            ticketId,
            request.reason(),
            request.performedBy()
        );
    ...
    public MarkTicketPaidCommand toMarkTicketPaidCommand(TicketId ticketId, MarkPaidRequest request) {
        return new MarkTicketPaidCommand(
            ticketId,
            request.reason(),
            request.performedBy()
        );
    ...
    public SellTicketResponse toSellTicketResponse(SellTicketResult result) {
        TicketResponse ticketResponse = result.ticket() != null ? toTicketResponse(result.ticket()) : null;
        return new SellTicketResponse(ticketResponse, result.status(), java.util.Collections.emptyList(), result.approvalRequestId());
    }
    ...
    public CancelSaleResponse toCancelSaleResponse(CancelSaleResult result) {
        ...
        List<com.tchalanet.server.core.sales.infra.web.model.LimitNotice> warnings = result.warnings().stream()
            .map(this::mapLimitNotice)
            .collect(Collectors.toList());
        return new CancelSaleResponse(ticketResponse, result.status(), warnings);
    }

    private com.tchalanet.server.core.sales.infra.web.model.LimitNotice mapLimitNotice(com.tchalanet.server.core.sales.application.command.model.LimitNotice notice) {
