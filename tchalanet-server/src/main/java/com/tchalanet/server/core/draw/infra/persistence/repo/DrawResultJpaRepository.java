...
public interface DrawResultJpaRepository extends JpaRepository<DrawResultJpaEntity, UUID> {
  ...
  @Query(
      value =
          "INSERT INTO draw_result (id, tenant_id, draw_id, source, status, numbers_main, numbers_extra, raw_payload, occurred_at, created_at, updated_at) VALUES (:id, :tenantId, :drawId, :source, :status, cast(:numbersMain as jsonb), cast(:numbersExtra as jsonb), cast(:rawPayload as jsonb), :occurredAt, now(), now()) ON CONFLICT (tenant_id, draw_id) DO UPDATE SET source = EXCLUDED.source, status = EXCLUDED.status, numbers_main = EXCLUDED.numbers_main, numbers_extra = EXCLUDED.numbers_extra, raw_payload = EXCLUDED.raw_payload, occurred_at = EXCLUDED.occurred_at, updated_at = now()",
      nativeQuery = true)
  int upsertResult(
      @Param("id") UUID id,
