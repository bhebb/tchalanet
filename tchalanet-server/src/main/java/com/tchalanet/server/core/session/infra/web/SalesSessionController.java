...
public class SalesSessionController {
  ...
  public ApiResponse<SalesSessionResponse> open(
    ...
    return ApiResponse.<SalesSessionResponse>created(SalesSessionResponse.fromDomain(session));
  }

  @Operation(summary = "Close a POS session (tenant)")
  @PostMapping("/{sessionId}/close")
  public ResponseEntity<SalesSessionResponse> close(
    ...
    return ResponseEntity.ok(SalesSessionResponse.fromDomain(session));
  }

  @Operation(summary = "Get current session for a terminal (tenant)")
  @GetMapping("/current")
  public ResponseEntity<SalesSessionResponse> current(@RequestParam TerminalId terminalId) {
    var ctx = contextResolver.currentOrNull();
    var tenantId = TenantId.of(ctx.tenantUuid());
    ...
    return result
        .map(s -> ResponseEntity.ok(SalesSessionResponse.fromDomain(s)))
