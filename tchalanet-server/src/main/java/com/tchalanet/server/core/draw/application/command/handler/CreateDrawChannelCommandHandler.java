...
public class CreateDrawChannelCommandHandler
    ...
    public DrawChannel handle(CreateDrawChannelCommand command) {
        ...
        List<DayOfWeek> dow = command.daysOfWeek() == null ? List.of() :
            command.daysOfWeek().stream().filter(Objects::nonNull).map(s -> DayOfWeek.valueOf(s.toUpperCase(Locale.ROOT))).toList();

        Integer sortOrder = command.sortOrder() == null ? 0 : command.sortOrder();

        DrawSource defaultSource;
        try {
            defaultSource = command.defaultSource() == null ? DrawSource.MANUAL : DrawSource.valueOf(command.defaultSource()) ;
        ...
        DrawChannel channel = new DrawChannel(
            id,
            command.name(),
            label,
            tenantId,
