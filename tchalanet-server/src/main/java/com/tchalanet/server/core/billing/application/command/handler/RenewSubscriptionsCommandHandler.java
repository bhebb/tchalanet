...
@Slf4j
public class RenewSubscriptionsCommandHandler implements VoidCommandHandler<RenewSubscriptionsCommand> {
    ...
    public void handle(RenewSubscriptionsCommand command) {
        ...
        for (Subscription sub : due) {
            try {
                ...
                log.info("Renewed subscription tenantId={} subscriptionId={} newEnd={}",
                    saved.tenantId(), saved.id(), newEnd);

                billingProvider.changePlan(new BillingParams(saved.tenantId(), saved.id()), "NOOP_RENEW");
            } catch (Exception e) {
                log.error("Failed to renew subscription tenantId={} subscriptionId={}",
                    sub.tenantId(), sub.id(), e);
            }
        }
    }
}
