...
public class GetLatestPublicDrawResultsQueryHandler
    implements QueryHandler<GetLatestPublicDrawResultsQuery, List<PublicLatestDrawResultsResponse>> {
    ...
    private PublicLatestDrawResultsResponse mapToLatestResponse(List<PublicDrawResultRow> rows) {
        ...
        ZoneId zone = first.getChannelTimezone() != null ? first.getChannelTimezone() : ZoneId.of("UTC");
        ...
        if (drawTime != null) {
            ...
            if (first.getDrawDate() != null && first.getDrawDate().isBefore(today.minusDays(1))) {
                return buildResponse(first, results, nextScheduledAt, nextDrawLabel, nextIsOpen, nextIsClosingSoon);
            }
    ...
    private PublicLatestDrawResultsResponse buildResponse(
        ...
        return new PublicLatestDrawResultsResponse(
            ...
            first.getChannelDrawTime() == null ? null : first.getChannelDrawTime().format(TIME_FORMATTER),
            results,
            nextScheduledAt,
            nextDrawLabel,
            nextIsOpen,
