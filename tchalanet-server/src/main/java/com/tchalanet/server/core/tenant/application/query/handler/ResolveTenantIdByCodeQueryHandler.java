...
public class ResolveTenantIdByCodeQueryHandler
    implements QueryHandler<ResolveTenantIdByCodeQuery, Optional<UUID>> {
  ...
  public Optional<UUID> handle(ResolveTenantIdByCodeQuery tenantCodeQuery) {
    ...
    var tenant =
        repo.findByCode(codeLower)
            .orElseThrow(
                () -> new IllegalArgumentException("Tenant not found for code=" + codeLower));
