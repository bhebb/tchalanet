...
/**
 * Small cache helper for Tenant lookups. Follows the project guidelines:
 * - constructor injection
 * - avoid returning nulls (use Optional)
 * - keep a UUID-based compatibility surface
 */
@Component
public class TenantCache {
    ...
    public void putTenantIdByCode(String codeLower, TenantId tenantId) {
        ...
        Cache c = cacheManager.getCache(CACHE_TENANT_BY_CODE);
        if (c == null) {
            log.debug("Cache '{}' not configured, skipping put for key={}", CACHE_TENANT_BY_CODE, codeLower);
            return;
        }
