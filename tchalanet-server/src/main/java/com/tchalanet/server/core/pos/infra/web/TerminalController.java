...
public class TerminalController {
    ...
    @PostMapping
    public UUID registerDevice(@PathVariable UUID tenantId, @RequestBody RegisterPosDeviceCommand command) {
    ...
    public void sendHeartbeat(@PathVariable UUID tenantId, @PathVariable UUID id, @RequestBody SendPosHeartbeatCommand command) {
    ...
    @PostMapping("/{id}/lock")
    public Terminal lockDevice(@PathVariable UUID tenantId, @PathVariable UUID id, @RequestBody LockRequest request) {
        var cmd = new LockTerminalCommand(tenantId, id, request.actorId(), request.reason());
        return commandBus.send(cmd);
    }

    @PostMapping("/{id}/unlock")
    public Terminal unlockDevice(@PathVariable UUID tenantId, @PathVariable UUID id, @RequestBody UnlockRequest request) {
        var cmd = new UnlockTerminalCommand(tenantId, id, request.actorId());
        return commandBus.send(cmd);
    }

    @PutMapping("/{id}/metadata")
    public Terminal updateMetadata(@PathVariable UUID tenantId, @PathVariable UUID id, @RequestBody UpdateMetadataRequest request) {
        var cmd = new UpdateTerminalMetadataCommand(tenantId, id, request.actorId(), request.metadataPatch(), request.heartbeatAlso());
        return commandBus.send(cmd);
    }

    @DeleteMapping("/{id}")
    public Terminal unregisterDevice(@PathVariable UUID tenantId, @PathVariable UUID id, @RequestBody UnregisterRequest request) {
        var cmd = new UnregisterTerminalCommand(tenantId, id, request.actorId(), request.reason());
        return commandBus.send(cmd);
    }

    @GetMapping("/{id}")
    public Optional<Terminal> getDevice(@PathVariable UUID tenantId, @PathVariable UUID id) {
        var query = new GetPosDeviceByIdQuery(tenantId, id);
        return queryBus.send(query);
    }

    @GetMapping("/{id}/status")
    public Map<String, Object> getDeviceStatus(@PathVariable UUID tenantId, @PathVariable UUID id) {
        var query = new GetPosDeviceStatusQuery(tenantId, id);
        return queryBus.send(query);
    }

    @GetMapping("/outlets/{outletId}")
    public List<Terminal> listDevicesByOutlet(@PathVariable UUID tenantId, @PathVariable UUID outletId) {
