...
public class UpdateTerminalMetadataCommandHandler
    implements VoidCommandHandler<UpdateTerminalMetadataCommand> {
  ...
  @Override
  @TchTx
  public void handle(UpdateTerminalMetadataCommand cmd) {
    Terminal t = reader.getRequired(cmd.tenantId(), cmd.terminalId());
    Terminal updated = t.mergeMetadata(cmd.metadataPatch());
    if (updated == t) return; // idempotent
    writer.save(updated);

    TerminalMetadataUpdatedEvent event =
        new TerminalMetadataUpdatedEvent(
