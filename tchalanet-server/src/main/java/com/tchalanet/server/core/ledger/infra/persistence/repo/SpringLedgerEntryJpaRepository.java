...
public interface SpringLedgerEntryJpaRepository extends JpaRepository<LedgerEntryJpaEntity, UUID> {
  List<LedgerEntryJpaEntity> findByTenantIdAndOccurredAtBetweenOrderByOccurredAtDesc(
      UUID tenantId, Instant from, Instant to);

  List<LedgerEntryJpaEntity> findByTenantIdAndRefTypeAndRefId(
      UUID tenantId, LedgerRefType refType, UUID refId);

  @Query(
      "SELECT COALESCE(SUM(CASE WHEN e.direction = 'CREDIT' THEN e.amount ELSE -e.amount END), 0) FROM LedgerEntryJpaEntity e WHERE e.tenantId = :tenantId")
