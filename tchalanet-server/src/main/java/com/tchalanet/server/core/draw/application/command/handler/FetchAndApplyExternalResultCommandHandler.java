 ...
 * et les enregistrer.
 *
 * <p>implémenter la logique métier complète.
 */
@UseCase
  ...
  public void handle(FetchAndApplyExternalResultCommand command) {
    ...
    var query =
        new ExternalDrawResultPort.DrawExternalQuery(
            ...
            draw.scheduledAt().toLocalDate(),
            draw.cutoffAt().toInstant(),
            command.force());
    ...
    if (applied) {
      var resultPayloadJson =
          String.format(
              "{\"numbers_main\":%s,\"numbers_extra\":%s,\"source\":\"%s\",\"occurred_at\":\"%s\",\"override\":false}",
              result.numbersMain(), result.numbersExtra(), result.source(), result.occurredAt());
      var event =
