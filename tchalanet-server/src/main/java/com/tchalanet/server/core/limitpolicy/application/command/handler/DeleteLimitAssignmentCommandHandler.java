...
public class DeleteLimitAssignmentCommandHandler
    implements CommandHandler<DeleteLimitAssignmentCommand, Void> {
  ...
  public Void handle(DeleteLimitAssignmentCommand cmd) {
    var assignment =
        reader
            .findById(cmd.tenantId(), cmd.assignmentId())
            .orElseThrow(() -> new IllegalStateException("Assignment not found"));
    if (!assignment.targetType().equals(cmd.targetType())
        || !assignment.targetId().equals(cmd.targetId())) {
