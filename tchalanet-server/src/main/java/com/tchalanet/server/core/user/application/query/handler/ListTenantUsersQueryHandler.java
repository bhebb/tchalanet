...
public class ListTenantUsersQueryHandler implements QueryHandler<PagedListTenantUsersQuery, TchPage<UserRow>> {

  private final UserReaderPort userReaderPort;

  @Override
  public TchPage<UserRow> handle(PagedListTenantUsersQuery query) {
    Pageable pageable = query.pageRequest().pageable();
    Page<AppUser> page = userReaderPort.findByTenantId(query.tenantId(), pageable);
