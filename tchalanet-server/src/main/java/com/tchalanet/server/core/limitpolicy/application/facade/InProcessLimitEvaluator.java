...
public class InProcessLimitEvaluator implements LimitEvaluator {
  ...
  public LimitEvaluationResult evaluate(OperationType operationType, LimitContext context) {
    ...
    BreachOutcome overall = details.isEmpty() ? BreachOutcome.ALLOW : BreachOutcome.BLOCK; // simplified
    return new LimitEvaluationResult(overall, details);
  }

  private List<LimitBreachDetail> evaluateRule(RuleKey ruleKey, LimitDefinition def, LimitContext context) {
    return switch (ruleKey) {
      ...
      case MAX_STAKE_PER_SELECTION_PER_TICKET -> evaluateMaxStakePerSelectionPerTicket(def, context);
      case MAX_SALES_COUNT_PER_SELECTION_PER_DRAW -> evaluateMaxSalesCountPerSelectionPerDraw(def, context);
      case MAX_EXPOSURE_PER_SELECTION_PER_DRAW -> evaluateMaxExposurePerSelectionPerDraw(def, context);
      case MAX_TOTAL_STAKE_PER_DRAW -> evaluateMaxTotalStakePerDraw(def, context);
      case MAX_POTENTIAL_PAYOUT_EXPOSURE_PER_SELECTION_PER_DRAW -> evaluateMaxPotentialPayoutExposurePerSelectionPerDraw(def, context);
  ...
  private List<LimitBreachDetail> evaluateMaxStakePerLine(LimitDefinition def, LimitContext context) {
    BigDecimal max = (BigDecimal) def.params().get("max");
    List<LimitBreachDetail> details = new ArrayList<>();
    for (var line : context.lines()) {
      if (line.stake().compareTo(max) > 0) {
        details.add(new LimitBreachDetail(
            def.ruleKey().name(),
            BreachOutcome.BLOCK,
            "Stake per line exceeds maximum allowed",
            TargetType.TENANT, // TODO: determine from limit assignment
            line.selection(),
            line.stake(),
            max
        ));
  ...
  private List<LimitBreachDetail> evaluateMinStakePerLine(LimitDefinition def, LimitContext context) {
    // Implement
    return List.of();
  }

  private List<LimitBreachDetail> evaluateMaxLinesPerTicket(LimitDefinition def, LimitContext context) {
    int max = (Integer) def.params().get("max_count");
    if (context.linesCount() > max) {
      return List.of(new LimitBreachDetail(
          def.ruleKey().name(),
          BreachOutcome.BLOCK,
  ...
  private List<LimitBreachDetail> evaluateMaxStakePerTicket(LimitDefinition def, LimitContext context) {
    BigDecimal max = (BigDecimal) def.params().get("max");
  ...
  private List<LimitBreachDetail> evaluateMaxStakePerSelectionPerTicket(LimitDefinition def, LimitContext context) {
    // Implement aggregation per selection
    return List.of();
  }

  private List<LimitBreachDetail> evaluateMaxSalesCountPerSelectionPerDraw(LimitDefinition def, LimitContext context) {
    int max = (Integer) def.params().get("max_count");
    for (var line : context.lines()) {
      var exposure = factsProvider.getSelectionExposure(
          context.tenantId(), context.drawId(), ScopeType.OUTLET, context.outletId(), line.betType(), line.selectionKey());
  ...
  private List<LimitBreachDetail> evaluateMaxExposurePerSelectionPerDraw(LimitDefinition def, LimitContext context) {
    BigDecimal max = (BigDecimal) def.params().get("max");
    for (var line : context.lines()) {
      var exposure = factsProvider.getSelectionExposure(
          context.tenantId(), context.drawId(), ScopeType.OUTLET, context.outletId(), line.betType(), line.selectionKey());
  ...
  private List<LimitBreachDetail> evaluateMaxTotalStakePerDraw(LimitDefinition def, LimitContext context) {
    BigDecimal max = (BigDecimal) def.params().get("max");
    BigDecimal current = factsProvider.getDrawTotalStake(context.tenantId(), context.drawId(), ScopeType.OUTLET, context.outletId());
  ...
  private List<LimitBreachDetail> evaluateMaxPotentialPayoutExposurePerSelectionPerDraw(LimitDefinition def, LimitContext context) {
    BigDecimal max = (BigDecimal) def.params().get("max");
    for (var line : context.lines()) {
      var exposure = factsProvider.getSelectionExposure(
          context.tenantId(), context.drawId(), ScopeType.OUTLET, context.outletId(), line.betType(), line.selectionKey());
      BigDecimal potential = exposure.potentialPayoutTotal().add(line.stake().multiply(line.optionalMultiplier()));
  ...
  private List<LimitBreachDetail> evaluateDailyStakeCap(LimitDefinition def, LimitContext context) {
    ...
    var totals = factsProvider.getDailyTotals(context.tenantId(), day, ScopeType.OUTLET, context.outletId());
  ...
  private List<LimitBreachDetail> evaluateDailyPayoutCap(LimitDefinition def, LimitContext context) {
    // For payout operation
    return List.of();
  }

  private List<LimitBreachDetail> evaluateMaxCancelsPerDay(LimitDefinition def, LimitContext context) {
    // For cancel operation
    return List.of();
  }

  private List<LimitBreachDetail> evaluateMaxPayoutPerLine(LimitDefinition def, LimitContext context) {
    // For payout operation
    return List.of();
  }

  private List<LimitBreachDetail> evaluateMaxPayoutPerTicket(LimitDefinition def, LimitContext context) {
    // For payout operation
    return List.of();
  }
}
