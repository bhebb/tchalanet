...
public class InProcessLimitEvaluator implements LimitEvaluator {
    ...
    private List<LimitBreachDetail> evaluateMaxSalesCountPerSelectionPerDraw(LimitDefinition def, LimitContext context) {
        ...
        for (var line : context.lines()) {
            var exposure = factsProvider.getSelectionExposure(
                context.tenantId(), context.drawId(), ScopeType.OUTLET, context.outletId().uuid(), line.betType(), line.selectionKey());
    ...
    private List<LimitBreachDetail> evaluateMaxTotalStakePerDraw(LimitDefinition def, LimitContext context) {
        BigDecimal max = (BigDecimal) def.params().get("max");
        BigDecimal current = factsProvider.getDrawTotalStake(context.tenantId(), context.drawId(), ScopeType.OUTLET, context.outletId().uuid());
    ...
    private List<LimitBreachDetail> evaluateDailyStakeCap(LimitDefinition def, LimitContext context) {
        ...
        var totals = factsProvider.getDailyTotals(context.tenantId(), day, ScopeType.OUTLET, context.outletId().uuid());
