...
public class OutletLimitAssignmentsController {
    ...
    @GetMapping
    public GetLimitAssignmentsResult getAssignments(@PathVariable UUID outletId, @RequestParam UUID tenantId) {
        return queryBus.send(new GetLimitAssignmentsQuery(tenantId, TargetType.OUTLET, outletId));
    }

    @PostMapping
    public Object createAssignment(@PathVariable UUID outletId, @RequestParam UUID tenantId, @RequestBody CreateLimitAssignmentRequest request) {
    ...
    public void deleteAssignment(@PathVariable UUID outletId, @PathVariable UUID assignmentId, @RequestParam UUID tenantId) {
        var cmd = new DeleteLimitAssignmentCommand(tenantId, assignmentId, TargetType.OUTLET, outletId);
        commandBus.send(cmd);
    }
