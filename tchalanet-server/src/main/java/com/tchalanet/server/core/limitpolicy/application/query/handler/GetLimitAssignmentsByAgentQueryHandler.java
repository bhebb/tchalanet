...
public class GetLimitAssignmentsByAgentQueryHandler
    implements QueryHandler<GetLimitAssignmentsByAgentQuery, GetLimitAssignmentsResult> {
  ...
  public GetLimitAssignmentsResult handle(GetLimitAssignmentsByAgentQuery query) {
    var assignments =
        assignmentReader.findActiveByTarget(
            query.tenantId(), TargetType.AGENT.name(), query.agentId().uuid());

    var summaries =
        assignments.stream()
            .map(
                assignment -> {
                  Optional<LimitDefinition> limitOpt =
                      limitDefinitionReader.findById(
                          assignment.limitDefinitionId()); // Change this line
                  if (limitOpt.isEmpty()) {
                    // skip or handle
                    return null;
                  }
