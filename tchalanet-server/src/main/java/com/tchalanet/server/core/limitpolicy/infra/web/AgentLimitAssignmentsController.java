...
public class AgentLimitAssignmentsController {
    ...
    @GetMapping
    public GetLimitAssignmentsResult getAssignments(@PathVariable UUID agentId, @RequestParam UUID tenantId) {
        return queryBus.send(new GetLimitAssignmentsQuery(tenantId, TargetType.AGENT, agentId));
    }

    @PostMapping
    public Object createAssignment(@PathVariable UUID agentId, @RequestParam UUID tenantId, @RequestBody CreateLimitAssignmentRequest request) {
    ...
    public void deleteAssignment(@PathVariable UUID agentId, @PathVariable UUID assignmentId, @RequestParam UUID tenantId) {
        var cmd = new DeleteLimitAssignmentCommand(tenantId, assignmentId, TargetType.AGENT, agentId);
        commandBus.send(cmd);
    }
