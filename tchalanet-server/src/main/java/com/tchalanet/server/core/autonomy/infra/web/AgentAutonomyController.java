...
public class AgentAutonomyController {
    ...
    public GetAutonomyPolicyRuleResult get(@PathVariable AgentId agentId, @RequestParam TenantId tenantId) {
        return queryBus.send(new GetAutonomyPolicyRuleQuery(
            tenantId, AutonomyTargetType.AGENT, agentId.uuid()
        ));
    }

    @PutMapping
    @ResponseStatus(value = HttpStatus.CREATED)
    public AutonomyPolicyRule upsert(@PathVariable AgentId agentId, @RequestParam TenantId tenantId, @RequestBody UpsertAutonomyRequest req) {
