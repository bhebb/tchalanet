...
public class DrawResultJdbcRepository {
  ...
  public List<DrawResultJpaEntity> findByCriteria(
    ...
    var sb =
        new StringBuilder(
            "select id, provider, provider_slot, result_slot_id, occurred_at, source_result, haiti_result, raw_payload, flags, status, quality, source, source_hash, fetched_at, override_reason from draw_result where deleted_at is null");
    ...
    return jdbc.query(
        ...
        (rs, rowNum) -> {
          ...
          e.setProviderSlot(rs.getString("provider_slot"));
          ...
          e.setStatus(
              com.tchalanet.server.core.drawresult.domain.model.DrawResultStatus.valueOf(
                  rs.getString("status")));
          ...
          if (src != null)
            e.setSource(com.tchalanet.server.core.drawresult.domain.model.DrawSource.valueOf(src));
