...
@Slf4j
public class ReconcileDailyLedgerCommandHandler implements VoidCommandHandler<ReconcileDailyLedgerCommand> {
    ...
    public void handle(ReconcileDailyLedgerCommand command) {
        try {
            var entries = ledgerReader.findByTenant(command.tenantId(), command.dayStart(), command.dayEnd(), Integer.MAX_VALUE, 0);
            var totalDebit = entries.stream()
                    .filter(e -> e.direction() == LedgerDirection.DEBIT)
                    .map(LedgerEntry::amount)
                    .reduce(BigDecimal.ZERO, BigDecimal::add);
            var totalCredit = entries.stream()
                    .filter(e -> e.direction() == LedgerDirection.CREDIT)
                    .map(LedgerEntry::amount)
                    .reduce(BigDecimal.ZERO, BigDecimal::add);
            var balance = totalCredit.subtract(totalDebit);
            log.info("Reconciled ledger for tenant {}: {} entries, debit={}, credit={}, balance={}",
                    command.tenantId(), entries.size(), totalDebit, totalCredit, balance);
        } catch (Exception e) {
