        ...
        try {
            NyResultDto[] response =
                nyLotteryWebClient
                    .get()
                    .uri(uriBuilder -> {
                        var b = uriBuilder.queryParam("$limit", 7).queryParam("$order", "draw_date DESC");
                        if (appToken != null && !appToken.isBlank()) b = b.queryParam("app_token", appToken);
                        return b.build();
                    })
                    .retrieve()
            ...
            for (NyResultDto row : response) {
                ...
                // NUMBERS (3 digits)
                addIfValid(results, UsLotteryProvider.NY, "NUMBERS", "MIDDAY", "US_NY_NUM3_MID",
                    date, occurredAtUtc, fetchedAt, splitDigits(row.middayDaily()), 3, ORIGIN);

                addIfValid(results, UsLotteryProvider.NY, "NUMBERS", "EVENING", "US_NY_NUM3_EVE",
                    date, occurredAtUtc, fetchedAt, splitDigits(row.eveningDaily()), 3, ORIGIN);

                // WIN4 (4 digits)
                addIfValid(results, UsLotteryProvider.NY, "WIN4", "MIDDAY", "US_NY_NUM4_MID",
                    date, occurredAtUtc, fetchedAt, splitDigits(row.middayWin4()), 4, ORIGIN);

                addIfValid(results, UsLotteryProvider.NY, "WIN4", "EVENING", "US_NY_NUM4_EVE",
                    date, occurredAtUtc, fetchedAt, splitDigits(row.eveningWin4()), 4, ORIGIN);
            }
