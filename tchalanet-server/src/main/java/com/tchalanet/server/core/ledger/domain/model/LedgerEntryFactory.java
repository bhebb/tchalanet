...
public class LedgerEntryFactory {
    ...
    public static LedgerEntry createReversal(LedgerEntry original, Instant occurredAt) {
        var inverseDirection = original.direction() == LedgerDirection.DEBIT ? LedgerDirection.CREDIT : LedgerDirection.DEBIT;
        return LedgerEntry.create(original.tenantId(), LedgerRefType.REVERSAL, original.refId(), original.amount(), inverseDirection, occurredAt);
    }
