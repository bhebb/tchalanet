...
@Slf4j
public class RecordTicketResultCommandHandler
    implements CommandHandler<RecordTicketResultCommand, Ticket> {
  ...
  public Ticket handle(RecordTicketResultCommand command) {
    ...
    var event =
        new TicketResultedEvent(
            UUID.randomUUID(), now, saved.getTenantId(), saved.getId(), command.winningAmount());
