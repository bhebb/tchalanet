...
public class AppSettingRepositoryAdapter implements AppSettingReaderPort {
  ...
  public List<AppSettingEntity> findForTenant(TenantId tenantId, List<String> namespaces) {
    if (namespaces == null || namespaces.isEmpty()) return List.of();
    return repo.findByActiveTrueAndDeletedAtIsNullAndLevelAndTenantIdAndNamespaceIn(AppSettingLevel.TENANT, tenantId.uuid(), namespaces);
  }

  @Override
  public List<AppSettingEntity> findForOutlet(TenantId tenantId, OutletId outletId, List<String> namespaces) {
    if (namespaces == null || namespaces.isEmpty()) return List.of();
    return repo.findByActiveTrueAndDeletedAtIsNullAndLevelAndTenantIdAndOutletIdAndNamespaceIn(AppSettingLevel.OUTLET, tenantId.uuid(), outletId.uuid(), namespaces);
  }

  @Override
  public List<AppSettingEntity> findForTerminal(TenantId tenantId, TerminalId terminalId, List<String> namespaces) {
    if (namespaces == null || namespaces.isEmpty()) return List.of();
    return repo.findByActiveTrueAndDeletedAtIsNullAndLevelAndTenantIdAndTerminalIdAndNamespaceIn(AppSettingLevel.TERMINAL, tenantId.uuid(), terminalId.uuid(), namespaces);
  }
}
