...
public class AppSettingValidator {
    ...
    public static void validateOrThrow(String namespace, String key, AppSettingValueType type, String value) {
        ...
        if (reg.type() != type) {
            throw new IllegalArgumentException("Wrong value_type for " + full + ": expected " + reg.type() + ", got " + type);
        }
        // parse check
        parse(type, value);
    }
