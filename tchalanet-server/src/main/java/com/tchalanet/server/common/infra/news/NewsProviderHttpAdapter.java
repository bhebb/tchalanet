package com.tchalanet.server.common.infra.news;
...
@Slf4j
public class NewsProviderHttpAdapter
    implements com.tchalanet.server.common.usecase.NewsProviderPort {
  ...
  // private static final int MAX_ITEMS = 10; replaced by maxItemsValue
  ...
  public List<Map<String, Object>> fetchLatest() {
    ...
    while (++attempt <= Math.max(1, maxRetries)) {
      try {
        ...
        if (tree.isArray()) {
          return objectMapper.convertValue(tree, new TypeReference<List<Map<String, Object>>>() {});
        }
        ...
        for (String candidate : new String[] {"items", "data", "results", "articles", "news"}) {
          ...
          if (node != null && node.isArray()) {
            return objectMapper.convertValue(
                node, new TypeReference<List<Map<String, Object>>>() {});
          }
        }

        if (tree.isObject()) {
          Map<String, Object> single =
              objectMapper.convertValue(tree, new TypeReference<Map<String, Object>>() {});
  ...
  private List<Map<String, Object>> parseXmlFeed(String xml, int maxItems) {
