...
public class DrawResultCatalogBack {

  private final DrawResultReaderPort reader;

  @Cacheable(
      cacheNames = "drawresult.id.by_slot_occurred",
      key = "#resultSlotId.value().toString() + '|' + #occurredAt.getEpochSecond()")
  ...
  @Cacheable(cacheNames = "drawresult.id.by_id", key = "#id.value().toString()")
