...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM app_user WHERE keycloak_sub = '00000000-0000-0000-0000-000000010001'::uuid AND deleted_at IS NULL) THEN
    INSERT INTO app_user (id, keycloak_sub, username, email, display_name, locale, status, created_at, updated_at)
    VALUES (
      '00000000-0000-0000-0000-000000010001'::uuid,
      '00000000-0000-0000-0000-000000010001'::uuid,
      'super_admin', 'super_admin@local', 'Super Admin', 'fr', 'ACTIVE', now(), now()
    );
  ELSE
    UPDATE app_user
    SET username = 'super_admin', email = 'super_admin@local', display_name = 'Super Admin', locale = 'fr', status = 'ACTIVE', updated_at = now()
  ...
  IF u_id IS NOT NULL AND r_id IS NOT NULL AND t_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM tenant_user WHERE tenant_id = t_id AND user_id = u_id AND role_id = r_id AND deleted_at IS NULL) THEN
      INSERT INTO tenant_user (id, tenant_id, user_id, role_id, autonomy_level, is_owner, created_at, updated_at)
      VALUES (gen_random_uuid(), t_id, u_id, r_id, 'full', true, now(), now());
    END IF;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM app_user WHERE keycloak_sub = '00000000-0000-0000-0000-000000010002'::uuid AND deleted_at IS NULL) THEN
    ...
    VALUES (
      ...
      '00000000-0000-0000-0000-000000010002'::uuid,
      'admin', 'admin@local', 'Admin', 'fr', 'ACTIVE', now(), now()
    );
  ELSE
    UPDATE app_user
    SET username = 'admin', email = 'admin@local', display_name = 'Admin', locale = 'fr', status = 'ACTIVE', updated_at = now()
  ...
  IF u_id IS NOT NULL AND r_id IS NOT NULL AND t_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM tenant_user WHERE tenant_id = t_id AND user_id = u_id AND role_id = r_id AND deleted_at IS NULL) THEN
      INSERT INTO tenant_user (id, tenant_id, user_id, role_id, autonomy_level, is_owner, created_at, updated_at)
      VALUES (gen_random_uuid(), t_id, u_id, r_id, 'full', false, now(), now());
    END IF;
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM app_user WHERE keycloak_sub = '00000000-0000-0000-0000-000000010003'::uuid AND deleted_at IS NULL) THEN
    ...
    VALUES (
      ...
      '00000000-0000-0000-0000-000000010003'::uuid,
      'agent', 'agent@local', 'Agent', 'fr', 'ACTIVE', now(), now()
    );
  ELSE
    UPDATE app_user
    SET username = 'agent', email = 'agent@local', display_name = 'Agent', locale = 'fr', status = 'ACTIVE', updated_at = now()
  ...
  IF u_id IS NOT NULL AND r_id IS NOT NULL AND t_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM tenant_user WHERE tenant_id = t_id AND user_id = u_id AND role_id = r_id AND deleted_at IS NULL) THEN
      INSERT INTO tenant_user (id, tenant_id, user_id, role_id, autonomy_level, is_owner, created_at, updated_at)
      VALUES (gen_random_uuid(), t_id, u_id, r_id, 'none', false, now(), now());
    END IF;
  END IF;
