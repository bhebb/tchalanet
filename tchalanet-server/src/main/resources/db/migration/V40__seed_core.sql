...
-- Roles
-- Insert roles if not already present (avoid ON CONFLICT with partial unique indexes)
INSERT INTO app_role (code, name, description, tenant_id, is_system)
SELECT 'SUPER_ADMIN', 'Super administrateur plateforme', 'Accès complet à la plateforme', NULL, true
WHERE NOT EXISTS (
  SELECT 1 FROM app_role WHERE code = 'SUPER_ADMIN' AND tenant_id IS NULL AND deleted_at IS NULL
);

INSERT INTO app_role (code, name, description, tenant_id, is_system)
SELECT 'TENANT_ADMIN', 'Admin opérateur', 'Administrateur d’un tenant / opérateur', NULL, true
WHERE NOT EXISTS (
  SELECT 1 FROM app_role WHERE code = 'TENANT_ADMIN' AND tenant_id IS NULL AND deleted_at IS NULL
);

INSERT INTO app_role (code, name, description, tenant_id, is_system)
SELECT 'OPERATOR', 'Opérateur / chef de PDV', 'Gère un point de vente', NULL, true
WHERE NOT EXISTS (
  SELECT 1 FROM app_role WHERE code = 'OPERATOR' AND tenant_id IS NULL AND deleted_at IS NULL
);

INSERT INTO app_role (code, name, description, tenant_id, is_system)
SELECT 'CASHIER', 'Caissier', 'Vente de tickets', NULL, true
WHERE NOT EXISTS (
  SELECT 1 FROM app_role WHERE code = 'CASHIER' AND tenant_id IS NULL AND deleted_at IS NULL
);
