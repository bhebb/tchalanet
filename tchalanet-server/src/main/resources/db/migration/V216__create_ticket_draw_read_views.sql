...
-- These views are for SELECT only — never for writes, settlements, or domain transitions.
...
CREATE
OR REPLACE VIEW v_ticket_summary
...
FROM ticket t
         JOIN terminal tm ON tm.id = t.terminal_id
         JOIN outlet o ON o.id = tm.outlet_id
...
CREATE
OR REPLACE VIEW v_ticket_print
  WITH (security_invoker = true)
AS
SELECT t.id         AS ticket_id,
       t.tenant_id,
...
FROM ticket t
         ...
         LEFT JOIN sales_session ss ON ss.id = t.session_id
         JOIN draw d ON d.id = t.draw_id
         JOIN draw_channel dc ON dc.id = d.draw_channel_id
WHERE t.deleted_at IS NULL;
...
CREATE
OR REPLACE VIEW v_draw_summary
  WITH (security_invoker = true)
AS
