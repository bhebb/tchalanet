 ...
 *
 * <ol>
 *   <li>{@code LocalDateTime} MUST NOT be used in domain or application layers.
 *   <li>{@code ZoneId.systemDefault()} MUST NOT be called in business modules.
 *   <li>{@code Instant.now()} (no-arg) and {@code LocalDate.now()} (no-arg) MUST NOT be called in
 *       core domain or application layers — use {@code TchTimeProvider} or pass an explicit {@code
 *       Clock}.
 * </ol>
 *
 * <p>Scope is intentionally limited to actively maintained core domains (draw, sales). Known
 * pre-existing violations in other domains are tracked in separate tasks.
 */
@DisplayName("Timezone enforcement (ArchUnit)")
class TimezoneEnforcementArchTest {

  /**
   * Core domains that have been audited and must comply with timezone rules. Expand this list as
   * each domain is reviewed.
   */
  private static final String[] COMPLIANT_CORE_PACKAGES = {
    "com.tchalanet.server.core.draw..", "com.tchalanet.server.core.sales..",
  };
  ...
  class NoLocalDateTime {
    ...
    void coreDomainAndApplicationMustNotUseLocalDateTime() {
      ArchRule rule =
          noClasses()
              .that()
              .resideInAnyPackage(COMPLIANT_CORE_PACKAGES)
              .and()
    ...
    void coreBusinessMustNotUseSystemDefaultZone() {
      ...
      ArchRule rule =
          noClasses()
              ...
              .resideOutsideOfPackage("..infra..")
              .should()
              .callMethod(ZoneId.class, "systemDefault")
              .because(
