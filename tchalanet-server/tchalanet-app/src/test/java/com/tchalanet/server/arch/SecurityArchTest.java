 ...
 *
 * <p>Tout {@link RestController} dont le {@code @RequestMapping} commence par l'un des préfixes
 * protégés DOIT porter {@code @PreAuthorize} ou {@code @Secured} au niveau classe OU sur chaque
 * méthode handler publique.
 *
 * <p>Préfixes protégés couverts :
 *
 * <ul>
 *   <li>{@code /admin/} — admin endpoints
 *   <li>{@code /platform/} — ops / platform endpoints
 *   <li>{@code /_sdr/} — Spring Data REST endpoints
 *   <li>{@code /tenant/tickets/} — POS ticket endpoints (cancel, print, sell, etc.)
 * </ul>
 *
...
class SecurityArchTest {
  ...
  /**
   * Scopes whose controllers MUST declare authorization on every handler. Add new prefixes here
   * when new protected paths are introduced.
   */
  ...
  /**
   * Regression: a {@code void} handler in a protected scope without authorization MUST be flagged.
   * Runs the same rule against a test-only fixture package (outside {@code com.tchalanet.server},
   * so the real scan above never sees it).
   */
  @Test
  ...
  private static class InProtectedScopeCondition
    ...
    InProtectedScopeCondition() {
      super(
          "mapped under a protected scope prefix (/admin/, /platform/, /_sdr/, /tenant/tickets/)");
    }

    @Override
    public boolean test(JavaClass javaClass) {
      return javaClass.isAnnotatedWith(RequestMapping.class)
          && Arrays.stream(javaClass.getAnnotationOfType(RequestMapping.class).value())
              .anyMatch(
                  path ->
  ...
  /**
   * Condition : {@code @PreAuthorize} or {@code @Secured} present at class level OR on every public
   * handler method.
   */
  private static class HaveAuthorizationAnnotationCondition extends ArchCondition<JavaClass> {
    ...
    public void check(JavaClass javaClass, ConditionEvents events) {
      boolean classLevelAnnotated =
          javaClass.isAnnotatedWith(PreAuthorize.class) || javaClass.isAnnotatedWith(Secured.class);
      ...
      List<JavaMethod> unsecured =
          handlerMethods.stream()
              .filter(
                  m -> !m.isAnnotatedWith(PreAuthorize.class) && !m.isAnnotatedWith(Secured.class))
              .toList();
      ...
      if (!unsecured.isEmpty()) {
        String detail =
            "Controller ["
                ...
                + unsecured.stream().map(JavaMethod::getName).toList();
        events.add(SimpleConditionEvent.violated(javaClass, detail));
      }
    }

    private static boolean isHandlerMethod(JavaMethod method) {
      return method.isAnnotatedWith(org.springframework.web.bind.annotation.GetMapping.class)
          ...
          || method.isAnnotatedWith(org.springframework.web.bind.annotation.RequestMapping.class);
    }
  }
}
