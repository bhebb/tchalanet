...
import com.tchalanet.server.common.context.operational.OperationalRequestContext;
...
class OperationalContextArchitectureTest {
  ...
  void platformUserContextPackageDoesNotExist() {
    assertThat(classes)
        .noneMatch(
            javaClass ->
                javaClass.getPackageName().startsWith("com.tchalanet.server.platform.usercontext"));
  }
  ...
  void flatOperationalContextBridgeUsageIsConstrainedDuringMigration() {
    noClasses()
        .that()
        .resideOutsideOfPackages("com.tchalanet.server.common.context..")
        .should()
        .dependOnClassesThat()
