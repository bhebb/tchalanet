DROP VIEW IF EXISTS sales_ticket_print_header_v;

CREATE VIEW sales_ticket_print_header_v
...
ALTER TABLE sales_ticket
  DROP COLUMN IF EXISTS potential_payout_amount;

ALTER TABLE sales_ticket_line
  DROP CONSTRAINT IF EXISTS chk_ticket_line__amounts;

ALTER TABLE sales_ticket_line
  DROP COLUMN IF EXISTS potential_payout_amount;

ALTER TABLE sales_ticket_line
  DROP COLUMN IF EXISTS potential_gain_mode,
  DROP COLUMN IF EXISTS min_potential_gain,
  DROP COLUMN IF EXISTS max_potential_gain,
  DROP COLUMN IF EXISTS total_potential_gain;

ALTER TABLE sales_ticket_line
  ADD CONSTRAINT chk_ticket_line__amounts CHECK (
    stake_amount >= 0
    AND payout_amount >= 0
  );
