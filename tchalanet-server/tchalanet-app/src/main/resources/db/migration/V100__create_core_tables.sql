...
CREATE TABLE notification (
  ...
  action_route varchar(240),
  action_label_key varchar(160),
  ...
  deleted_by uuid,
  version bigint NOT NULL DEFAULT 0,
  CONSTRAINT chk_notification__scope CHECK (scope IN ('TENANT','PLATFORM','GLOBAL')),
  CONSTRAINT chk_notification__scope_tenant CHECK (
    (scope = 'TENANT' AND tenant_id IS NOT NULL)
    OR (scope IN ('PLATFORM','GLOBAL') AND tenant_id IS NULL)
  ),
  CONSTRAINT chk_notification__audience_type CHECK (audience_type IN ('SPECIFIC_ACTORS','PLATFORM_ADMINS','ALL_APP_USERS','TENANT_ADMINS','TENANT_APP_USERS','TENANT_SELLER_TERMINALS')),
  CONSTRAINT chk_notification__severity CHECK (severity IN ('INFO','WARNING','ERROR','CRITICAL')),
  CONSTRAINT chk_notification__status CHECK (status IN ('DRAFT','PUBLISHED','EXPIRED','CANCELLED','PURGED')),
  CONSTRAINT chk_notification__source_type CHECK (source_type IN ('SYSTEM','ADMIN','SUPER_ADMIN','OPS','SUPPORT','DOMAIN_EVENT')),
  CONSTRAINT chk_notification__kind CHECK (kind IS NULL OR kind IN ('INFO','WARNING','ACTION_REQUIRED','SYSTEM_ERROR')),
  CONSTRAINT chk_notification__category CHECK (category IS NULL OR category IN ('PAGE_MODEL','TENANT_CONFIG','USER','OUTLET','TERMINAL','SESSION','SALES','DRAW','RESULT','PAYOUT','BATCH','SYSTEM','SECURITY')),
  CONSTRAINT chk_notification__created_actor_type CHECK (created_by_actor_type IS NULL OR created_by_actor_type IN ('APP_USER','SELLER_TERMINAL'))
);
...
CREATE TABLE notification_translation (
  ...
  deleted_by uuid,
  version bigint NOT NULL DEFAULT 0,
  CONSTRAINT uq_notification_translation_locale UNIQUE (notification_id, locale),
  CONSTRAINT chk_notification_translation__locale CHECK (locale IN ('fr','en','ht'))
);

CREATE TABLE notification_publication (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  notification_id uuid NOT NULL REFERENCES notification(id),
  tenant_id uuid REFERENCES tenant(id),
  ...
  expires_at timestamptz,
  republished_from_publication_id uuid REFERENCES notification_publication(id),
  reason text,
  created_by_actor_type varchar(32),
  created_by_actor_id uuid,
  created_at timestamptz NOT NULL DEFAULT now(),
  ...
  deleted_by uuid,
  version bigint NOT NULL DEFAULT 0,
  CONSTRAINT uq_notification_publication_no UNIQUE (notification_id, publication_no),
  CONSTRAINT chk_notification_publication__status CHECK (status IN ('PUBLISHED','EXPIRED','CANCELLED')),
  CONSTRAINT chk_notification_publication__audience_type CHECK (audience_type IN ('SPECIFIC_ACTORS','PLATFORM_ADMINS','ALL_APP_USERS','TENANT_ADMINS','TENANT_APP_USERS','TENANT_SELLER_TERMINALS')),
  CONSTRAINT chk_notification_publication__actor_type CHECK (created_by_actor_type IS NULL OR created_by_actor_type IN ('APP_USER','SELLER_TERMINAL'))
);

CREATE TABLE notification_recipient (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  notification_id uuid NOT NULL REFERENCES notification(id),
  publication_id uuid NOT NULL REFERENCES notification_publication(id),
  tenant_id uuid REFERENCES tenant(id),
  recipient_actor_type varchar(32) NOT NULL,
  ...
  deleted_by uuid,
  version bigint NOT NULL DEFAULT 0,
  CONSTRAINT uq_notification_recipient_actor UNIQUE (publication_id, recipient_actor_type, recipient_actor_id),
  CONSTRAINT chk_notification_recipient__actor_type CHECK (recipient_actor_type IN ('APP_USER','SELLER_TERMINAL'))
);
...
CREATE TABLE notification_user_state (
  ...
  deleted_by uuid,
  version bigint NOT NULL DEFAULT 0,
  CONSTRAINT uq_notification_user_state_actor UNIQUE (publication_id, actor_type, actor_id),
  CONSTRAINT chk_notification_user_state__actor_type CHECK (actor_type IN ('APP_USER','SELLER_TERMINAL'))
);

CREATE TABLE notification_delivery_policy (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  notification_id uuid NOT NULL REFERENCES notification(id),
  publication_id uuid REFERENCES notification_publication(id),
  tenant_id uuid REFERENCES tenant(id),
  ...
  deleted_by uuid,
  version bigint NOT NULL DEFAULT 0,
  CONSTRAINT uq_notification_delivery_policy_channel UNIQUE (notification_id, publication_id, channel),
  CONSTRAINT chk_notification_delivery_policy__channel CHECK (channel IN ('IN_APP','EMAIL','SMS','WHATSAPP','SLACK'))
);
...
CREATE TABLE notification_trigger_log (
  ...
  deleted_by uuid,
  version bigint NOT NULL DEFAULT 0,
  CONSTRAINT uq_notification_trigger_source UNIQUE (trigger_key, source_type, source_id)
);
