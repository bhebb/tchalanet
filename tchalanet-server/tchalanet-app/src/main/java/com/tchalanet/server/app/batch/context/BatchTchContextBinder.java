...
public class BatchTchContextBinder {
    ...
    public void bind(JobParameters jp) {
        String tenantIdRaw = defaultStr(jp.getString(JobParamKeys.TENANT_ID), null);

        String requestId =
            defaultStr(jp.getString(JobParamKeys.REQUEST_ID), UUID.randomUUID().toString());
        String actor = defaultStr(jp.getString(JobParamKeys.ACTOR), "batch");
