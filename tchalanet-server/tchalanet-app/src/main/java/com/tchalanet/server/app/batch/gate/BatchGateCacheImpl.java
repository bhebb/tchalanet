...
import com.tchalanet.server.common.job.gate.BatchGateFlagStore;
...
public class BatchGateCacheImpl implements BatchGateCache {
  ...
  public Optional<Boolean> getGlobal(JobKey jobKey) {
    if (jobKey == null) return Optional.empty();
    String cacheKey = NAMESPACE + ":g:" + jobKey.value();
    return Optional.ofNullable(flagCache.getBool(cacheKey, () -> loadGlobalFlag(jobKey)));
  }
  ...
  public void evictGlobal(JobKey jobKey) {
    if (jobKey != null) flagCache.evict(NAMESPACE + ":g:" + jobKey.value());
  }
