...
public record RecordManualDrawResultRequest(
    @NotNull @Schema(description = "Draw calendar date.", example = "2026-05-02")
        LocalDate drawDate,
    @NotBlank @Schema(description = "Result slot key.", example = "NY_MID") String slotKey,
    @NotBlank
        @Schema(
            description = "Operator/user identifier who recorded the result.",
            example = "ops@tchalanet.com")
        String recordedBy,
    @Schema(
            description = "Optional notes.",
            example = "Provider site unavailable; entered from official bulletin.")
        String notes,
    @NotBlank
    ...
    @NotBlank
        ...
        @Schema(
            description = "Haiti lot 2 result. Mapped server-side to pick4 prefix.",
            example = "45")
        String lot2,
    @NotBlank
        @Pattern(regexp = "\\d{2}", message = "lot3 must contain exactly 2 digits")
        @Schema(
            description = "Haiti lot 3 result. Mapped server-side to pick4 suffix.",
            example = "67")
        String lot3,
    ...
    @NotBlank
        @Schema(
            description = "Operational reason for manual entry.",
            example = "Manual correction approved by platform ops")
        String reason) {}
