...
public class PublicRuntimeBootstrapService {
  ...
  private PublicSettingsView resolvePublicSettings(String locale) {
    ...
    try {
      for (SettingView s :
          settingsAdminCatalog.listActiveByExposure(SettingExposure.PUBLIC_RUNTIME, null)) {
        if (s.valueType() == SettingValueType.BOOLEAN) {
          features.put(
              s.namespace() + "." + s.settingKey(), Boolean.parseBoolean(s.settingValue()));
        }
      }
    } catch (Exception e) {
  ...
  // Null tenantId → TenantThemeRuntimeService serves the global default preset (seeded as
  // "tchalanet")
