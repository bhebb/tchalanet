...
/**
 * Unified navigation entry — covers both {@code "destination"} and {@code "action"} items in shell
 * fragments (top app bar, navigation drawer, profile menu).
 *
 * <p>Discriminated by {@code type}:
 *
 * <ul>
 *   <li>{@code "destination"} — routable link; uses {@code activeMatch}, {@code badge}, {@code
 *       children}.
 *   <li>{@code "action"} — stateless action trigger; uses {@code style}, {@code confirm}.
 * </ul>
 *
 * <p>JSON field names use snake_case where the shell fragment schema requires it ({@code
 * label_key}, {@code active_match}, {@code reason_key}).
 */
public record NavigationEntry(
  ...
  public static NavigationEntry exactDestination(
      String id, String labelKey, String path, String icon) {
    return new NavigationEntry(
        id,
        "destination",
        labelKey,
  ...
  public static NavigationEntry action(String id, String labelKey, String icon) {
    return new NavigationEntry(
        id, "action", labelKey, null, null, "action", icon, null, null, false, null, null, null,
        "neutral", null);
  }

  /** Primary-styled action (e.g. CTA button). */
  public static NavigationEntry primaryAction(
      String id, String labelKey, String icon, String path) {
    return new NavigationEntry(
        id,
        "action",
        labelKey,
