 ...
 * <p>
 * Remaining sections (limits, i18n, pageModels)
 ...
 * openDraws, unread)
 * - canCreateSellerTerminal requires identity + address, games_pricing, and draws all
 * non-MISSING
 */
@Component
...
public class TenantReadinessAssembler {
    ...
    private static final List<SectionDescriptor> SECTIONS = List.of(
        ...
        new SectionDescriptor("generated_draws", "readiness.section.generated_draws", "/app/admin/draws"),
        ...
        new SectionDescriptor("subscription", "readiness.section.subscription", "/app/admin/subscription"),
    ...
    public TenantSetupView computeSetup(TenantReadinessView view) {
        ...
        Map<String, TenantReadinessStatus> statusById = sections.stream()
        ...
        int completed = (int) REQUIRED_STEP_GROUPS.stream()
        ...
        var next = REQUIRED_STEP_GROUPS.stream()
            .flatMap(List::stream)
    ...
    /**
     * Whether at least one Draw instance has actually been generated (not just configured).
     * Required: configured channels/games aren't enough to sell without a real Draw to bet on.
     */
    private TenantReadinessStatus checkGeneratedDraws(TchRequestContext ctx) {
        try {
            var page = queryBus.ask(new ListDrawsQuery(
                DrawSearchCriteria.of(null, null, null, null),
                PageRequest.of(0, 1)));
            return page != null && page.totalElements() > 0
    ...
    private TenantReadinessStatus rollUp(List<TenantReadinessSection> sections) {
        var rollupSections = sections.stream()
            .filter(s -> !NON_ROLLUP_SECTIONS.contains(s.id()))
            .toList();
        boolean anyMissing = rollupSections.stream().anyMatch(s -> s.status() == TenantReadinessStatus.MISSING);
        boolean anyPartial = rollupSections.stream().anyMatch(s -> s.status() == TenantReadinessStatus.PARTIAL);
        boolean allKnownReady = rollupSections.stream()
            .filter(s -> s.status() != TenantReadinessStatus.UNKNOWN)
            .allMatch(s -> s.status() == TenantReadinessStatus.READY);
        boolean anyKnown = rollupSections.stream()
