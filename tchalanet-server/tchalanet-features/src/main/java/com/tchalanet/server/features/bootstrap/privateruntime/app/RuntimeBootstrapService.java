...
public class RuntimeBootstrapService {
  ...
  public RuntimeBootstrapResponse privateBootstrap(TchRequestContext ctx) {
    ...
    CurrentUserView currentUser =
        identityApi.getCurrentUser(new GetCurrentUserRequest(ctx.currentUserIdRequired()));
    AccessSnapshotView accessSnapshot =
        accessControlApi.resolveUserAccess(ctx.currentUserIdRequired());
  ...
  public PrivateRuntimeStateResponse privateState(TchRequestContext ctx) {
    ...
    return new PrivateRuntimeStateResponse(
        status, readiness, notifications, null, versions, notices.isEmpty() ? null : notices);
  }
  ...
  private RuntimeBootstrapResponse sellerTerminalBootstrap(TchRequestContext ctx) {
    var terminal =
        queryBus.ask(
            new GetSellerTerminalQuery(ctx.tenantIdRequired(), ctx.sellerTerminalIdRequired()));
    ...
    return new RuntimeBootstrapResponse(
        ...
        entitlements,
        readiness,
        RuntimeNotificationSummary.empty(),
        navigationDrawer,
        pageModelRef,
        pageModelRef.route(),
        portalConfigProperties.toView(),
  ...
  private PrivateRuntimeStateResponse sellerTerminalState(TchRequestContext ctx) {
    ...
    return new PrivateRuntimeStateResponse(
        ...
        RuntimeNotificationSummary.empty(),
        null,
        RuntimeVersionHints.of("boot-v1"),
        null);
  }

  // ── space resolution ──────────────────────────────────────────────────────

  private PrivateBootstrapSpace resolveSpace(
    ...
    if (hasTenantRole(accessSnapshot, "TENANT_OWNER")
        || hasTenantRole(accessSnapshot, "TENANT_ADMIN")) {
    ...
    return switch (role) {
      ...
      case TENANT_OWNER, TENANT_ADMIN -> PrivateBootstrapSpace.ADMIN;
    };
  }
  ...
  private TenantContextView resolveTenantContext(
      TchRequestContext ctx, PrivateBootstrapSpace space) {
    ...
    return tenantLookup
        .findById(tenantId)
        .map(t -> new TenantContextView(t.tenantId().value().toString(), t.code(), t.displayName()))
        .orElse(
            new TenantContextView(tenantId.value().toString(), ctx.effectiveTenantCode(), null));
  }
  ...
  private EntitlementsView resolveEntitlements(
    ...
    return new EntitlementsView(
        roles, tenantScope == null ? List.of() : List.copyOf(tenantScope.permissionKeys()));
  }

  private List<String> rolesForSpace(
      AccessSnapshotView accessSnapshot, PrivateBootstrapSpace space) {
  ...
  private TenantAccessScopeView tenantScopeForContext(
    ...
    var tenantId = ctx.effectiveTenantIdOrNull();
    if (tenantId == null) {
      return accessSnapshot.tenantScopes().size() == 1
          ? accessSnapshot.tenantScopes().getFirst()
          : null;
    }
    return accessSnapshot.tenantScopes().stream()
  ...
  private boolean isTenantAdminScope(TenantAccessScopeView scope) {
    return scope.roleCodes() != null
        && (scope.roleCodes().contains("TENANT_OWNER")
            || scope.roleCodes().contains("TENANT_ADMIN"));
  }
  ...
  private Map<String, Boolean> resolveFeatureFlags(
    ...
    try {
      var resolved =
          settingsCatalog.resolve(ResolveSettingsCriteria.forTenant(tenantId, List.of()));
  ...
  private RuntimeThemeView resolveTheme(
    ...
    try {
      ...
      notices.add(
          RuntimeBootstrapNotice.warning(
              "theme.fallback", "Theme could not be loaded; default applied."));
      return RuntimeThemeView.fallback();
    }
  }
  ...
  private RuntimeI18nBundle resolveI18n(
    ...
    try {
      ...
      log.warn(
          "runtime.bootstrap: i18n bundle load failed for locale={} space={}", locale, space, e);
      notices.add(
          RuntimeBootstrapNotice.warning(
              "i18n.fallback", "i18n bundle could not be loaded; frontend fallback will be used."));
  ...
  private RuntimeNotificationSummary resolveNotifications(
      TchRequestContext ctx, CurrentUserView user, List<RuntimeBootstrapNotice> notices) {
    ...
    try {
      ...
      notices.add(
          RuntimeBootstrapNotice.warning(
              "notifications.unavailable", "Notification summary could not be loaded."));
