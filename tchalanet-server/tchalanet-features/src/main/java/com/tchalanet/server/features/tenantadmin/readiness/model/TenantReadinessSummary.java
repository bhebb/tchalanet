 ...
 * unread). MUST NOT contain section-level data — only the rolled-up status, the missing count, and
 * a bounded top-issues list.
 */
public record TenantReadinessSummary(
    TenantReadinessStatus status, int missingCount, List<TenantReadinessIssue> topIssues) {
