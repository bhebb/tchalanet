...
/**
 * Ops controller for batch gate management (enable/disable). NOTE: /api/v1 prefix is added
 * automatically by platform config. Endpoints: - GET
 * /platform/ops/batch/gates/{jobKey}?tenant_id=... - GET
 * /platform/ops/batch/gates/effective?job_keys=...&tenant_id=... - PUT
 * /platform/ops/batch/gates/{jobKey}
 */
@RestController
...
public class OpsBatchGateController {
  ...
  private static final List<String> DEFAULT_JOB_KEYS =
      List.of(
  ...
  public ApiResponse<Void> updateGate(
      @PathVariable String jobKey, @Valid @RequestBody GateUpdateRequest request) {
