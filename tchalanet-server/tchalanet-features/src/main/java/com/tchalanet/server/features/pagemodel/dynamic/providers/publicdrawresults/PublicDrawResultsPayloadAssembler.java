 ...
 *
 * <p>Grouped reads (target ≤ 2 per dashboard-overview-runtime-v1 §12): 1. {@link
 * ListPublicDrawResultSlotsQuery} — systematic, latest + next per slot 2. {@link
 * SearchPublicDrawResultsQuery} — conditional, bounded history when the widget props request it
 * ({@code includeHistory=true}).
 *
 * <p>Memoization is performed in {@link PublicDrawResultsProvider} using a key derived from the
 * props (slotKeys + provider + includeHistory + historyLimit) so two widgets with different needs
 * in the same request don't collide.
 */
@Component
...
public class PublicDrawResultsPayloadAssembler {
  ...
  public Payload assemble(Spec spec) {
    ...
    if (spec.includeHistory()) {
      int limit = Math.max(MIN_HISTORY_LIMIT, Math.min(MAX_HISTORY_LIMIT, spec.historyLimit()));
      var page =
          queryBus.ask(
              new SearchPublicDrawResultsQuery(
                  spec.slotKeys(), spec.provider(), null, null, PageRequest.of(0, limit)));
      history = page != null && page.items() != null ? page.items() : List.of();
    }

    return new Payload(slots != null ? slots : List.of(), history);
  }

  /** Specification of what to load, derived from a widget's props. */
  public record Spec(
      List<String> slotKeys, String provider, boolean includeHistory, int historyLimit) {

    public String memoKey() {
      return "public_draw_results:"
          + "slots="
          + (slotKeys == null ? "" : String.join(",", slotKeys))
          + "|provider="
  ...
  public record Payload(
      List<PublicDrawResultSlotView> slots, List<PublicDrawResultHistoryRowView> history) {}
}
