...
public class TicketPrintDocumentMapper {
    ...
    var content =
        new ReceiptDocumentContent(
            ...
            receipt.sections().stream()
                .map(
                    section ->
                        new DocumentSection(section.title(), toDocumentLines(section.lines())))
                .toList(),
  ...
  private DocumentTemplateKey templateKey(
    ...
    var docFormat =
        profile == null || profile.outputFormat() == null
            ? DocumentFormat.PDF
  ...
  private DocumentOptions options(
    ...
    var paperSize =
        (requested == PaperSize.RECEIPT_58MM || requested == PaperSize.RECEIPT_80MM)
            ? requested
  ...
  private boolean showQrCode(TenantInternalDocumentConfig.ReceiptConfig tenantReceiptConfig) {
    return !receiptConfigEnabled(tenantReceiptConfig)
        || Boolean.TRUE.equals(tenantReceiptConfig.showQrCode());
  }

  private boolean receiptConfigEnabled(
      TenantInternalDocumentConfig.ReceiptConfig tenantReceiptConfig) {
