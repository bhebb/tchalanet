...
public class GetTenantKpisService {
  ...
  public KpisResponse getKpis(TenantKpisCriteria criteria) {
    ...
    TenantKpisView view =
        queryBus.ask(new GetTenantKpisQuery(tenantId, criteria.fromDate(), criteria.toDate()));
  ...
  private KpisView toKpisView(TenantKpisView view) {
    if (view == null) {
      return new KpisView(
          0L, BigDecimal.ZERO, BigDecimal.ZERO, BigDecimal.ZERO, 0L, 0L, null, null, 0, null);
    }
    BigDecimal payoutRatio =
        view.totalSales().compareTo(BigDecimal.ZERO) > 0
            ? view.totalPayout().divide(view.totalSales(), 4, java.math.RoundingMode.HALF_UP)
            : null;
    BigDecimal avgTicket =
        view.ticketsSold() > 0
            ? view.totalSales()
                .divide(BigDecimal.valueOf(view.ticketsSold()), 2, java.math.RoundingMode.HALF_UP)
            : null;
