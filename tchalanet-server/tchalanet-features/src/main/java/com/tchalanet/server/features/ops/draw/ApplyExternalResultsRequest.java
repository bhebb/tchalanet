...
public record ApplyExternalResultsRequest(
    @Schema(
            description =
                "Optional tenant codes to target. Empty means all active tenants up to the interactive launch limit.")
        List<String> tenantCodes,
    @Schema(
            description =
                "Base calendar date used for the fetch window. Defaults server-side if omitted.",
            example = "2026-05-02")
        LocalDate baseDate,
    @Min(0)
        @Schema(
            description = "Number of days before baseDate to include.",
            example = "1",
            defaultValue = "0")
        Integer daysBack,
    @Size(max = 50)
        @Schema(
            description =
                "Optional slot keys to fetch. Empty means all eligible active result slots.",
    ...
    @Schema(
            ...
            example = "Provider correction verified by ops")
        String reason) {
