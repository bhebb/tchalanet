 ...
 *
 * <p>Hard rules: - calls owning domain APIs only (no direct INSERTs into other domains) - never
 * copies transactional data - V1 supports MINIMAL / DEFAULT_HAITI_LOTTERY / DEMO profiles only -
...
public class TenantProvisioningOrchestrator {
  ...
  public TenantProvisioningResultView provision(TenantProvisioningRequest request) {
    ...
    var created = tenantConfigApi.getTenantByCode(new GetTenantByCodeRequest(request.code()));
    ...
    var appliedPlanCode =
        applyProvisioningPlan(created.tenantId(), resolveProvisioningPlanCode(request));

    var domainStatuses =
        new TenantProvisioningDomainStatuses(
            "CREATED",
            "SEEDED_VIA_LISTENER",
            defaultTheme.code(),
            "DEFAULT",
  ...
  private static List<String> includedDomains(TenantProvisioningProfile profile) {
    return switch (profile) {
      case MINIMAL -> List.of("tenant_identity", "pagemodels", "theme", "settings", "i18n");
      case DEFAULT_HAITI_LOTTERY ->
          List.of(
              "tenant_identity",
              "pagemodels",
              "theme",
  ...
  private static List<String> notCopiedData() {
    return List.of("tickets", "sales", "audit", "notifications", "stats");
  }

  private void persistProvisioningProfileOptions(
      TenantId tenantId, TenantProvisioningRequest request) {
    var maryajGratisEnabled =
        request.maryajGratisEnabled() == null || request.maryajGratisEnabled();
    var rules =
        jsonUtils.parse(
            """
            {
  ...
  private static List<String> nextSteps(
      TenantProvisioningProfile profile, String initialAdminEmail) {
    ...
    return switch (profile) {
      case MINIMAL ->
          adminCreated
              ? List.of(
                  "CONFIGURE_GAMES",
                  "CONFIGURE_DRAW_CHANNELS",
                  "CREATE_SELLER_TERMINAL",
                  "CONFIGURE_SELLER_RULES",
                  "CONFIGURE_LIMITS",
  ...
  private void ensureProfileCatalog(TenantId tenantId, TenantProvisioningProfile profile) {
    ...
    for (String gameCode : defaultHaitiLotteryGameCodes()) {
      tenantGameApi.ensureTenantGame(
          EnsureTenantGamesRequest.builder().tenantId(tenantId).gameCode(gameCode).build());
    }
  ...
  private ThemePresetView resolveDefaultThemePreset() {
    return themeCatalog
        .findByCode(DEFAULT_THEME_PRESET_CODE)
  ...
  private InitialAdminProvisioningResult provisionInitialAdmin(
    ...
    String credentialStatus =
        adminResult.temporaryCredentialIssued()
            ...
            : (adminResult.created()
                ? "TEMPORARY_CREDENTIAL_NOT_RETURNED"
  ...
  private static List<String> defaultHaitiLotteryGameCodes() {
    return Arrays.stream(GameCode.values()).map(GameCode::name).toList();
  }
  ...
  private void upsertTenantLimit(
      TenantId tenantId, LimitScopeRef scope, RuleKey ruleKey, String paramKey, long paramValue) {
    commandBus.execute(
  ...
  private record TenantProvisioningContextResult(
      TenantReadinessView readiness, InitialAdminProvisioningResult initialAdmin) {}
