...
public class PosTicketReceiptService {
  ...
  public ResponseEntity<ByteArrayResource> print(
    ...
    var receipt =
        queryBus.ask(new FormatTicketReceiptPrintQuery(ticketId, profile, request.buyerLocale()));
    ...
    if (request.shouldSendToBuyer()) {
      ...
      var outboundDocument =
          toCommunicationDocument(receipt, request, rendered, receiptConfig, pdfOptions);
      var message =
  ...
  private TenantInternalDocumentConfig.ReceiptConfig resolveReceiptConfig(TchRequestContext ctx) {
    ...
    var tenantDocument =
        tenantConfigApi.getTenantDocumentConfig(
  ...
  private ResponseEntity<ByteArrayResource> toFileResponse(RenderedDocument document) {
    ...
    return new ResponseEntity<>(new ByteArrayResource(document.bytes()), headers, HttpStatus.OK);
  }
    ...
    return switch (request.channel()) {
      case SLACK, SLACK_INTERNAL, SLACK_TENANT_WEBHOOK ->
          new OutboundRecipient(
              ctx.effectiveTenantIdOrNull(), ctx.userId(), null, request.channelKey());
