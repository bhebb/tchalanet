...
class PageModelOnboardingServiceTest {
  ...
  void seedDefaultsSkipsTemplateWhenPageModelAlreadyExists() {
    ...
    var service =
        new PageModelOnboardingService(catalog, commandBus, queryBus, mock(JsonUtils.class));
  ...
  void seedDefaultsContinuesToNextTemplateWhenSeedingFails() {
    ...
    when(catalog.findDefaultGlobalTemplates())
        .thenReturn(List.of(failingTemplate, successTemplate));
