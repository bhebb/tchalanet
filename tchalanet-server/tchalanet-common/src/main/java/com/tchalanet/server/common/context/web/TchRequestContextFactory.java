...
public class TchRequestContextFactory {
   ...
   *
   * <p>Used after IdentityBootstrapStep + AccessResolutionStep. Tenant/access facts come from
   * ResolvedAccessContext, not from provider claims.
   */
  public TchRequestContext createFromResolvedAccess(
      HttpServletRequest req, ApiScope scope, ResolvedAccessContext resolved) {
    var base = baseData(req);

    var effectiveTenantUuid =
        resolved.effectiveTenantId() == null ? null : resolved.effectiveTenantId().value();

    var appUserUuid = resolved.appUserId() == null ? null : resolved.appUserId().value();

    var legacySystemRoles = toLegacyRoles(resolved.roleCodes(), resolved.actorType());

    var deletedVisibility = resolveDeletedVisibility(req, resolved.superAdmin());
   ...
   *
   * <p>Used for /public/**, public bootstrap, and legacy unauthenticated flows. Provider claims are
   * not read here; access facts come from the resolved-access pipeline.
   */
  public TchRequestContext createPublic(
      HttpServletRequest req, String defaultTenantCode, ApiScope scope) {
    var base = baseData(req);
  ...
  private static Set<TchRole> toLegacyRoles(Set<String> roleCodes, TchActorType actorType) {
  ...
  private BaseRequestData baseData(HttpServletRequest req) {
    ...
    return new BaseRequestData(
        req.getLocale(), requestId, clientIp, req.getHeader("User-Agent"), idempotencyKey);
  }
  ...
  private record BaseRequestData(
      Locale locale, String requestId, String clientIp, String userAgent, String idempotencyKey) {}
}
