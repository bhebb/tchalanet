 ...
 *
 * <p>Delegates to the target bus for all commands. Fail-open: if the ObservationRegistry is noop or
 * unavailable, the command executes normally. Does not reference SimpleCommandBus directly.
 */
@Slf4j
public class ObservedCommandBus implements CommandBus {
  ...
  public ObservedCommandBus(
      CommandBus delegate, ObservationRegistry registry, Set<String> allowlist) {
  ...
  public <R> R execute(Command<R> command) {
    ...
    var observation =
        Observation.createNotStarted("tch.command." + name, registry)
