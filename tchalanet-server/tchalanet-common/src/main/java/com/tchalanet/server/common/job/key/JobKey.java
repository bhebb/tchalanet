 ...
 *
 * <p>Format: <domain>:<capability>:<action> (minimum 3 segments) Example: draw:lifecycle:settle,
 * results:external:refresh
 *
 * <p>Rules: - segments separated by ':' - only lowercase letters, digits, underscores allowed - max
...
public final class JobKey {
  ...
  /**
   * Create a JobKey from raw input. Validates format and normalizes (trim, lowercase).
   *
  ...
  public static JobKey of(String raw) {
    ...
    if (segments.length < MIN_SEGMENTS) {
      throw new IllegalArgumentException(
          "jobKey must have at least "
              + MIN_SEGMENTS
              + " segments (domain:capability:action): "
              + normalized);
    }
