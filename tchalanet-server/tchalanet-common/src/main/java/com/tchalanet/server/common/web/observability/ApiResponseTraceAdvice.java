 ...
 *
 * <p>Does not modify the {@link ApiResponse} record. Returns a thin projection ({@link
 * ApiResponseWithTrace}) that adds the trace field while preserving all existing fields. Fail-open:
 * absent trace context produces no {@code trace} field.
 *
 * <p>Runs after {@code ApiResponseBodyAdvice} (which wraps raw controller return values into {@link
 * ApiResponse}). The body seen here is always already an {@link ApiResponse} or a non-ApiResponse
 * type that is passed through unchanged.
 */
...
public class ApiResponseTraceAdvice implements ResponseBodyAdvice<Object> {
  ...
  /**
   * Thin projection of {@link ApiResponse} that adds a {@code trace} field. Serialized by Jackson
   * as a plain object — no record modification needed.
   */
