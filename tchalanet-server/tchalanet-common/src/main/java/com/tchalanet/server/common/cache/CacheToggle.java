 ...
 * redeploy — the escape hatch when a cache starts returning bad data. Eviction/clear still work on
 * a disabled cache.
 *
 * <p>State is in-memory and per-instance: an operator must toggle each running instance (or this
 * can be backed by a shared store / settings later). It is not a functional mechanism — see cache
 * policy §11.
 */
@Component
