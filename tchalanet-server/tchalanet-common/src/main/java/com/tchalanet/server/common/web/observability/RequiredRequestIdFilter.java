 ...
 *
 * <ul>
 *   <li>missing header → 400 request_id.missing
 *   <li>invalid format → 400 request_id.invalid
 *   <li>valid header → MDC + request attribute, cleanup in finally
 * </ul>
 *
 * <p>On missing/invalid, a server-generated {@code requestId} is included in the ProblemDetail for
 * log correlation, but is never returned as X-Request-Id.
 */
@Slf4j
public class RequiredRequestIdFilter extends OncePerRequestFilter {
  ...
  private void rejectMissing(HttpServletRequest req, HttpServletResponse res) throws IOException {
    ...
    log.warn(
        "request_id.missing method={} path={} serverRequestId={}",
        req.getMethod(),
        req.getRequestURI(),
        serverRequestId);
    writeProblem(
  ...
  private void rejectInvalid(HttpServletRequest req, HttpServletResponse res) throws IOException {
    var serverRequestId = generateServerRequestId();
    log.warn(
        "request_id.invalid method={} path={} serverRequestId={}",
        req.getMethod(),
        req.getRequestURI(),
        serverRequestId);
    writeProblem(
