 ...
 *
 * <p>Backed by an injected {@link Clock} so that tests can use a fixed clock. Never use {@code
 * Instant.now()}, {@code LocalDate.now()}, or {@code ZoneId.systemDefault()} directly in domain or
 * application code — use this interface instead.
 *
...
public interface TchTimeProvider {
  ...
  /** Exposes the underlying clock, e.g. for passing to domain services that need it directly. */
  Clock clock();
}
