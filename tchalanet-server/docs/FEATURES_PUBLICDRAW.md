# Module features.publicdraw — Documentation

Version: 2026-01-11

But: exposer un read‑model public orienté UI pour les résultats de tirage (Home / Historique / Détail) en s'appuyant uniquement sur des façades globales (ResultSlotCatalog, DrawResultCatalog, NextDrawCalculator). Ce module ne doit pas accéder directement aux repos/entités internes de `drawresult` ou `resultslot`.
...
- GET /public/results/latest
  ...
  - Réponse : 200 List<PublicLatestDrawResultsResponse>
...
- Autorisé :
  ...
  - `com.tchalanet.server.core.drawresult.api.DrawResultCatalog` (findById, findByDateRange, findByResultSlotIdAndOccurredAt, findRefBySlotKeyAndDate)
  - `NextDrawCalculator` (service utilitaire — idéalement déplacé dans core.common.time ou core.resultslot.service)
...
5) Cache (optionnel mais recommandé pour la latence UI)
...
- Latest per channel (simple impl) :
