...
| `platform.identity.api` views/requests | `ExternalUserSubject` | Exposes provider-neutral subject in public identity contracts | Keep neutral subject naming while remaining identity mapping cleanup continues |
...
| `common.types.id.ExternalUserSubject` | Provider-neutral typed ID | Transitional provisioning and public API contract | Keep until remaining identity mapping coupling is removed; it is no longer persisted on `app_user` |
