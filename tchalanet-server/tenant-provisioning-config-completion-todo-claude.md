...
- [x] Activate/Suspend
- [ ] **Onglet Abonnement — placeholder à implémenter.** Brancher sur `GET /platform/subscriptions/{tenantId}` : plan actif, statut, dates, action "Appliquer/Changer de plan" (dialog sensible, motif requis — cf. spec §21).
- [ ] **Onglet Droits d'usage — placeholder à implémenter.** Dépend de C (endpoint entitlements agrégé). Afficher features activées + limites (max seller-terminals, max admins, max draw channels…).
- [ ] **Onglet Seller-terminals — placeholder à implémenter.** Lister via l'API seller-terminal existante filtrée par tenant (déjà utilisée côté `admin-seller-terminals.page.ts` pour le tenant courant — vérifier s'il existe une variante "tous tenants / par tenantId" côté super-admin, sinon l'ajouter côté backend).
- [ ] **Onglet Audit — placeholder à implémenter.** Vérifier l'existence d'un `AuditLog` query filtrable par `tenantId` (le module `platform.audit.api.AuditLog` est déjà utilisé côté annotations `@AuditLog` sur les commandes plan/subscription — confirmer qu'il y a une lecture paginée exploitable côté UI).
