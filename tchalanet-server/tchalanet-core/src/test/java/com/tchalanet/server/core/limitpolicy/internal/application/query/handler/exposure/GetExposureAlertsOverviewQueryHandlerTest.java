...
class GetExposureAlertsOverviewQueryHandlerTest {
  ...
  private static final DrawId DRAW = DrawId.of(UUID.fromString("20000000-0000-0000-0000-000000000001"));
  ...
  void computes_stake_ratio_when_max_stake_rule_present() {
    when(alerts.topByStake(any(), any(), anyInt()))
        .thenReturn(List.of(
            new ExposureAlertsReaderPort.Row(BetType.MATCH_1_2D, "12",
                new BigDecimal("600.00"), 6L)));
  ...
  void scope_key_for_draw_channel_contains_type_and_uuid() {
    ...
    assertThat(result.scopeKey()).startsWith("DRAW_CHANNEL:")
        .contains(CHANNEL.value().toString());
  }
  ...
  void scope_key_for_seller_terminal_contains_type_and_uuid() {
    ...
    assertThat(result.scopeKey()).startsWith("SELLER_TERMINAL:")
  ...
  void multiple_rows_are_all_returned() {
    when(alerts.topByStake(any(), any(), anyInt()))
        .thenReturn(List.of(
            new ExposureAlertsReaderPort.Row(BetType.MATCH_1_2D, "12", new BigDecimal("900"), 9L),
            new ExposureAlertsReaderPort.Row(BetType.MATCH_1_2D, "34", new BigDecimal("750"), 5L)));
    when(assignments.listActiveForTargets(any(), any())).thenReturn(List.of());
  ...
  private static LimitAssignment exposureAssignment(LimitScopeRef scope) {
    return LimitAssignment.createNew(
        ...
        scope, true, BreachOutcome.BLOCK,
