...
class TicketCommandMutationCoverageTest {
  ...
  void recordPrintMutation() {
    ...
    var handler =
        new RecordTicketPrintCommandHandler(
  ...
  void cancelMutation() {
    ...
    var writer = new CapturingWriter();
    var handler =
        new CancelTicketCommandHandler(
            new SingleTicketReader(original), writer, new CapturingPublisher(), CLOCK, fixedIds());

    handler.handle(
        new CancelTicketCommand(
            TENANT, original.identity().id(), USER, "customer requested cancellation"));
  ...
  void printThenCancelSequence() {
    ...
    var printHandler =
        new RecordTicketPrintCommandHandler(
            store, store, publisher, CLOCK, fixedIds(), new TicketPrintPolicyService());
    var cancelHandler = new CancelTicketCommandHandler(store, store, publisher, CLOCK, fixedIds());
  ...
  private static void setContext() {
    TchContext.set(
        TchRequestContext.startupTenant(TENANT.value(), "test").withAppUserId(USER.value()));
  }
