...
class EvaluatePromotionQueryHandlerTest {
    ...
    void appliesSellerTerminalOverrides() {
        ...
        var rule = maryajRule();
        var handler = new EvaluatePromotionQueryHandler(
            () -> List.of(rule),
            (tenant, terminal) -> List.of(new SellerTerminalPromotionEffectOverride(
        ...
        assertThat(decision.effects().getFirst().choiceMode()).isEqualTo(PromotionChoiceMode.SELLER_SELECTS);
        assertThat(decision.contextHash()).contains(".");
        assertThat(decision.notices()).contains(
            "promotionDecision.applied",
            "promotionDecision.terminalOverrideApplied"
        );
    }
