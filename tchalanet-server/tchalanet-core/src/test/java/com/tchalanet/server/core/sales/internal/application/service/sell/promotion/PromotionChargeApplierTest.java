...
/**
 * Money path: promotional charge waiver. A WAIVE_CHARGE effect must waive ONLY the
 * charge type it targets (keeping the original amount for print/audit but making it
 * non-buyer-facing, so the ticket total drops), and must be a strict no-op for any
 * other effect type. Currently only exercised via a heavy Spring integration test.
 */
