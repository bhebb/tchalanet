...
class TicketLinePreparationServiceTest {
  ...
  void resolvesEffectiveSellerTerminalOddsAndSnapshotsSettlementTermsOnly() {
    ...
    assertThat(
            lines.getFirst().settlementTermsSnapshot().terms().getFirst().payoutRule().multiplier())
  ...
  void resolvesLotto3BoxPricingVariantBeforeOddsLookup() {
    ...
    assertThat(queryBus.captured.pricingVariantCode())
  ...
  void exactPlusBoxDistributesOddCentStakeWithoutRejectingSale() {
    ...
    assertThat(line.settlementTermsSnapshot().terms().get(0).payoutBaseAmount())
        .isEqualByComparingTo("5.01");
    assertThat(line.settlementTermsSnapshot().terms().get(1).payoutBaseAmount())
        .isEqualByComparingTo("5.00");
    assertThat(
            line.settlementTermsSnapshot().terms().stream()
  ...
  void pricingRuleSnapshotsAreNotRetroactive() {
    ...
    var firstLine =
        service
            .toTicketLines(TENANT_ID, SELLER_TERMINAL_ID, List.of(input), CurrencyCode.of("HTG"))
            .getFirst();
  ...
  private static TicketLinePreparationService service(
      QueryBus queryBus, TenantGameApi tenantGameApi) {
  ...
  private static final class CapturingQueryBus implements QueryBus {
    ...
    public <R> R ask(Query<R> query) {
      ...
      var resolvedOdds =
          effectiveOddsByVariant.getOrDefault(captured.pricingVariantCode(), effectiveOdds);
      return (R)
              ...
              new TenantBetTypeOptionConfigView(
                  ...
                  List.of(
                      ...
                      new TenantBetOptionView(
                          (short) 3, "Exact + Box", "Exact + Box", false, true, 3)))));
    }
