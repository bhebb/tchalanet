 ...
 *
 * <ul>
 *   <li>{@code stake}, all charges, and {@code total} share the same currency.
 *   <li>{@code total == stake + Σ buyer-facing charges}.
 * </ul>
 *
 * <p>Charges paid by SELLER or TENANT are still listed (for reporting and ledger), but they do NOT
 * contribute to {@code total}. Only BUYER-facing charges inflate the buyer total.
 *
 * <p>The {@code charges} list is immutable. Pass {@link List#of()} when there is no charge.
 */
public record TicketMoneyBreakdown(Money stake, List<TicketCharge> charges, Money total) {
  public TicketMoneyBreakdown {
    ...
    if (!total.currency().equals(currency)) {
      throw new IllegalArgumentException("Total currency must match stake currency");
    }
    for (var charge : charges) {
      if (!charge.amount().currency().equals(currency)) {
        throw new IllegalArgumentException("Charge currency must match stake currency: " + charge);
      }
    }
    ...
    if (!expectedTotal.equals(total)) {
      throw new IllegalArgumentException(
          "Invariant violated: stake ("
              + stake
              + ") + buyer-facing charges ("
              + buyerFacing
              + ") must equal total ("
              + total
              + "), got "
