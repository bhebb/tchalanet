  ...
  private static List<DrawId> fetch(
    ...
    var criteria =
        new SettleableDrawCriteria(
            ctx.tenantId, ctx.from, ctx.to, ctx.maxDraws != null ? ctx.maxDraws : 1000L, ctx.force);

    return port.findSettleableDrawIds(criteria);
  }

  private record JobCtx(TenantId tenantId, Instant from, Instant to, Long maxDraws, boolean force) {
    static JobCtx from(
        Clock clock, String tenantIdRaw, Long daysBack, Long maxDraws, String forceRaw) {
