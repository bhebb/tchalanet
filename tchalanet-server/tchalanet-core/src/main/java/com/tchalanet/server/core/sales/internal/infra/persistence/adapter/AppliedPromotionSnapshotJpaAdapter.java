...
class AppliedPromotionSnapshotJpaAdapter implements AppliedPromotionSnapshotWriterPort {
  ...
  public void createIfAbsent(
    ...
    if (repository
        .findByTicketIdAndPromotionDecisionId(ticketId.value(), decision.decisionId().value())
        .isPresent()) {
      return;
    }
  ...
  private Map<String, Object> toSnapshotJson(
      PromotionDecision decision, Ticket ticket, Instant appliedAt) {
  ...
  private Map<String, Object> toLineEffectJson(TicketLine line) {
    ...
    json.put(
        "originalStakeAmount",
        line.origin().name().equals("PROMOTION") ? null : line.stakeAmount().amount());
  ...
  private Map<String, Object> toChargeEffectJson(TicketCharge charge) {
    ...
    json.put(
        "finalBuyerChargeAmount",
        charge.isWaived() ? java.math.BigDecimal.ZERO : charge.amount().amount());
