...
// [Phase 3A] @UseCase + QueryHandler pour câblage CQRS (analysis §MAJEUR command_query_handlers.md
// §4.2)
...
public class ResolveEffectivePageModelQueryHandler
  ...
  public PageModelDoc handle(ResolveEffectivePageModelQuery q) {
    ...
    boolean alreadyDefaultTenant =
        q.tenantId().map(TenantId::value).filter(DEFAULT_TENANT_UUID::equals).isPresent();

    if (alreadyDefaultTenant) {
      throw new TchNotFoundException(
          "PAGE_MODEL_NOT_FOUND", "Page model not found: " + q.logicalId());
    }
  ...
  private PageModelDoc toDoc(PageModelInstance inst) {
    if (inst == null || inst.modelJson() == null) {
      throw new TchNotFoundException("PAGE_MODEL_EMPTY", "Page model has empty modelJson");
    }
    ...
    try {
      ...
      throw new RuntimeException(
          "Failed to convert PageModelInstance.modelJson to PageModelDoc", e);
    }
  }
}
