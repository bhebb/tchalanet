...
/**
 * Per-request context for PageModel dynamic resolution. Memoizes both successful and failed loads
 * so a grouped payload is fetched at most once per request. A failure is re-thrown for every widget
 * that depends on the same source, but the underlying load is not retried.
 */
