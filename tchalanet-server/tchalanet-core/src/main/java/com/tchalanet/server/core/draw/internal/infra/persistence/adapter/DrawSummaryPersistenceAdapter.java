...
public class DrawSummaryPersistenceAdapter
  ...
  public Optional<DrawSummary> findById(DrawId drawId) {
    ...
    return repo.findByTenantIdAndDrawId(tenantId.value(), drawId.value()).map(mapper::toProjection);
  }
  ...
  public TchPage<DrawSummary> findByCriteria(DrawSearchCriteria criteria, Pageable pageable) {
    ...
    if (criteria.from() != null) {
      spec =
          spec.and(
              (root, query, cb) -> cb.greaterThanOrEqualTo(root.get("drawDate"), criteria.from()));
    }
  ...
  public TchPage<DrawSummary> listNext(DrawSearchCriteria criteria, Pageable pageable) {
    ...
    int lookaheadHours =
        criteria.lookaheadHours() == null ? DEFAULT_LOOKAHEAD_HOURS : criteria.lookaheadHours();
