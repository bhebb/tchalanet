...
public class SlackDrawResultFetchNotificationAdapter implements DrawResultFetchNotificationPort {
  ...
  private SendOutboundMessageRequest slackRequest(
      String type, String title, String body, String correlationKey) {
   ...
   *
   * <p>Si OutboundRecipient a un factory, utilise par exemple : return
   * OutboundRecipient.of(SLACK_INTERNAL);
   *
   * <p>Si SlackProvider résout le channel via metadata["slackChannel"], tu peux retourner null.
   */
  ...
  private String buildFetchedBatchMessage(List<DrawResultFetchNotification> notifications) {
    ...
    sb.append("[INFO] Draw results fetched\n\nDate: ").append(first.resultDate());
    sb.append("\nResults: ").append(notifications.size()).append("\n\n");
  ...
  private String buildFetchFailureBatchMessage(
    ...
    sb.append("[WARN] Draw result fetch failures\n\nBase Date: ").append(notification.baseDate());
    sb.append("\nDays Back: ").append(notification.daysBack());
    sb.append("\nFailures: ").append(notification.totalFailures()).append("\n\n");
