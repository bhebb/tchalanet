...
/** Archive dataset provider for high-volume {@code sales_ticket_line} rows. */
@Component
@RequiredArgsConstructor
@Slf4j
public class SalesTicketLineArchiveDatasetProvider implements ArchiveDatasetProvider {

  static final int SCHEMA_VERSION = 1;
  static final String TABLE = "sales_ticket_line";

  private static final ArchiveDatasetKey KEY = ArchiveDatasetKey.of(TABLE, "Sales Ticket Lines");
  ...
  public ArchiveDatasetPlan plan(ArchivePeriod period, UUID tenantId) {
    ...
    long count = ticketRepo.countLinesByTicketPeriod(from, to, tenantId);
  ...
  public ArchiveExportResult export(ArchiveExportRequest request) {
    ...
    long[] exported = {0};
    ticketRepo.streamLinesByTicketPeriod(from, to, request.tenantId(), row -> {
      request.rowSink().accept(row);
      exported[0]++;
    });

    log.info("sales_ticket_line export: {} rows period={}/{} tenant={}",
  ...
  public List<ArchiveLookupEntry> generateLookupRows(
    ...
    return ticketRepo.findLineLookupRows(from, to, tenantId).stream()
