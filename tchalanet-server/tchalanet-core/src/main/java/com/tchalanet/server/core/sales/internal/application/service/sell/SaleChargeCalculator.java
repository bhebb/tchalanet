...
public class SaleChargeCalculator {
  ...
  public List<TicketCharge> compute(TenantId tenantId, SellTicketCommand command) {
    ...
    if (opts.sendSms()) {
      appendCommunicationChargeIfApplicable(
          charges, tenantId, CommunicationChannel.SMS, TicketChargeType.BUYER_SMS);
    }

    if (opts.sendWhatsapp()) {
      appendCommunicationChargeIfApplicable(
          charges, tenantId, CommunicationChannel.WHATSAPP, TicketChargeType.BUYER_WHATSAPP);
    }

    if (opts.sendEmail()) {
      appendCommunicationChargeIfApplicable(
          charges, tenantId, CommunicationChannel.EMAIL, TicketChargeType.BUYER_EMAIL);
    }
  ...
  private void appendCommunicationChargeIfApplicable(
    ...
    communicationFeePolicy
        .feeFor(tenantId, channel)
        .ifPresent(
            fee -> {
              ...
              out.add(
                  new TicketCharge(
                      chargeType, fee.amount(), ChargePaidBy.fromCommunicationFee(fee.paidBy())));
            });
  }
}
