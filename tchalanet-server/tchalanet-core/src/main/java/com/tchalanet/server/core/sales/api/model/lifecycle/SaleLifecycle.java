...
public record SaleLifecycle(
  ...
  public SaleLifecycle approved(UserId by, Instant now) {
    ApprovalTrace updated =
        (approval != null)
            ? new ApprovalTrace(
                approval.requestId(), approval.requestedBy(), approval.requestedAt(), now, by)
            : new ApprovalTrace(null, null, null, now, by);
    return new SaleLifecycle(
        TicketSaleStatus.APPROVED, soldAt, placedAt, updated, rejection, cancellation, voiding);
  }
