...
/** Archive dataset provider for {@code sales_ticket_line_coverage} snapshot rows. */
@Component
@RequiredArgsConstructor
@Slf4j
public class SalesTicketLineCoverageArchiveDatasetProvider implements ArchiveDatasetProvider {

  static final int SCHEMA_VERSION = 1;
  static final String TABLE = "sales_ticket_line_coverage";

  private static final ArchiveDatasetKey KEY = ArchiveDatasetKey.of(TABLE, "Sales Ticket Line Coverages");
  ...
  public ArchiveDatasetPlan plan(ArchivePeriod period, UUID tenantId) {
    ...
    long count = ticketRepo.countLineCoveragesByTicketPeriod(from, to, tenantId);
  ...
  public ArchiveExportResult export(ArchiveExportRequest request) {
    ...
    long[] exported = {0};
    ticketRepo.streamLineCoveragesByTicketPeriod(from, to, request.tenantId(), row -> {
      request.rowSink().accept(row);
      exported[0]++;
    });

    log.info("sales_ticket_line_coverage export: {} rows period={}/{} tenant={}",
  ...
  public List<ArchiveLookupEntry> generateLookupRows(
    ...
    return ticketRepo.findLineCoverageLookupRows(from, to, tenantId).stream()
