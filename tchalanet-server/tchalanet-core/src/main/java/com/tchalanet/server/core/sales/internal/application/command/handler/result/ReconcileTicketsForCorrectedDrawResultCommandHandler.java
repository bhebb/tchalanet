...
public class ReconcileTicketsForCorrectedDrawResultCommandHandler
    implements CommandHandler<
        ...
        ReconcileTicketsForCorrectedDrawResultResult> {
  ...
  public ReconcileTicketsForCorrectedDrawResultResult handle(
    ...
    var projection =
        drawResultReader
            .findProjectionById(command.correctedDrawResultId())
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        "Corrected draw result projection not found: "
    ...
    for (var ticket : tickets) {
      try {
        ...
        correctedEvents.add(
            new TicketResultCorrectedEvent(
                EventId.of(idGenerator.newUuid()),
                now,
                saved.identity().tenantId(),
                command.drawId(),
        ...
        skipped++;
        log.warn(
            "sales.reconcile.corrected-result.skip ticketId={} drawId={} cause={}",
            ticket.identity().id(),
            command.drawId(),
            ex.toString());
      }
    ...
    return new ReconcileTicketsForCorrectedDrawResultResult(
        tickets.size(), correctedEvents.size(), skipped);
  }
}
