...
public class ApproveTicketSaleCommandHandler
    implements CommandHandler<ApproveTicketSaleCommand, Ticket> {
  ...
  public Ticket handle(ApproveTicketSaleCommand cmd) {
    ...
    var promotionDecisionId =
        saved.lines().stream()
            .filter(
                l ->
                    l.pricingSource() == TicketLinePricingSource.PROMOTION
    ...
    log.info(
        "Ticket approved ticketId={} tenantId={}",
        saved.identity().id(),
        saved.identity().tenantId());
