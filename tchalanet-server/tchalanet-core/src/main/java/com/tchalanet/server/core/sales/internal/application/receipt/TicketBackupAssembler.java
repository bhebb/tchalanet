...
public class TicketBackupAssembler {
  ...
  public TicketBackupAssembler(
      TicketReceiptMessageFormatter messageFormatter, TicketReceiptI18nResolver i18nResolver) {
  ...
  public TicketBackupInfo assemble(TicketReceiptView receipt) {
    ...
    var codeMsg =
        formatOrFallback(codeTpl, "Votre code est {code}.", "{code}", receipt.displayCode());

    var verifyTpl = translations.text(TicketReceiptI18nKeys.MESSAGE_BACKUP_VERIFY);
    var verifyMsg =
        formatOrFallback(verifyTpl, "Verifier sur {url}", "{url}", receipt.verificationUrl());

    return new TicketBackupInfo(
        receipt.displayCode(), receipt.verificationUrl(), shareable, codeMsg, verifyMsg);
  }
