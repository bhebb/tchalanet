...
public class PromotionJsonMapper {
  ...
  public static PromotionDecision fromJson(UUID id, Map<String, Object> json) {
    ...
    var effects =
        (effectsRaw instanceof List<?> list)
            ? list.stream()
    ...
    var notices =
        (noticesRaw instanceof List<?> list)
  ...
  private static PromotionEffect effectFromJson(Map<String, Object> m) {
    ...
    var campaignId =
        campaignIdRaw != null
            ? PromotionCampaignId.of(UUID.fromString((String) campaignIdRaw))
            : null;
    ...
    var choiceMode =
        choiceModeRaw != null ? PromotionChoiceMode.valueOf((String) choiceModeRaw) : null;
    var strategyRaw = m.get("generationStrategy");
    var strategy =
        strategyRaw != null ? SelectionGenerationStrategy.valueOf((String) strategyRaw) : null;
    var regenerable = Boolean.TRUE.equals(m.get("regenerableBeforeConfirm"));
    var maxRegenRaw = m.get("maxRegenerationsBeforeConfirm");
    int maxRegen =
        maxRegenRaw instanceof Number n ? n.intValue() : PromotionEffect.DEFAULT_MAX_REGENERATIONS;
    var amountRaw = m.get("amount");
    var amount =
        amountRaw instanceof Number n
  ...
  public static Map<String, Object> effectToJson(PromotionEffect effect) {
    ...
    out.put(
        "campaignId", effect.campaignId() == null ? null : effect.campaignId().value().toString());
    ...
    out.put(
        "quantityTiers",
        effect.quantityTiers().stream().map(PromotionJsonMapper::quantityTierToJson).toList());
    ...
    out.put(
        "generationStrategy",
        effect.generationStrategy() == null ? null : effect.generationStrategy().name());
