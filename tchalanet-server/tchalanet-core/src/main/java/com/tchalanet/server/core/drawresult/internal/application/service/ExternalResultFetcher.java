...
public class ExternalResultFetcher {
  ...
  public ResolvedExternalResults fetch(
    ...
    var p3 = sourceCfg.activePick3().map(cfg -> bundle.findByGameCode(cfg.gameCode())).orElse(null);

    var p4 = sourceCfg.activePick4().map(cfg -> bundle.findByGameCode(cfg.gameCode())).orElse(null);
