...
public class ResultSlotScheduleCalculator {
  ...
  public PublicNextResultTimeView calculateNextResultTime(
      LocalTime drawTime, String timezone, boolean slotActive) {
    if (!slotActive) {
    ...
    var todayDrawTime =
        now.withHour(drawTime.getHour()).withMinute(drawTime.getMinute()).withSecond(0).withNano(0);

    var expectedAt = now.isBefore(todayDrawTime) ? todayDrawTime : todayDrawTime.plusDays(1);

    var countdownSeconds = Math.max(0, ChronoUnit.SECONDS.between(now, expectedAt));
