...
public class PromotionTicketLineFactory {
  ...
  private TicketLine createLine(
    ...
    var selectionResult =
        resolveUniqueSelection(
            decision, effect, existingLines, generatedLines, command, index, betType, betOption);

    var payoutBase = effect.amount().setScale(2, RoundingMode.UNNECESSARY);

    var pricingVariantCode =
        SettlementVariantResolver.resolve(betType, betOption, selectionResult.rawSelection());

    var payoutRuleResolution =
        queryBus.ask(
    ...
    var payoutRule =
        payoutRuleResolution.effectiveRuleType() == PayoutRuleType.FIXED_AMOUNT
  ...
  private PromotionSelectionResolver.SelectionResult resolveUniqueSelection(
    ...
    for (int attempt = 0; attempt < MAX_SELECTION_ATTEMPTS; attempt++) {
      var result =
          selectionResolver.resolveSelection(decision, effect, command, index, betType, betOption);
      var selectionKey =
          selectionApi.canonicalize(betType, betOption, result.rawSelection()).key().value();
  ...
  private Set<String> usedSelections(
      BetType betType, List<TicketLine> existingLines, List<TicketLine> generatedLines) {
