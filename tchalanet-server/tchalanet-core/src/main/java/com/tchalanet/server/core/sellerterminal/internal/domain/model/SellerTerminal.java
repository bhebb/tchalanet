...
public record SellerTerminal(
  ...
  public SellerTerminal unblock(Instant now) {
    ...
    return new SellerTerminal(
        ...
        commissionRate,
        lastSeenAt,
        activatedAt != null ? activatedAt : now,
        null,
        null,
        null,
        disabledAt,
  ...
  public SellerTerminal resetPin(Instant now) {
    if (status == SellerTerminalStatus.DISABLED) {
      throw new SellerTerminalStatusException(
          id, status, "Cannot reset PIN for a disabled terminal");
    }
    return new SellerTerminal(
        id,
        tenantId,
