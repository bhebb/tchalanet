...
public class PayoutRepositoryAdapter implements PayoutReaderPort {
    ...
    @Override
    public Optional<PayoutClaim> findByTicketId(TicketId ticketId) {
        return jpaRepo.findByTicketId(ticketId.value()).map(mapper::toDomain);
    }

    @Override
    public Optional<PayoutClaim> findById(PayoutId payoutId) {
        return jpaRepo.findById(payoutId.value()).map(mapper::toDomain);
    }

    @Override
    public PayoutClaim getById(PayoutId payoutId) {
        return findById(payoutId)
            .orElseThrow(() -> new TchNotFoundException(payoutId.toString(), "PayoutClaim not found: "));
    }
}
