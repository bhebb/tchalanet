...
public class UpsertSellerTerminalPricingRuleOverrideCommandHandler
    implements CommandHandler<UpsertSellerTerminalPricingRuleOverrideCommand, UpsertSellerTerminalPricingRuleOverrideResult> {
    ...
    @Override
    @TchTx
    public UpsertSellerTerminalPricingRuleOverrideResult handle(UpsertSellerTerminalPricingRuleOverrideCommand c) {
        validate(c);
        ...
        var existing = reader.findActiveByNaturalKey(
            ...
            gameCode, c.pricingVariantCode());

        if (existing.isPresent()) {
            var updated = existing.get().update(
                effectiveOdds(c), ruleType(c), c.fixedAmount(), c.effectiveFrom(), c.effectiveTo(), c.reason(), c.actorId());
        ...
        var created = SellerTerminalOddsOverride.createNew(
            ...
            c.tenantId(), c.sellerTerminalId(),
            gameCode, c.pricingVariantCode(), betType, c.betOption(),
            effectiveOdds(c), ruleType(c), c.fixedAmount(),
            c.effectiveFrom(), c.effectiveTo(), c.reason(), c.actorId());
