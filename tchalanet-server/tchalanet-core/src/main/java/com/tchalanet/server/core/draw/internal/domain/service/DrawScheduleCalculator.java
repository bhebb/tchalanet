...
/**
 * Pure domain calculator that converts a channel schedule definition (date + time-of-day + timezone
 * + cutoff offset) into a {@link DrawScheduleSnapshot}.
 *
 * <p>This class is stateless and has no Spring dependencies; it can be used in both application
 * handlers and unit tests without a Spring context.
 *
 * <p>Usage:
 *
 * <pre>{@code
...
public final class DrawScheduleCalculator {
  ...
  public static DrawScheduleSnapshot compute(
      LocalDate drawDate, LocalTime drawTime, ZoneId zoneId, Duration cutoffBeforeDraw) {
    ...
    if (cutoffBeforeDraw.isZero()) {
      throw new IllegalArgumentException(
          "cutoffBeforeDraw must be > 0 (cutoff must be strictly before draw)");
    }
