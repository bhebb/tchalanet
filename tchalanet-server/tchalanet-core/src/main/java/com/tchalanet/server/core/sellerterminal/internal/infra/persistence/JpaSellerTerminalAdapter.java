...
public class JpaSellerTerminalAdapter
    implements SellerTerminalReaderPort, SellerTerminalWriterPort {
  ...
  public SellerTerminal save(SellerTerminal terminal) {
    var entity =
        repository.findById(terminal.id().value()).orElseGet(() -> mapper.toNewEntity(terminal));
  ...
  public Optional<SellerTerminal> findById(TenantId tenantId, SellerTerminalId id) {
    return repository.findByTenantIdAndId(tenantId.value(), id.value()).map(mapper::toDomain);
  }
  ...
  public Optional<SellerTerminal> findByExternalSubject(
    ...
    return repository
        .findByExternalSubject(provider, issuer, externalSubject)
        .map(mapper::toDomain);
  }
  ...
  public TchPage<SellerTerminalSummaryRow> search(
    ...
    Specification<SellerTerminalJpaEntity> spec =
        tenantSpec(tenantId.value()).and(SellerTerminalSpecifications.matching(criteria));
