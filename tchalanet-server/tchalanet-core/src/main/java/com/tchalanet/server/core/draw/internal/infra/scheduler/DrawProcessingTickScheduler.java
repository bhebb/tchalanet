...
public class DrawProcessingTickScheduler {
      ...
      try {
        var execution =
            batchJobStarter.start(
                DRAW_CLOSE, closeParamsFor(tenantId, Math.max(1, cfg.getMaxItemsPerTick()), now));
        log.info(
        ...
        errors++;
        log.warn(
            "draw.processing.close tenant failed tenantId={} err={}",
            tenantId,
            ex.getMessage(),
            ex);
      }
  ...
  private StepSummary runFetch(Instant now) {
    ...
    for (SlotDate candidate : due) {
      try {
        var execution =
            batchJobStarter.start(RESULTS_EXTERNAL_FETCH, fetchParamsFor(candidate, now));
  ...
  private StepSummary runApply(Instant now) {
    ...
    var dueByDate =
        due.stream()
            .collect(
                Collectors.groupingBy(SlotDate::drawDate, LinkedHashMap::new, Collectors.toList()));
    ...
    for (var entry : dueByDate.entrySet()) {
      ...
      if (processingCandidates.isEmpty()) {
        log.info("draw.processing.apply.skip no_processing_candidates drawDate={}", drawDate);
        continue;
      }
      ...
      for (TenantId tenantId : tenants) {
        try {
          ...
          var execution =
              batchJobStarter.start(
                  RESULTS_EXTERNAL_APPLY, applyParamsFor(tenantId, drawDate, slotKeys, now));
          log.info(
      ...
      try {
        var expectedOccurredAt =
            OccurredAtResolver.resolveOrThrow(
                null, candidate.drawDate(), slot.drawTime(), slot.timezone());

        out.add(
            new DrawProcessingSlotDate(
                slot.id(), slot.slotKey(), candidate.drawDate(), expectedOccurredAt));
  ...
  private List<SlotDate> dueSlotDates(
      String step, Instant now, DrawProperties.DueAfterDraw config) {
  ...
  private ResultSlotView toResultSlotView(Object rawSlot) {
    ...
    try {
      ...
      log.warn(
          "draw.processing.slot.invalid type={} err={}",
  ...
  private void clearContext(String source, TenantId tenantId) {
    try {
      ...
      log.error(
          "{} failed to clear context tenantId={} err={}", source, tenantId, ex.getMessage(), ex);
    }
  }
