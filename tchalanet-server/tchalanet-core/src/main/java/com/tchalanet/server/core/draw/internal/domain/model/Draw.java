...
import com.tchalanet.server.core.draw.internal.domain.exception.DrawCannotSettleWithoutResultException;
import com.tchalanet.server.core.draw.internal.domain.exception.DrawInvalidCancelException;
...
public final class Draw {
  ...
  public void overrideResult(DrawResultId resultId, Instant now, String reason) {
    ...
    if (this.status != DrawStatus.RESULTED) {
      throw new DrawInvalidOverrideException(
          id, "Can only override result for RESULTED draws (current: " + this.status + ")");
    }
  ...
  public void cancel(String reasonCode, String reasonLabel, Instant now) {
    ...
    this.cancelReasonLabel =
        (reasonLabel == null || reasonLabel.isBlank()) ? null : reasonLabel.trim();
  ...
  public void reschedule(LocalDate drawDate, Instant scheduledAt, Instant cutoffAt) {
    ...
    if (this.status != DrawStatus.SCHEDULED) {
      throw new IllegalStateException(
          "Cannot reschedule draw that is not SCHEDULED (current: " + this.status + ")");
    }
