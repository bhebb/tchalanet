...
public class GetExposureAlertsOverviewQueryHandler
  ...
  public ExposureAlertsOverviewView handle(GetExposureAlertsOverviewQuery q) {
    ...
    alerts.topByStake(q.drawId(), q.scope(), q.limit()).forEach(row -> rows.put(key(row), row));

    var items = rows.values().stream().map(row -> toItem(row, maxStake)).toList();

    return new ExposureAlertsOverviewView(q.tenantId(), q.drawId(), scopeKey(q.scope()), items);
  }
  ...
  private LimitContext contextForScope(GetExposureAlertsOverviewQuery q, java.time.Instant now) {
    ...
    return new LimitContext(
        q.tenantId(), userId, sellerTerminalId, q.drawId(), drawChannelId, now, List.of());
  }

  private ExposureAlertItemView toItem(ExposureAlertsReaderPort.Row row, BigDecimal maxStake) {
  ...
  private String scopeKey(LimitScopeRef scope) {
    return switch (scope) {
      ...
      case LimitScopeRef.SellerTerminalScope sellerTerminalScope ->
          "SELLER_TERMINAL:" + sellerTerminalScope.sellerTerminalId().value();
      case LimitScopeRef.DrawChannelScope channel ->
          "DRAW_CHANNEL:" + channel.drawChannelId().value();
