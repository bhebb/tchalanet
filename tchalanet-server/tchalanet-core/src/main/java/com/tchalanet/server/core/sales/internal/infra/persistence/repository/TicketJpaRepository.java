...
public interface TicketJpaRepository extends TchJpaRepository<TicketJpaEntity, UUID> {
  ...
  @Query(
      "SELECT COUNT(t) FROM TicketJpaEntity t WHERE t.sellerTerminalId = :sellerTerminalId AND t.tenantId = :tenantId AND t.createdAt >= :from AND t.createdAt < :to")
  ...
  @Query(
      "SELECT COALESCE(SUM(t.totalAmount), 0) FROM TicketJpaEntity t WHERE t.sellerTerminalId = :sellerTerminalId AND t.tenantId = :tenantId AND t.createdAt >= :from AND t.createdAt < :to")
  ...
  @Query(
      "SELECT COUNT(t) FROM TicketJpaEntity t WHERE t.tenantId = :tenantId AND t.saleStatus = :status AND t.createdAt >= :from AND t.createdAt < :to")
  ...
  @Query(
      "SELECT COALESCE(SUM(t.totalAmount), 0) FROM TicketJpaEntity t WHERE t.tenantId = :tenantId AND t.saleStatus = :status AND t.createdAt >= :from AND t.createdAt < :to")
  ...
  @Query(
      "SELECT COUNT(DISTINCT t.sellerTerminalId) FROM TicketJpaEntity t WHERE t.tenantId = :tenantId AND t.saleStatus = :status AND t.sellerTerminalId IS NOT NULL AND t.createdAt >= :from AND t.createdAt < :to")
  ...
  @Query(
      "SELECT l.gameCode, COUNT(DISTINCT t.id), COALESCE(SUM(l.stakeAmount), 0) "
