...
/**
 * Lists the upcoming draws ({@code SCHEDULED}/{@code OPEN}) across all channels of the current
 * tenant within the next {@code lookaheadHours} hours. Tenant scope is inferred from the request
 * context (RLS-filtered).
 *
 ...
 */
public record ListUpcomingDrawsTenantWideQuery(int lookaheadHours, int limit)
