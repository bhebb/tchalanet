...
public class GenerateDrawsForRangeCommandHandler
  ...
  public GenerateDrawsForRangeResult handle(GenerateDrawsForRangeCommand command) {
    ...
    if (command.dryRun()) {
      ...
      return new GenerateDrawsForRangeResult(
          0, acc.skippedNotDay, acc.alreadyExists, 0, acc.skippedProviderClosed);
    }
    ...
    return new GenerateDrawsForRangeResult(
        created, acc.skippedNotDay, acc.alreadyExists, conflicts, acc.skippedProviderClosed);
  }

  private Acc generateRows(
      GenerateDrawsForRangeCommand command, List<DrawChannelCalendarRow> channels) {
    ...
    for (var c : channels) {
      providerClosedCache.computeIfAbsent(
          c.resultSlotId(),
          slot ->
              resultSlotCalendarReader.findUnavailableDates(slot, command.from(), command.to()));
    }
    ...
    while (!date.isAfter(command.to())) {
      for (var c : channels) {
        ...
        var providerClosed = providerClosedCache.getOrDefault(c.resultSlotId(), Set.of());
        ...
        var snap =
            DrawScheduleCalculator.compute(
                date, c.drawTime(), zone, Duration.ofSeconds(c.cutoffSec()));
  ...
  private record Acc(
      List<NewDrawRow> rows, int skippedNotDay, int alreadyExists, int skippedProviderClosed) {}
