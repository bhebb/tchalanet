...
public class OverrideDrawResultCommandHandler
  ...
  public OverrideDrawResultResult handle(OverrideDrawResultCommand command) {
    ...
    resultReader
        .findByResultSlotIdAndResultDate(slot.id(), command.drawDate())
        .ifPresent(
            drId -> {
  ...
  private void applyOverrideToDraws(
      DrawResultWriterPort.UpsertResult res, ResultSlotView slot, String reason) {
  ...
  private @NonNull ObjectNode buildSourceResult(
      OverrideDrawResultCommand command, ResultSlotView slot, Instant occurredAt) {
