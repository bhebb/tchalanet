...
/**
 * Confirm: payload = preparationId + idempotencyKey only. Rejoue le pipeline sell standard
 * (validation money/limits/session conservée) avec les sélections promotionnelles épinglées depuis
 * la préparation — les numéros du ticket sont exactement ceux du dernier preview. Double confirm
 * même idempotencyKey -> même ticket. Aucune régénération possible ici.
 */
@UseCase
...
public class ConfirmPreparedSaleCommandHandler
  ...
  public ConfirmPreparedSaleResult handle(ConfirmPreparedSaleCommand cmd) {
    ...
    store.confirm(
        preparation.id(), result.ticketId().value(), cmd.idempotencyKey(), timeProvider.now());
  ...
  /**
   * Pins each stored generated selection on the rebuilt command. Index is per gameCode group order
   * (quantity = 1 in V1). decisionId is left null: the sell pipeline issues a fresh decision id at
   * confirm and PromotionSelectionResolver matches choices by gameCode + index.
   */
