...
/**
 * Versioned platform template for the default Maryaj gratuit campaign. Lives in code (git-versioned
 * seed); a dedicated template table is a later evolution if needed. Each tenant gets its own
 * instance and can then pause, deactivate, or edit amount/eligibility independently.
 */
public final class MaryajGratisDefaultTemplate {
  ...
  static final List<PromotionQuantityTier> DEFAULT_QUANTITY_TIERS =
      List.of(
  ...
  public static CreatePromotionCampaignCommand createCommand(TenantId tenantId, Instant now) {
    return createCommand(
        new InstantiateDefaultMaryajGratisCommand(
            tenantId, null, null, null, null, null, null, null, null, null, null, null),
        now);
  }

  public static CreatePromotionCampaignCommand createCommand(
      InstantiateDefaultMaryajGratisCommand cmd, Instant now) {
    var payoutBaseAmount =
        positiveAmountOrDefault(cmd.payoutBaseAmount(), DEFAULT_PAYOUT_BASE_AMOUNT);
    var quantityMode =
        cmd.quantityMode() == null ? PromotionQuantityMode.TIERED_PAID_AMOUNT : cmd.quantityMode();
    ...
    if (quantityMode == PromotionQuantityMode.PER_PAID_AMOUNT && stepPaidAmount == null) {
      throw new IllegalArgumentException(
          "stepPaidAmount is required for PER_PAID_AMOUNT Maryaj gratis");
    }
    var quantityTiers =
        quantityMode == PromotionQuantityMode.TIERED_PAID_AMOUNT
    ...
    if (quantityMode == PromotionQuantityMode.TIERED_PAID_AMOUNT) {
      maxQuantity =
          quantityTiers.stream().mapToInt(PromotionQuantityTier::quantity).max().orElse(quantity);
    }
    var choiceMode =
        cmd.choiceMode() == null ? PromotionChoiceMode.AUTO_GENERATE : cmd.choiceMode();
    ...
    if (quantityMode == PromotionQuantityMode.TIERED_PAID_AMOUNT) {
      effectParams.put(
          "quantityTiers",
          quantityTiers.stream().map(MaryajGratisDefaultTemplate::quantityTierParams).toList());
      effectParams.put("maxQuantity", String.valueOf(maxQuantity));
    }
    ...
    return new CreatePromotionCampaignCommand(
        ...
        List.of(
            new PromotionRuleConfigInput(
                ...
                100,
                List.of(
                    new PromotionEligibilityConfigInput(
                        PromotionEligibilityType.MIN_PAID_TOTAL, Map.of("amount", MIN_PAID_TOTAL))),
                List.of(
                    new PromotionEffectConfigInput(
                        PromotionEffectType.FREE_GAME_LINE, effectParams)))));
  }
  ...
  private static List<PromotionQuantityTier> quantityTiersOrDefault(
      List<PromotionQuantityTier> tiers) {
