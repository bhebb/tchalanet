...
/**
 * SalePreparation retention (DOMAIN_SALES.md §11) : - expire les DRAFT au-delà du TTL (filet du
 * check paresseux au confirm/regenerate) ; - purge EXPIRED/CANCELLED après 7 jours ; - purge
 * CONFIRMED après 30 jours (le ticket persisté reste la vérité).
 */
@Component
@ConditionalOnProperty(
    prefix = "tch.sales.preparation",
...
public class SalePreparationRetentionScheduler {
    ...
    try {
      ...
      for (TenantId tenantId : tenantRegistry.listActiveTenantIds()) {
        try {
          ...
          log.warn("sales: preparation retention tenant {} failed — {}", tenantId, ex.getMessage());
        } finally {
          binder.clear();
        }
      }
  ...
  private void runForTenant(java.time.Instant now) {
    ...
    int purgedTerminal =
        preparationRepository.purgeByStatusOlderThan(
            List.of(SalePreparationStatus.EXPIRED, SalePreparationStatus.CANCELLED),
            now.minus(TERMINAL_RETENTION));
    int purgedConfirmed =
        preparationRepository.purgeByStatusOlderThan(
            List.of(SalePreparationStatus.CONFIRMED), now.minus(CONFIRMED_RETENTION));
    if (expired > 0 || purgedTerminal > 0 || purgedConfirmed > 0) {
      log.info(
          "sales: preparation retention expired={} purgedTerminal={} purgedConfirmed={}",
          expired,
          purgedTerminal,
          purgedConfirmed);
    }
