...
/**
 * Applies PromotionDecision effects to sale materialization. V1 creates/updates TicketLine
 * snapshots. Settlement must later consume only these snapshots.
 */
@Component
  ...
  public AppliedSalePromotionEffects apply(
    ...
    for (var effect : decision.effects()) {
      switch (effect.type()) {
        ...
        case FREE_GAME_LINE ->
            lines.addAll(
                promotionTicketLineFactory.createLines(
                    effect, decision, lines, command, sellerTerminalId, currency));
      }
    }

    return new AppliedSalePromotionEffects(List.copyOf(lines), List.copyOf(finalCharges));
  }
}
