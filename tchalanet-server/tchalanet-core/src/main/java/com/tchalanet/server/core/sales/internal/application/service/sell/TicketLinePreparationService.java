 ...
 *
 * <p>This service trusts its caller for input validity: command-level checks (line number, stake
 * positive, bet type / option range, game support) live in {@link
 * SalePreparationOrchestrator#validateCommand}. The checks left here are internal invariants that
 * should never fire if the policy service did its job — if they do, it's a bug, not a user error.
 *
 * <p>Responsibilities:
 *
 * <ul>
 *   <li>Generate the line id via {@link IdGenerator}.
 *   <li>Resolve effective payout rules via {@link ResolveSellerTerminalPayoutRuleQuery}.
 *   <li>Snapshot the settlement terms used later if the official result matches.
 *   <li>Canonicalize the raw selection via {@link SelectionApi}.
 *   <li>Wrap amounts in {@link Money} with the ticket's currency.
 * </ul>
 */
@Component
...
public class TicketLinePreparationService {
  ...
  public List<TicketLine> toTicketLines(
    ...
    return lines.stream().map(l -> toTicketLine(tenantId, sellerTerminalId, l, currency)).toList();
  }
  ...
  private TicketLine toTicketLine(
    ...
    for (int i = 0; i < termPlan.terms().size(); i++) {
      ...
      var resolvedPayoutRule = resolvePayoutRule(tenantId, sellerTerminalId, input, plannedTerm);
      var payoutRule = resolvedPayoutRule.payoutRule();
      preparedTerms.add(
          new PreparedSettlementTerm(
    ...
    return new TicketLine(
        ...
        selectionApi.canonicalize(
            input.betType(), termPlan.canonicalBetOption(), input.rawSelection()),
  ...
  private SettlementTermsSnapshot settlementTermsSnapshot(
      TicketSettlementTermPlan termPlan, List<PreparedSettlementTerm> preparedTerms) {
    ...
    for (var preparedTerm : preparedTerms) {
      ...
      terms.add(
          new SettlementTermSnapshot(
              ...
              policy == SelectionPolicy.IMPLICIT_BEST_MATCH
                  ? null
                  : termPlan.betOptionLabelSnapshot(),
              payoutRule,
              payoutRule.type() == PayoutRuleType.STAKE_MULTIPLIER
                  ? preparedTerm.stakeAmount()
                  : null,
  ...
  private ResolvedPayoutRule resolvePayoutRule(
    ...
    if (resolution.effectiveRuleType() == PayoutRuleType.FIXED_AMOUNT) {
      return new ResolvedPayoutRule(
          PayoutRuleSnapshot.fixedAmount(resolution.effectiveFixedAmount()), resolution.source());
    }
    return new ResolvedPayoutRule(
  ...
  private static List<BigDecimal> stakeAllocation(
      BigDecimal stake, TicketSettlementTermPlan termPlan) {
...
record ResolvedPayoutRule(PayoutRuleSnapshot payoutRule, OddsSource source) {}
