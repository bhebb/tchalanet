...
public class LimitResolver {
  ...
  public EffectiveLimits resolve(List<LimitAssignment> assignments, LimitContext ctx) {
    ...
    return switch (scope) {
      ...
      case LimitScopeRef.SellerTerminalScope sellerTerminalScope ->
          ctx.sellerTerminalId() != null
                  && ctx.sellerTerminalId().equals(sellerTerminalScope.sellerTerminalId())
              ? 60
              : -1;

      case LimitScopeRef.DrawChannelScope channel ->
          ctx.drawChannelId() != null && ctx.drawChannelId().equals(channel.drawChannelId())
              ? 30
              : -1;
