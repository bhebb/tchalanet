...
/**
 * Maps the CA DrawGamePastDrawResults JSON. The payload is a single object with a {@code
 * PreviousDraws} array (plus a {@code MostRecentDraw}); each draw carries an ISO {@code DrawDate}
 * and a position-keyed {@code WinningNumbers} object ({@code
 * {"1":{"Number":"5"},"2":{"Number":"8"},...}}).
 *
 ...
 * providerSlotCode therefore selects EVENING = highest DrawNumber, MIDDAY = next; a blank slot
 * takes the most recent draw of the date.
 */
@Component
@Slf4j
