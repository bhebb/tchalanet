...
/**
 * Recomputes the settlement variant of a sale line from {@code betType + betOption + selection} and
 * exposes commercial/admin labels. No persistence, no bus — pure delegation to the domain resolver
 * plus catalog labels.
 */
@Component
public class SettlementExplanationService implements SettlementExplanationApi {
  ...
  public ComputedSettlementVariantView explain(BetType betType, Short betOption, String selection) {
    ...
    return new ComputedSettlementVariantView(
        variant.name(), commercialLabel(betType, betOption, variant), variant.adminLabel());
  }

  private static String commercialLabel(
      BetType betType, Short betOption, PricingVariantCode variant) {
