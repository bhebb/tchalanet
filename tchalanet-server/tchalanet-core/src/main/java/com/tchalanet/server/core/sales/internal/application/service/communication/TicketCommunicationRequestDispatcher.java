...
public class TicketCommunicationRequestDispatcher {
  ...
  public void enqueueTicketPlaced(
      Ticket ticket, SaleCommunicationOptions options, CorrelationId correlationId) {
    ...
    if (options.sendWhatsapp()) {
      enqueue(
          ticket, CommunicationChannel.WHATSAPP, options.buyerPhoneNumber(), locale, correlationId);
    }
  ...
  private void enqueue(
    ...
    var request =
        new SendOutboundMessageRequest(
            ...
            channel,
            new OutboundRecipient(ticket.identity().tenantId(), null, destination, null),
            locale,
            metadata);
  ...
  private Map<String, Object> ticketMetadata(
      Ticket ticket, CorrelationId correlationId, CommunicationChannel channel) {
    ...
    metadata.put(
        "idempotencyKey",
        "sales.ticket-placed:" + ticket.identity().id().value() + ":" + channel.name());
