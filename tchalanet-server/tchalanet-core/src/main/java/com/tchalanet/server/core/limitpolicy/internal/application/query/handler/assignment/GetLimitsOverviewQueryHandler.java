...
public class GetLimitsOverviewQueryHandler
  ...
  public LimitsOverviewView handle(GetLimitsOverviewQuery query) {
    var assignments =
        assignmentReader.listByTarget(query.limitScopeRef()).stream()
            .filter(a -> !a.deleted())
            .map(
                a ->
                    new LimitsOverviewView.Assignment(
                        a.id(),
                        a.ruleKey(),
                        a.enabled(),
                        a.onBreach(),
    ...
    return new LimitsOverviewView(query.limitScopeRef(), assignments);
  }
}
