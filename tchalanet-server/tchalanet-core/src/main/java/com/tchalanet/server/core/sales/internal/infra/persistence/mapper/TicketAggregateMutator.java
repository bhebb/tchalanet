...
public class TicketAggregateMutator {
  ...
  private void assertImmutableFields(TicketJpaEntity managed, Ticket domain) {
    ...
    requireSame(
        "sellerTerminalId",
        managed.getSellerTerminalId(),
        domain.context().sellerTerminalId().value());
    requireSame("drawId", managed.getDrawId(), domain.context().drawId().value());
    requireSame(
        "drawChannelId", managed.getDrawChannelId(), domain.context().drawChannelId().value());
    ...
    requireSame(
        "verificationCode",
        managed.getVerificationCode(),
        domain.codes().verificationCode().value());
    ...
    requireSame(
        "stakeAmount", managed.getStakeAmount(), domain.money().breakdown().stake().amount());
    requireSame(
        "totalAmount", managed.getTotalAmount(), domain.money().breakdown().total().amount());
  }
  ...
  private void assertImmutableLineFields(
      TicketLineJpaEntity managed, TicketLine domain, Ticket ticket) {
    ...
    requireSame(
        ...
        managed.getSelectionPolicySnapshot(),
        domain.selectionPolicySnapshot());
    requireSame(
        "line.betOptionLabelSnapshot",
        managed.getBetOptionLabelSnapshot(),
        domain.betOptionLabelSnapshot());
    requireSame("line.selectionKey", managed.getSelectionKey(), domain.selection().key().value());
    requireSame(
        "line.displaySelection", managed.getDisplaySelection(), domain.selection().displayLabel());
    requireSame("line.stakeAmount", managed.getStakeAmount(), domain.stakeAmount().amount());
    requireSame(
        "line.settlementTermsSnapshot",
        managed.getSettlementTermsSnapshot(),
        domain.settlementTermsSnapshot());
  }
  ...
  private record ChargeKey(Object type, Object paidBy) {
