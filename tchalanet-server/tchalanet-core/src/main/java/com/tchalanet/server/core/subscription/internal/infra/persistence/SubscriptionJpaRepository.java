...
/**
 * Repository for tenant_subscription (core/subscription). RLS enforced via Postgres policies
 * (tenant_id).
 */
@Repository
public interface SubscriptionJpaRepository extends JpaRepository<SubscriptionJpaEntity, UUID> {

  /** Find subscription by tenant_id (RLS enforced). Maps to spec S7 (tenant-scoped persistence). */
