...
public interface TicketJpaMapper {
  ...
  default TicketJpaEntity toEntity(Ticket ticket) {
    ...
    var chargeEntities =
        ticket.money().breakdown().charges().stream().map(this::toChargeEntity).toList();
  ...
  default List<TicketChargeJpaEntity> toChargeEntities(List<TicketCharge> charges) {
    ...
    return charges.stream().map(this::toChargeEntity).toList();
  }
  ...
  default TicketChargeJpaEntity toChargeEntity(TicketCharge charge) {
    ...
    entity.setWaivedByRuleId(
        charge.waivedByRuleId() == null ? null : charge.waivedByRuleId().value());
    entity.setWaivedByDecisionId(
        charge.waivedByDecisionId() == null ? null : charge.waivedByDecisionId().value());
  ...
  default TicketIdentity toIdentity(TicketJpaEntity entity) {
    return new TicketIdentity(TicketId.of(entity.getId()), TenantId.of(entity.getTenantId()));
  }
  ...
  default TicketLifecycle toLifecycle(TicketJpaEntity entity) {
    return new TicketLifecycle(
        toSaleLifecycle(entity), toResultLifecycle(entity), toSettlementLifecycle(entity));
  }
  ...
  default DecisionTrace toVoidedTrace(TicketJpaEntity entity) {
    ...
    return new DecisionTrace(
        entity.getVoidedAt(), nullableUserId(entity.getVoidedBy()), entity.getVoidReason());
  }

  default TicketOrigin toOrigin(TicketJpaEntity entity) {
    return new TicketOrigin(entity.getSaleChannel());
  }
  ...
  default List<TicketLine> toDomainLines(
      List<TicketLineJpaEntity> entities, @Context CurrencyCode currency) {
    if (entities == null) return List.of();

    return entities.stream().map(line -> toDomainLine(line, currency)).toList();
  }
  ...
  default TicketLine toDomainLine(TicketLineJpaEntity entity, @Context CurrencyCode currency) {
    return new TicketLine(
        ...
        entity.getGameCode(),
        entity.getBetType(),
        new Selection(SelectionKey.of(entity.getSelectionKey()), entity.getDisplaySelection()),
        new Money(entity.getStakeAmount(), currency),
        ...
        entity.getPricingSource() == null
            ? TicketLinePricingSource.STANDARD
            : entity.getPricingSource(),
        entity.getSelectionSource() == null
            ? TicketLineSelectionSource.CUSTOMER_SELECTED
            : entity.getSelectionSource(),
        entity.getPromotionDecisionId() == null
            ? null
            : PromotionDecisionId.of(entity.getPromotionDecisionId()),
  ...
  default void applyContext(TicketContext context, @MappingTarget TicketJpaEntity entity) {
    ...
    entity.setSellerTerminalId(
        context.sellerTerminalId() == null ? null : context.sellerTerminalId().value());
  ...
  default void applySettlementLifecycle(
      SettlementLifecycle settlement, @MappingTarget TicketJpaEntity entity) {
  ...
  default List<TicketCharge> toDomainCharges(
      List<TicketChargeJpaEntity> entities, CurrencyCode currency) {
    if (entities == null) {
      return List.of();
    }

    return entities.stream().map(entity -> toDomainCharge(entity, currency)).toList();
  }

  default TicketCharge toDomainCharge(TicketChargeJpaEntity entity, CurrencyCode currency) {
    return new TicketCharge(
        entity.getChargeType(),
        new Money(entity.getAmount(), currency),
        entity.getPaidBy(),
        entity.getWaivedByDecisionId() == null
            ? null
            : PromotionDecisionId.of(entity.getWaivedByDecisionId()),
