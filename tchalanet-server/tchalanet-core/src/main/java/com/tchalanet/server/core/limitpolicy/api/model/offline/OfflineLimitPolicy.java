...
/**
 * Tenant offline policy: bounds within which an {@code OfflineGrant} can be issued and within which
 * submissions remain syncable.
 *
 ...
 * @param syncAcceptedExtension extra grace window after {@code validUntil} during which the server
 *     still accepts late uploads
...
public record OfflineLimitPolicy(
  ...
  public OfflineLimitPolicy {
    ...
    if (maxTicketCount <= 0) throw new IllegalArgumentException("maxTicketCount must be > 0");
  }
}
