...
public class ExposureFactsReaderAdapter implements ExposureFactsReaderPort {
  ...
  public LimitFactsSnapshot snapshot(LimitContext ctx) {
    var betTypes = ctx.lines().stream().map(line -> line.betType()).distinct().toList();
    ...
    for (var scope : ctx.scopes()) {
      ...
      var rows =
          repo.findFactsForBetTypes(
              ctx.drawId().value(), scopeRow.scopeType(), scopeRow.scopeId(), betTypes);

      for (var entity : rows) {
        var key = new LimitFactsSnapshot.Key(scope, entity.getBetType(), entity.getSelectionKey());

        facts.put(
            key,
            new LimitFactsSnapshot.Fact(cents(entity.getStakeTotal()), entity.getSalesCount()));
      }
    }
