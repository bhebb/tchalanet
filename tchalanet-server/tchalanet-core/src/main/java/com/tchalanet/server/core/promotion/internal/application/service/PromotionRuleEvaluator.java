...
public class PromotionRuleEvaluator {
  ...
  private PromotionEffect applyQuantityMode(
      PromotionEffect effect, PromotionEvaluationContext ctx) {
  ...
  private PromotionEffect applyPerPaidAmount(
      PromotionEffect effect, PromotionEvaluationContext ctx) {
  ...
  private PromotionEffect applyTieredPaidAmount(
      PromotionEffect effect, PromotionEvaluationContext ctx) {
    var paidTotal = ctx.paidTotal();
    ...
    return effect.quantityTiers().stream()
        ...
        .filter(
            tier -> tier.maxPaidAmount() == null || paidTotal.compareTo(tier.maxPaidAmount()) <= 0)
        .findFirst()
  ...
  private boolean eligible(PromotionRule rule, PromotionEvaluationContext ctx) {
    ...
    for (var line : rule.eligibilityLines()) {
      var count =
          ctx.paidGameCodes().stream().filter(gameCode -> gameCode.equals(line.gameCode())).count();
