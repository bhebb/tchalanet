...
public class TicketReceiptMessageFormatter {
  ...
  public String formatShareableText(TicketReceiptView receipt) {
            ...
            section ->
                section
                    ...
                    .forEach(
                        line ->
                            lines.add(
                                translations.text(TicketReceiptI18nKeys.MESSAGE_GAME)
                                    + ": "
                                    + lineLabel(section, line, translations))));
    lines.add(
        translations.text(TicketReceiptI18nKeys.MESSAGE_AMOUNT) + ": " + receipt.totalAmount());
    ...
    lines.set(
        ...
        translations.text(TicketReceiptI18nKeys.MESSAGE_AMOUNT)
            ...
            + moneyFormatter.format(receipt.totalAmount(), null));
    lines.add(
        translations.text(TicketReceiptI18nKeys.VERIFICATION) + ": " + receipt.verificationUrl());
  ...
  private List<String> lineSummaries(
      TicketReceiptView receipt, TicketReceiptI18nResolver.TicketReceiptTranslations translations) {
    return receipt.gameSections().stream()
        .flatMap(
            section -> section.lines().stream().map(line -> lineLabel(section, line, translations)))
        .toList();
  }
