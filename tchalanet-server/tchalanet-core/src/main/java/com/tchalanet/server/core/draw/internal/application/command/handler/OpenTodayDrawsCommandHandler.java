...
public class OpenTodayDrawsCommandHandler
  ...
  private static final String PROVIDER_CLOSED_REASON_LABEL =
      "No provider draw for this slot on this date";
  ...
  public OpenDueDrawsResult handle(OpenTodayDrawsCommand command) {
    ...
    var defaultSalesOpenTime =
        drawProperties.getScheduler().getOpenToday().getDefaultSalesOpenTime();
    var openable =
        port.findOpenableForSalesOpenTime(
            command.now(), command.drawDate(), defaultSalesOpenTime, command.batchSize());
    ...
    int canceledProviderClosed =
        cancelIds.isEmpty()
            ? 0
            : port.bulkCancelScheduled(
                cancelIds,
                PROVIDER_CLOSED_REASON_CODE,
                PROVIDER_CLOSED_REASON_LABEL,
                command.now());
