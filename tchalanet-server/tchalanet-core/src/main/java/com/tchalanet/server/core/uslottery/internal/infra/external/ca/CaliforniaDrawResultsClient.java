 ...
 *
 * <p>DAILY3 -&gt; gameId 9 (confirmed). DAILY4 -&gt; gameId 10 (sequential, needs verification via
 * {@code DrawGamePastDrawResults/10/1/20} — if wrong the fetch returns empty, no crash).
 */
@Component
@Slf4j
...
public class CaliforniaDrawResultsClient implements UsLotteryProviderClient {
  ...
  public UsLotteryProviderResponse fetch(UsLotteryProviderQuery query) {
    ...
    for (var code : query.externalGameCodes()) {
      ...
      var url =
          joinUrl(cfg.getBaseUrl(), cfg.getLatestPath()) + "/" + game.get().gameId() + "/1/20";
      ...
      var body =
          cache.getOrFetch(PROVIDER.name(), query.drawDate(), queryHash, () -> fetchBody(url));
    ...
    return new UsLotteryProviderResponse(
        PROVIDER, query.drawDate(), query.drawTime(), query.timezone(), List.copyOf(merged), null);
  }
