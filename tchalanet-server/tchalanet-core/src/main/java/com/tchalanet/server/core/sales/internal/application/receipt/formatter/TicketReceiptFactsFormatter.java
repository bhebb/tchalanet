...
public class TicketReceiptFactsFormatter {
  ...
  public List<TicketReceiptTextLine> ticketIdentityLines(
    ...
    if (profile != null && profile.compact()) {
      ...
      addLabel(
          lines,
          translations.text(TicketReceiptI18nKeys.TICKET),
          receipt.ticketCode(),
          true,
          profile);
      addLabel(
    ...
    addLabel(
        lines,
        translations.text(TicketReceiptI18nKeys.SALE_TIMESTAMP),
        formatInstant(receipt.placedAt(), receipt.timezone()),
        false,
        profile);
    addLabel(
        lines,
        translations.text(TicketReceiptI18nKeys.TERMINAL),
        receipt.terminalCode(),
        false,
        profile);
    addLabel(
        lines,
        translations.text(TicketReceiptI18nKeys.SELLER),
        receipt.sellerDisplayName(),
        false,
        profile);
