 ...
 *
 * <p>Belongs to a {@link TicketJpaEntity}; cascaded via the parent. Currency is inherited from the
 * parent ticket (not stored on the line).
 */
@Entity
@Getter
@Setter
...
@Table(
    ...
    indexes = {
      ...
      @Index(
          name = "idx_ticket_line_tenant_draw_game",
          columnList = "tenant_id, draw_id, game_code"),
      @Index(name = "idx_ticket_line_result_status", columnList = "tenant_id, result_status")
    },
    uniqueConstraints = {
      @UniqueConstraint(
