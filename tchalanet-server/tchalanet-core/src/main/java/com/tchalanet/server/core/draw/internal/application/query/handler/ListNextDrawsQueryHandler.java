...
@Slf4j
public class ListNextDrawsQueryHandler
    implements QueryHandler<ListNextDrawsQuery, TchPage<DrawSummary>> {
  ...
  public TchPage<DrawSummary> handle(ListNextDrawsQuery query) {
    ...
    log.debug(
        "Listing next draws for slot: {}, lookahead: {}h, limit: {}",
        query.resultSlotId(),
        query.lookaheadHours(),
        query.limitPerChannel());

    var criteria =
        DrawSearchCriteria.forNext(
