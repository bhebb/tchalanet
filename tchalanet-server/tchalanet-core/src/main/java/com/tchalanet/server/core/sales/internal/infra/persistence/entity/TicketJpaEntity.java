...
/**
 * JPA entity for the {@link Ticket} aggregate root.
 *
 * <p>Inherits from {@link BaseTenantEntity} which provides {@code id}, {@code tenant_id}, {@code
 * version}, technical audit columns, and {@code deleted_at}.
 *
 * <p>Soft-delete is enabled at the ticket level via {@link SQLDelete} + {@link SQLRestriction}. The
 * DELETE statement is guarded by {@code version} to interact correctly with optimistic locking.
 *
 * <p>Tenant isolation is enforced by the RLS policy on {@code sales_ticket} (see {@code
 * V20260515__create_sales_ticket.sql}). No application-side tenant filtering is required.
 *
...
@Table(
    name = "sales_ticket",
    indexes = {
      @Index(name = "idx_sales_ticket_tenant", columnList = "tenant_id"),
      @Index(name = "idx_sales_ticket_tenant_draw", columnList = "tenant_id, draw_id"),
      @Index(
          name = "idx_sales_ticket_tenant_seller_terminal",
          columnList = "tenant_id, seller_terminal_id"),
      @Index(name = "idx_sales_ticket_tenant_sale_status", columnList = "tenant_id, sale_status"),
      @Index(
          name = "idx_sales_ticket_tenant_result_status",
          columnList = "tenant_id, result_status"),
      @Index(
          name = "idx_sales_ticket_tenant_settlement_status",
          columnList = "tenant_id, settlement_status"),
      @Index(name = "idx_sales_ticket_sold_at_desc", columnList = "tenant_id, sold_at DESC")
    },
    uniqueConstraints = {
      @UniqueConstraint(
...
public class TicketJpaEntity extends BaseTenantEntity {
  ...
  @NotAudited
  @Formula(
      "(SELECT rs.slot_key FROM draw_channel dc JOIN result_slot rs ON rs.id = dc.result_slot_id WHERE dc.id = draw_channel_id)")
  private String resultSlotKey;

  /** Result provider code — resolved via sub-select for list projections. */
  @NotAudited
  @Formula(
      "(SELECT rs.provider FROM draw_channel dc JOIN result_slot rs ON rs.id = dc.result_slot_id WHERE dc.id = draw_channel_id)")
  private String resultProvider;

  /** Result timezone — resolved via sub-select for list projections. */
  @NotAudited
  @Formula(
      "(SELECT COALESCE(rs.timezone, dc.timezone) FROM draw_channel dc JOIN result_slot rs ON rs.id = dc.result_slot_id WHERE dc.id = draw_channel_id)")
