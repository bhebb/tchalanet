...
public class TenantPricingOddsJpaAdapter implements TenantPricingOddsReaderPort, TenantPricingOddsWriterPort {
    ...
    @Cacheable(
        ...
        key = "#tenantId.value().toString() + ':' + (#gameCode == null ? '' : #gameCode.trim().toUpperCase())")
    public List<TenantPricingOdds> findActiveByTenant(TenantId tenantId, String gameCode) {
        var normalizedGameCode = gameCode == null || gameCode.isBlank()
            ? null
    ...
    @CacheEvict(
        cacheNames = {
            PricingCacheNames.TENANT_ODDS_LIST,
            PricingCacheNames.TENANT_ODDS_BY_VARIANT
        },
        allEntries = true)
    public TenantPricingOdds save(TenantPricingOdds odds) {
        var entity = repo.findByNaturalKey(
                odds.tenantId().value(),
                odds.gameCode(),
                odds.pricingVariantCode().name())
