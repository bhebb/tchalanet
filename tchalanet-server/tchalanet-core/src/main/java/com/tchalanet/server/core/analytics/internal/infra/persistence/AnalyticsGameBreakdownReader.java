 ...
 *
 * <p>{@code analytics_selection} is the current projection that carries stable {@code game_code}.
 * This keeps dashboard/report queries inside core.analytics without bending {@code
 * analytics_daily.dimension_id} into a non-UUID key.
 */
@Component
public class AnalyticsGameBreakdownReader {
  ...
  public List<GameBreakdownRow> findPlatformGameBreakdown(LocalDate from, LocalDate to, int limit) {
  ...
  public record GameBreakdownRow(
      String gameCode, long ticketsSold, BigDecimal grossSales, BigDecimal netRevenueEstimated) {}
}
