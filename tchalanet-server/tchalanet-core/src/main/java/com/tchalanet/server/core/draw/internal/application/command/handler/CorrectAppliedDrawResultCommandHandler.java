 ...
 *
 * <p>Flow : 1. Require drawId, correctedDrawResultId, reason, idempotencyKey 2. Lock/idempotency
 * check 3. Load Draw 4. salesGuard.assertCanCorrectAppliedResult(...) 5. Vérifier draw a déjà
 * drawResultId 6. Vérifier correctedDrawResultId différent de previous 7.
 * draw.overrideResult(correctedDrawResultId, now, reason) 8. Save draw 9. Marquer previous
 * DrawResult comme OVERRIDDEN (TODO: implémenter) 10. Publish DrawResultCorrectedEvent AFTER_COMMIT
 */
@UseCase
...
public class CorrectAppliedDrawResultCommandHandler
  ...
  public void handle(CorrectAppliedDrawResultCommand command) {
    ...
    if (processedEventPort.alreadyProcessed(HANDLER_KEY, idempotentEventId)) {
      log.warn(
          "draw.correct_result duplicate ignored drawId={} key={}",
          command.drawId(),
    ...
    salesGuard.assertCanCorrectAppliedResult(
        draw.id(), command.correctedDrawResultId(), command.force());

    var now = clock.instant();

    draw.overrideResult(command.correctedDrawResultId(), now, command.reason());
    ...
    var channel =
        drawChannelCatalog
            ...
            .orElseThrow(
                () -> new IllegalStateException("DrawChannel not found: " + draw.drawChannelId()));
    ...
    AfterCommit.run(
        () -> {
          if (!processedEventPort.markProcessedIfAbsent(HANDLER_KEY, idempotentEventId)) {
            log.warn(
                "draw.correct_result duplicate skipped-after-commit drawId={} key={}",
                command.drawId(),
                command.idempotencyKey());
            return;
          }
