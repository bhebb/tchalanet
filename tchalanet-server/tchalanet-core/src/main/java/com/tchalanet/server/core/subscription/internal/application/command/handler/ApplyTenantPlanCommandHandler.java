...
/**
 * Handler for ApplyTenantPlanCommand. Maps to spec requirement S1 (apply plan to tenant).
 *
 * <p>CRITICAL per REFACTORING_GUIDE.md and spec S6: - Validates plan via PlanCatalog.findByCode()
 * (public API only) - NEVER accesses catalog/plan/internal or PlanJpaRepository - Uses planCode
 * (String), not PlanId
 */
@UseCase
...
public class ApplyTenantPlanCommandHandler
  ...
  private final ApplicationEventPublisher
      eventPublisher; // Keep for now, if other events are published
  ...
  public ApplyTenantPlanResult handle(ApplyTenantPlanCommand cmd) {
    ...
    var plan =
        planCatalog
            .findByCode(cmd.planCode())
            .orElseThrow(() -> new IllegalArgumentException("Plan not found: " + cmd.planCode()));

    if (!plan.active()) {
      // Policy: reject inactive plans for new assignments (per spec S1 scenario "plan inactive
      // policy")
