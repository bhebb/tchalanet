...
public class EvaluatePromotionQueryHandler
    implements QueryHandler<EvaluatePromotionQuery, PromotionDecision> {
  ...
  public PromotionDecision handle(EvaluatePromotionQuery q) {
    ...
    var overrides =
        ctx.tenantId() == null || ctx.sellerTerminalId() == null
    ...
    var effectiveHash =
        effectiveRules.overrideHash() == null ? hash : hash + "." + effectiveRules.overrideHash();
    var effects =
        effectiveRules.rules().stream()
