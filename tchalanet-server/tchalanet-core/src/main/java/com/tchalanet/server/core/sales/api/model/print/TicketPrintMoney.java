...
public record TicketPrintMoney(
    Money stake, List<TicketPrintCharge> charges, Money buyerChargesTotal, Money totalAmount) {
  public TicketPrintMoney {
    charges = List.copyOf(charges);
  }

  /**
   * Total amount saved from waived charges (promotion discount on fees). Returns zero if no charges
   * were waived.
   */
