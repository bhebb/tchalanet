...
public class DrawLifecycleJpaAdapter implements DrawLifecyclePort {
  ...
  private OpenableDrawRow mapOpenableRow(
      com.tchalanet.server.core.draw.internal.infra.persistence.projection.OpenableDrawProjection
          projection) {
    TenantId tenantId = TenantId.of(projection.getTenantId());
    DrawId drawId = DrawId.of(projection.getDrawId());
    var resultSlotId =
        projection.getResultSlotId() == null
            ? null
  ...
  private static void assertImmutableFields(
      com.tchalanet.server.core.draw.internal.infra.persistence.DrawJpaEntity entity, Draw draw) {
  ...
  private static void requireSame(String field, Object actual, Object expected) {
    if (!Objects.equals(actual, expected)) {
      throw new IllegalStateException(
          "Draw immutable field changed: " + field + " expected=" + actual + " actual=" + expected);
    }
  }
}
