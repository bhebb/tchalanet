...
public class UsLotteryProviderRawCache {
  ...
  public String getOrFetch(
    ...
    if (existing != null) {
      log.debug(
          "Provider raw cache hit provider={} date={} queryHash={}", provider, drawDate, queryHash);
      return existing;
    }
    ...
    synchronized (lock) {
      try {
        ...
        log.debug(
            "Provider raw cache miss provider={} date={} queryHash={}",
            provider,
            drawDate,
            queryHash);
        ...
        if (value == null) {
          log.debug(
              "Provider returned null; not caching provider={} date={} queryHash={}",
              provider,
              drawDate,
              queryHash);
          return null;
