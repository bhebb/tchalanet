...
public final class SelectionKeyCanonicalizer {
  ...
  public static SelectionKey canonicalize(
      BetType betType, Short rawBetOption, String rawSelectionKey) {
    ...
    String canonical =
        switch (betType) {
          case MATCH_1_2D, MATCH_2_2D, MATCH_3_2D -> canonicalizeDigitsStrict(s, 2);

          case MARRIAGE_2D2D -> canonicalize2dPair(BetOption.from(betType, rawBetOption), s);

          case LOTTO3_3D -> canonicalizeDigits(BetOption.from(betType, rawBetOption), s, 3);

          case LOTTO4_PATTERN -> canonicalizeLotto4(rawBetOption, s);

          case LOTTO5_PATTERN -> canonicalizeDigits(BetOption.from(betType, rawBetOption), s, 5);
        };
  ...
  private static String canonicalizeLotto4(Short rawOption, String s) {
    ...
    return switch (option) {
      case LOTTO4_STRAIGHT, LOTTO4_BOX, LOTTO4_EXACT_PLUS_BOX -> canonicalizeExactDigits(s, 4);

      case LOTTO4_FRONT_PAIR -> canonicalizeLotto4FrontPair(s);

      case LOTTO4_BACK_PAIR -> canonicalizeLotto4BackPair(s);

      default -> throw new IllegalArgumentException("Unsupported LOTTO4 option: " + option);
    };
  }
