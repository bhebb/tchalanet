...
public class TexasDrawResultsMapper {
  ...
  public UsLotteryProviderResponse map(
      String xml, String sourceHash, String url, UsLotteryProviderQuery query) {
    ...
    var results =
        feed.get().getEntries().stream()
            ...
            .filter(
                raw -> ProviderSlotCodeMatcher.matches(raw.slot().name(), query.providerSlotCode()))
  ...
  private Optional<TxRawDraw> toRawDraw(SyndEntry entry) {
    ...
    var description =
        clean(entry.getDescription() == null ? "" : entry.getDescription().getValue());
  ...
  private Optional<UsLotteryProviderResult> toProviderResult(
      TxRawDraw raw, String sourceHash, String url, UsLotteryProviderQuery query) {

    var expectedSize = raw.game().expectedSize;
    var quality =
        raw.main().size() == expectedSize ? ResultQuality.COMPLETE : ResultQuality.SUSPECT;
    ...
    var extras =
        StringUtils.isBlank(raw.fireball()) ? List.<String>of() : List.of(raw.fireball().trim());
