...
class TenantThemeRuntimeServiceTest {
  ...
  private ThemePresetView activePreset(String code) {
    ...
    return new ThemePresetView(
        ThemePresetId.of(UUID.randomUUID()),
        code,
        "tchalanet",
        config,
        null,
  ...
  void returnsTokensForRequestedLightMode() {
    when(persistence.findActiveByTenantId(TENANT))
        .thenReturn(Optional.of(tenantTheme(PRESET_CODE, "SYSTEM")));
  ...
  void returnsTokensForDarkMode() {
    when(persistence.findActiveByTenantId(TENANT))
        .thenReturn(Optional.of(tenantTheme(PRESET_CODE, "DARK")));
    when(themeCatalog.findByCode(PRESET_CODE)).thenReturn(Optional.of(activePreset(PRESET_CODE)));
  ...
  void requestedModeOverridesDefault() {
    when(persistence.findActiveByTenantId(TENANT))
        .thenReturn(Optional.of(tenantTheme(PRESET_CODE, "LIGHT")));
    when(themeCatalog.findByCode(PRESET_CODE)).thenReturn(Optional.of(activePreset(PRESET_CODE)));
  ...
  void fallbackAppliedWhenPresetInactive() {
    var inactivePreset =
        new ThemePresetView(
            ...
            Instant.now());
    when(persistence.findActiveByTenantId(TENANT))
        .thenReturn(Optional.of(tenantTheme("old-preset", "SYSTEM")));
