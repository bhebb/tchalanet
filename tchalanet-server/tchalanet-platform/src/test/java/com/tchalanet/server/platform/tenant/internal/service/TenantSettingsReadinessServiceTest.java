...
class TenantSettingsReadinessServiceTest {
  ...
  void disabledSendChannelDoesNotRequireFeeFields() {
    var delivery =
        new TenantInternalCommunicationConfig.BuyerTicketDeliveryConfig(
  ...
  void enabledReceiptRequiresStructuralOptionsButNotHeaderFooter() {
    ...
    assertThat(section(readiness, "print").blockingReasons())
        .containsExactlyInAnyOrder(
            "settings.print.paper_size_missing", "settings.print.show_qr_code_missing");
  }
  ...
  private TenantInternalDocumentConfig.ReceiptConfig receipt(
      boolean enabled, String header, String footer, String paperSize, Boolean showQrCode) {
    return new TenantInternalDocumentConfig.ReceiptConfig(
        enabled, header, footer, paperSize, showQrCode, "sales.ticket.receipt.v1");
  }
  ...
  private TenantInternalCommunicationConfig.BuyerTicketDeliveryConfig delivery(
    ...
    return new TenantInternalCommunicationConfig.BuyerTicketDeliveryConfig(
        channel, channel, channel);
  }

  private TenantInternalCommunicationConfig.DeliveryChannelConfig channel(
      boolean enabled, String amount, String currency, String paidBy) {
    return new TenantInternalCommunicationConfig.DeliveryChannelConfig(
        enabled, amount == null ? null : new BigDecimal(amount), currency, paidBy);
  }
  ...
  private SectionReadiness section(TenantSettingsReadinessView readiness, String section) {
