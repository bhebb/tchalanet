...
public class TenantGamePersistenceAdapter {
  ...
  public TenantGame save(TenantGame tenantGame) {
    var existing =
        tenantGame.gameId() != null
            ? repository.findByTenantIdAndGameId(
                tenantGame.tenantId().value(), tenantGame.gameId().value())
  ...
  public Optional<TenantGame> findByTenantGameId(TenantId tenantId, TenantGameId tenantGameId) {
    return repository
        .findByIdAndTenantId(tenantGameId.value(), tenantId.value())
        .map(mapper::toDomain);
  }

  public Optional<TenantGame> findByTenantIdAndGameCode(TenantId tenantId, String gameCode) {
    return repository
        .findByTenantIdAndGameCode(tenantId.value(), gameCode.toUpperCase())
        .map(mapper::toDomain);
  }

  public List<TenantGame> findAllByTenantId(TenantId tenantId) {
    return repository.findByTenantId(tenantId.value()).stream().map(mapper::toDomain).toList();
  }
