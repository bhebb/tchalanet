...
class TenantGameBetOptionConfigs {
    ...
    List<TenantBetTypeOptionConfig> normalize(String gameCode, List<TenantBetTypeOptionConfig> requested) {
    ...
    TenantGameBetOptionConfigView toView(TenantGame tenantGame) {
        return new TenantGameBetOptionConfigView(
            tenantGame.gameCode(),
            effectiveFor(tenantGame).stream()
                .map(this::toView)
                .toList());
    }
    ...
    private TenantBetTypeOptionConfig defaultFor(BetType betType) {
        var options = BetOption.allowedFor(betType).stream()
            .filter(BetOption::enabledByDefault)
            .toList();
    ...
    private TenantBetTypeOptionConfig normalizeBetType(TenantBetTypeOptionConfig config) {
        ...
        if (allowedOptions.isEmpty()) {
            if (config.defaultOption() != null) {
                throw new IllegalArgumentException("defaultOption is not allowed for betType: " + config.betType());
            }
            return new TenantBetTypeOptionConfig(config.betType(), policy, null, List.of());
        }

        var requestedOptions = config.options() == null || config.options().isEmpty()
            ? defaultFor(config.betType()).options()
            : config.options();
        validateOptions(config.betType(), requestedOptions, config.defaultOption());
        var normalizedOptions = requestedOptions.stream()
            .sorted(Comparator.comparingInt(TenantBetOptionConfig::displayOrder))
            .toList();
        var defaultOption = config.defaultOption() != null
            ? config.defaultOption()
            : normalizedOptions.stream()
                ...
                .orElseThrow(() -> new IllegalArgumentException(
                    "At least one option must be enabled for betType: " + config.betType()));
        return new TenantBetTypeOptionConfig(config.betType(), policy, defaultOption, normalizedOptions);
    }
    ...
    private void validateSupportedBetTypes(String gameCode, List<TenantBetTypeOptionConfig> configs) {
        ...
        for (var config : configs) {
            ...
            if (!code.supports(config.betType())) {
                throw new IllegalArgumentException("Unsupported betType " + config.betType()
                    + " for game " + gameCode);
            }
    ...
    private void validateOptions(BetType betType, List<TenantBetOptionConfig> options, Short defaultOption) {
        var seen = new HashSet<Short>();
        var enabled = new HashSet<Short>();
        for (var option : options) {
            BetOption.from(betType, option.code());
            if (!seen.add(option.code())) {
                throw new IllegalArgumentException("Duplicate betOption " + option.code()
                    + " for betType " + betType);
            }
        ...
        if (enabled.isEmpty()) {
            throw new IllegalArgumentException("At least one option must be enabled for betType: " + betType);
        }
        if (defaultOption != null && !enabled.contains(defaultOption)) {
            throw new IllegalArgumentException("defaultOption must be an enabled option for betType: " + betType);
        }
    }
