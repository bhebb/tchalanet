 ...
 * tenant override. This is the single place that validates tenant access; callers must not derive
 * an effective tenant by reading headers directly.
 *
 * <p>Contract:
 *
 * <ul>
 *   <li>Normal user — the effective tenant always comes from the user's single active membership. A
 *       request header never selects the tenant for a normal user.
 ...
 *       consults another tenant. Requires {@code X-Tch-Tenant-Override} (valid UUID), the {@code
 *       platform.tenant.override} permission, and a non-blank {@code X-Tch-Override-Reason}. The
 *       target tenant's existence/active status is enforced downstream when tenant metadata is
 *       hydrated.
 * </ul>
 */
@Component
...
public class EffectiveTenantResolver {
  ...
  public EffectiveTenant resolveForAppUser(
      HttpServletRequest request, UserId userId, boolean superAdmin, Set<String> permissionKeys) {
