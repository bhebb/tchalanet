...
/**
 * Orchestrates full tenant user provisioning: 1. Create Firebase identity + app_user + durable
 * external identity mapping. 2. Create tenant membership. 3. Assign the requested role via
 * access-control.
 */
@Service
...
public class TenantUserProvisioningService {
  ...
  /**
   * Provisions a new user in a tenant with an explicit tenantCode. Authorization and tenant context
   * are resolved from Tchalanet after authentication.
   */
  @Transactional
  ...
  /**
   * Provisions a new user from the admin controller, in the current request's tenant context. The
   * membership carries the requested outlet/terminal, and {@code actor} is recorded as the role
   * assigner.
   */
  @Transactional
  public CreateUserResult provisionTenantUser(
  ...
  private void assignMembershipAndRole(
    ...
    if (role != null) {
      accessControlApi.assignRoleToUser(
          new AssignRoleToUserRequest(tenantId, userId, role.name(), actor));
