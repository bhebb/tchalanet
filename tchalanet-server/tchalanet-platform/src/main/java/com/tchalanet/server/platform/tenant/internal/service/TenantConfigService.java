...
public class TenantConfigService {
  ...
  public void createTenant(CreateTenantRequest request) {
    ...
    publishStatus(
        now,
        saved.id(),
        Boolean.TRUE.equals(request.activate()) ? TenantStatus.DRAFT : null,
        saved.status(),
        Boolean.TRUE.equals(request.activate()) ? "activated_on_create" : "tenant_created");
  }
  ...
  private JsonNode loadDefaultTenantInternalSettings() {
    ...
    try {
      ...
      Arrays.sort(
          resources, Comparator.comparing(r -> r.getFilename() == null ? "" : r.getFilename()));
        ...
        try (var is = resource.getInputStream()) {
          ...
          if (!fragment.isObject()) {
            throw new IllegalStateException(
                "Tenant config fragment must be a JSON object: " + resource.getFilename());
          }
  ...
  public TchPage<TenantSummaryView> listTenants(ListTenantsRequest request) {
    var page =
        tenantRepository.search(
            request.search().likePattern(), request.status(), request.pageable());
  ...
  public void updateTenantIdentity(UpdateTenantIdentityRequest request) {
    ...
    if (request.displayName() != null
        && !Objects.equals(request.displayName(), tenant.displayName())) {
    ...
    if (request.timezone() != null || request.currency() != null) {
      ...
      var currency =
          request.currency() == null ? tenant.currency() : Currency.getInstance(request.currency());
  ...
  @Transactional(readOnly = true)
  public TenantRuntimeView getTenantRuntimeView(
    ...
    var locale =
        tenants
            ...
            .map(
                tc -> {
                  return jsonUtils.treeToValue(
                      persistedConfigCopy(tc), TenantInternalSettings.class);
                })
            .map(TenantInternalSettings::locale)
            .orElse(null);
    List<String> supportedLocales =
        locale != null ? locale.effectiveSupportedLanguages() : java.util.List.of();
  ...
  private TenantConfigView toView(TenantContextLookupView registry, boolean includeDetails) {
    ...
    var themeCode =
        includeDetails && registry.activeThemeId().isPresent()
            ? themeCatalog
                .findById(registry.activeThemeId().get())
    ...
    BigDecimal defaultCommissionRate =
        jpa.map(TenantJpaEntity::getDefaultCommissionRate).orElse(null);
