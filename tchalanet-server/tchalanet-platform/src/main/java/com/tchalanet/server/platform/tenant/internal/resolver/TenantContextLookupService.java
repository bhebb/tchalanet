...
/**
 * Pre-context tenant lookup — resolves tenant identity before RLS binding. Uses {@link
 * TenantPreContextLookupApi} (rawDataSource, read-only, cached).
 */
@Component
...
public class TenantContextLookupService implements TenantContextLookup {
  ...
  public Optional<TenantContextInfo> findByCode(String tenantCode) {
    ...
    return registry
        .findByCode(tenantCode.trim().toLowerCase())
