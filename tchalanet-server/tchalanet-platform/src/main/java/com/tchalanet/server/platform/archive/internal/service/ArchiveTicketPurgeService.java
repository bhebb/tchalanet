 ...
 *
 * <p>This is intentionally not partition DDL. It performs bounded deletes for the current sales
 * ticket schema, where ticket children reference {@code sales_ticket(id)}.
 */
@Service
...
public class ArchiveTicketPurgeService {
  ...
  public TicketPurgePlan plan(UUID tenantId, LocalDate periodStart, LocalDate periodEnd) {
    ...
    if (!archiveVerified(tenantId, periodStart, periodEnd)) {
      return new TicketPurgePlan(
          ...
          archived.charges(),
          false,
          "verified archive objects are missing or invalid");
    }
    ...
    if (!hot.equals(archived)) {
      return new TicketPurgePlan(
          ...
          archived.charges(),
          false,
          "hot row counts do not match verified archive row counts");
    }
  ...
  @Transactional
  public TicketPurgeResult purge(
      UUID tenantId,
      LocalDate periodStart,
      LocalDate periodEnd,
      int batchSize,
    ...
    log.warn(
        "archive ticket purge executed: tenant={} period={}/{} charges={} lines={} tickets={} requestedBy={} reason={}",
        tenantId,
        periodStart,
        periodEnd,
        deletedCharges,
  ...
  private long deleteLoop(
      String sql, UUID tenantId, LocalDate periodStart, LocalDate periodEnd, int batchSize) {
    long total = 0;
    int deleted;
    do {
      deleted =
          jdbc.update(
              sql, params(tenantId, periodStart, periodEnd).addValue("batchSize", batchSize));
      total += deleted;
  ...
  private TicketPurgePlan notEligible(
      UUID tenantId, LocalDate periodStart, LocalDate periodEnd, String reason) {
    Counts hot = hotCounts(tenantId, periodStart, periodEnd);
    Counts archived = archivedCounts(tenantId, periodStart, periodEnd);
    return new TicketPurgePlan(
        tenantId,
