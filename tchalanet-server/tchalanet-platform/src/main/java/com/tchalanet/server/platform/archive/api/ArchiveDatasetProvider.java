 ...
 *
 * <ul>
 *   <li>Implementations live in the <em>owning</em> module (e.g., {@code core.sales}, {@code
 *       platform.audit}), never in {@code platform.archive.internal}.
 *   <li>{@code platform.archive} orchestrates providers discovered via Spring injection; it only
 *       sees this interface, never the implementation class.
 *   <li>Implementations may use their own internal repositories — that is safe because they reside
 *       in their own module and are not imported by {@code platform.archive}.
 * </ul>
 */
public interface ArchiveDatasetProvider {
   ...
   *
   * <p>{@code tenantId} is {@code null} for global/platform datasets. The orchestrator uses the
   * plan to decide whether to proceed, skip or warn.
   */
   ...
   *
   * <p>Must be idempotent: if the object already exists for this runId+segmentNo, return the
   * existing result without re-writing.
   */
   ...
   *
   * <p>Returns {@link ArchiveLookupResult#notFound()} when no archive record matches. Never throws
   * for missing data — callers interpret {@code found == false}.
   */
   ...
   *
   * <p>Called by the executor after saving the archive object to DB. The provider knows which
   * fields are meaningful for lookup (e.g., entityId, publicCode, businessDate). The executor
   * inserts the returned entries into {@code archive_lookup_index}.
   *
   * <p>Return an empty list if this dataset does not support per-entity lookup.
   */
  List<ArchiveLookupEntry> generateLookupRows(
