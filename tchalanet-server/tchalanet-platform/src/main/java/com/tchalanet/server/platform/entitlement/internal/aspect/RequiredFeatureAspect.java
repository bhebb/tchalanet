...
/**
 * Aspect that intercepts methods annotated with {@link RequiredFeature} (or whose declaring class
 * is annotated) and checks the current tenant has the required feature enabled.
 *
 * <p>The annotation is resolved from the join point rather than bound as an advice parameter: a
 * pointcut of the form {@code @annotation(x) || @within(x)} cannot reliably bind {@code x} in
 * Spring AOP (when only one branch matches the other binds {@code null}), which previously caused
 * an NPE on every {@code @RequiredFeature} endpoint.
 */
@Aspect
@Component
...
public class RequiredFeatureAspect {
  ...
  @Before(
      "@annotation(com.tchalanet.server.platform.entitlement.api.RequiredFeature)"
