...
public class EscPosDocumentRenderer implements DocumentRenderer {
  ...
  public EscPosDocumentRenderer(
      EscPosBuilder escpos, @Qualifier("qrEscPosNativeRenderer") QrRenderer qrEscPos) {
    this.escpos = escpos;
  ...
  private void appendLines(List<byte[]> parts, List<DocumentLine> lines, EscPosCodePage codePage) {
    for (DocumentLine line : lines) {
      var style = line.style() == null ? LineStyle.NORMAL : line.style();

      boolean bold =
          style == LineStyle.BOLD || style == LineStyle.TITLE || style == LineStyle.WARNING;
