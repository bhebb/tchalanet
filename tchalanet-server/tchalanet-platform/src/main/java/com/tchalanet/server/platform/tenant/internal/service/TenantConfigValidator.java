...
/**
 * Validates the tenant config JSONB structure for all 4 sections: rules, document, communication,
 * locale.
 *
 * <p>Called both on tenant creation (full validation) and on settings update. All validation throws
...
public class TenantConfigValidator {
  ...
  private static final List<String> TOP_LEVEL_KEYS =
      List.of("rules", "document", "communication", "locale");
            ...
            entry -> {
              if (!TOP_LEVEL_KEYS.contains(entry.getKey())) {
                throw new IllegalArgumentException(
                    "tenant config contains unknown top-level key: '"
                        + entry.getKey()
                        + "'. Allowed: "
                        + TOP_LEVEL_KEYS);
              }
  ...
  public void validateCommunicationConfig(JsonNode config) {
    var typed = jsonUtils.treeToValue(config, TenantInternalSettings.class);
    if (typed == null
        || typed.communication() == null
        || typed.communication().buyerTicketDelivery() == null) {
      return;
    }
  ...
  private void validateDeliveryChannel(
      String path, TenantInternalCommunicationConfig.DeliveryChannelConfig channel) {
    if (channel == null) {
      return;
    }
  ...
  private void validateHolidayRules(
      List<
              com.tchalanet.server.platform.tenant.api.model.view.TenantBusinessCalendarRules
                  .TenantRecurringHolidayRule>
          holidays) {
    if (holidays == null) {
      return;
    }
