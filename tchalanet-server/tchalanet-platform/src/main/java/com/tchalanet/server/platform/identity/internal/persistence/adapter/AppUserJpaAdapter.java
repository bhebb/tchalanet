...
public class AppUserJpaAdapter {
  ...
  public Page<AppUser> findAll(Pageable pageable) {
    ...
    return new PageImpl<>(
        page.getContent().stream().map(this::toUser).toList(), pageable, page.getTotalElements());
  }
  ...
  public List<AppUser> findUnassigned(String q, int page, int size) {
    ...
    return repository.findUnassigned(nameLike, size, offset).stream().map(this::toUser).toList();
  }
  ...
  private void persistExternalIdentity(
      AppUserJpaEntity appUser, ExternalUserSubject externalSubject) {
