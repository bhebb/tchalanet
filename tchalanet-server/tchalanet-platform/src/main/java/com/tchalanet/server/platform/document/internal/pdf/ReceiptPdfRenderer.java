...
public class ReceiptPdfRenderer {
  ...
  public byte[] render(
      ReceiptModel model, byte[] qrPngBytes, ReceiptModel postQrModel, PaperSize paperSize) {
  ...
  private float drawLine(
      ...
      throws IOException {
    var font =
        line.style() == ReceiptLineStyle.BOLD
  ...
  private float calculateThermalHeight(
      ReceiptModel model, byte[] qrPngBytes, ReceiptModel postQrModel) {
  ...
  private void drawText(
      PDPageContentStream cs, String text, float x, float y, PDType1Font font, float fontSize)
      throws IOException {
    cs.beginText();
