...
public record TenantConfig(
  ...
  public TenantConfig clearAddressId(Instant now) {
    return new TenantConfig(
        id, code, name, displayName, type, timezone, currency, status, config, null, activeThemeId);
  }

  /** Set active theme. */
  public TenantConfig withActiveThemeId(ThemePresetId themeId, Instant now) {
    return new TenantConfig(
        id, code, name, displayName, type, timezone, currency, status, config, addressId, themeId);
  }

  /** Clear active theme. */
  public TenantConfig clearActiveThemeId(Instant now) {
    return new TenantConfig(
        id, code, name, displayName, type, timezone, currency, status, config, addressId, null);
  }
