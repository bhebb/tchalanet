...
public class ContactRequestSubmissionService implements ContactRequestApi {
  ...
  public ContactRequestSubmittedView submit(SubmitContactRequestCommand command) {
    ...
    if (result.notificationFailed()) {
      log.warn(
          "Contact request {} saved but internal notification failed", result.view().requestId());
    }
    return result.view();
  }
  ...
  public SubmitContactResult submitWithNotification(SubmitContactRequestCommand command) {
    ...
    var entity =
        ContactRequestJpaEntity.create(
            UUID.randomUUID(),
            reference,
            command.intent(),
            command.fullName(),
    ...
    var view =
        new ContactRequestSubmittedView(
            reference,
            "RECEIVED",
  ...
  private boolean sendNotification(ContactRequestJpaEntity entity) {
    ...
    try {
      ...
      log.error(
          "Failed to send internal notification for contact request {}: {}",
          entity.getReference(),
          ex.getMessage(),
          ex);
      return false;
