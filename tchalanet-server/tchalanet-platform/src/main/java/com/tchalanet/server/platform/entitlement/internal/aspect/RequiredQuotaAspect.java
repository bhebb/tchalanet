...
/**
 * Aspect that intercepts methods annotated with {@link RequiredQuota} and checks if the current
 * tenant respects the defined quota. The 'usage' value is resolved by delegating to {@link
 * UsageResolver}.
 */
@Aspect
@Component
...
public class RequiredQuotaAspect {
  ...
  public void checkQuota(RequiredQuota requiredQuota) {
    ...
    entitlementApi.requireLimitAtMost(tenantId, requiredQuota.limit(), requestedUsage);
  }
}
