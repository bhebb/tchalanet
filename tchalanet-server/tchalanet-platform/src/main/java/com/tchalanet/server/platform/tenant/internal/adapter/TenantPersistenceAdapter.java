...
/**
 * Persistence adapter for TenantConfig. Implements write port with separate create/update methods.
 * Per DOMAIN_TENANT_CONFIG.md: - No RLS on tenant table (platform registry) - Uses typed IDs
 * (TenantId) - Maps domain ↔ entity via TenantMapper - create() for INSERT only - update() for
 * UPDATE only (throws EntityNotFoundException if not found) - getRequiredByIdActive() for direct
 * required reads in application services - Evicts catalog/tenant caches on write (per user request)
 */
@Component
