 ...
 * ExpectedActorTypeResolver}): {@code POS} resolves a SellerTerminal, otherwise an AppUser. There
 * is <strong>no fallback</strong> between the two resolvers — a POS request with no terminal
 * mapping is denied; it does not try the AppUser resolver (and vice versa). This avoids a wasted DB
 * lookup for terminal requests and keeps actor resolution unambiguous.
 */
@Component
@Slf4j
...
public class UserBootstrapFilterImpl implements IdentityBootstrapStep {
  ...
  private void bootstrapAppUser(
      HttpServletRequest request, ExternalAuthenticatedUser externalUser) {
  ...
  private void bootstrapSellerTerminal(
    ...
    var bootstrappedActor =
        BootstrappedActor.sellerTerminal(
