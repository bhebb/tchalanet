...
public class UsageResolver {
  ...
  public int currentUsage(TenantId tenantId, String usageKey) {
    ...
    if (matching.isEmpty()) {
      throw ProblemRest.internal("No usage provider found for usageKey: " + usageKey);
    }

    if (matching.size() > 1) {
      throw ProblemRest.internal("Multiple usage providers found for usageKey: " + usageKey);
    }
