 ...
 *
 * <p>DP3 cascade: 1) Tenant default theme (tenant_theme where is_default=true) 2) Platform default
 * (single preset flagged as default; if none, conventional code default-light) 3) Hardcoded safe
 * preset default-light (last resort)
 */
@Service
...
public class TenantThemeFallbackService {
  ...
  /**
   * Resolves a fallback preset code for a tenant when the requested preset is unavailable. Never
   * returns null.
   */
  ...
  private boolean isAvailable(String code) {
    return themeCatalog.findByCode(code).filter(ThemePresetView::active).isPresent();
  }
}
