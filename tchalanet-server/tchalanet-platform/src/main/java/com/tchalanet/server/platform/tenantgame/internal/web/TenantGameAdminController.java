...
public class TenantGameAdminController {
        ...
        games.stream()
            .map(
                g -> {
                  var catalogName =
                      gameCatalog
                          .findByCode(g.gameCode())
                          .map(gv -> gv.name())
                          .orElse(g.gameCode());
                  var category =
                      gameCatalog.findByCode(g.gameCode()).map(gv -> gv.category()).orElse(null);
  ...
  @GetMapping("/catalog")
  public ApiResponse<List<TenantGameCatalogItemView>> catalog(
  ...
  public ApiResponse<EnableTenantGameResult> enable(
      @PathVariable String gameCode, @CurrentContext TchRequestContext ctx) {
    var result =
        adminService.enableGame(
            EnableTenantGameRequest.builder()
  ...
  public ApiResponse<DisableTenantGameResult> disable(
      @PathVariable String gameCode, @CurrentContext TchRequestContext ctx) {
    var result =
        adminService.disableGame(
            DisableTenantGameRequest.builder()
                .tenantId(ctx.tenantIdRequired())
                .gameCode(gameCode)
                .build());
  ...
  public ApiResponse<TenantGameBetOptionConfigView> updateBetOptions(
      ...
      @CurrentContext TchRequestContext ctx) {
    var result =
        adminService.updateBetOptionConfig(
            UpdateTenantGameBetOptionConfigRequest.builder()
                .tenantId(ctx.tenantIdRequired())
                .gameCode(gameCode)
  ...
  public record UpdateBetOptionsWebRequest(List<TenantBetTypeOptionConfig> betTypes) {}
}
